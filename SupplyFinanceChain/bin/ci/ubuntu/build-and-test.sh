...
# Ensure APP defaults to SupplyFinanceChaind if it's not set.
: ${APP:=SupplyFinanceChaind}
echo "using APP: ${APP}"
...
# in SupplyFinanceChaind that we want to run
...
: ${APP_ARGS:=}
if [[ ${APP} == "SupplyFinanceChaind" ]]; then
