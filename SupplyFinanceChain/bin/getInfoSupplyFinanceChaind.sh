...
SupplyFinanceChaind_exe=/opt/SupplyFinanceChain/bin/SupplyFinanceChaind
conf_file=/etc/opt/SupplyFinanceChain/SupplyFinanceChaind.cfg

while getopts ":e:c:" opt; do
    case $opt in
        e)
            SupplyFinanceChaind_exe=${OPTARG}
            ;;
        c)
            conf_file=${OPTARG}
            ;;
...
tmp_loc=$(mktemp -d --tmpdir SupplyFinanceChain_info.XXXX)
cd /tmp
chmod 751 SupplyFinanceChain_info.*
cd ~
echo ${tmp_loc}

cleaned_conf=${tmp_loc}/cleaned_SupplyFinanceChaind_cfg.txt
...
if [[ -x ${SupplyFinanceChaind_exe} ]]
then
    pgrep SupplyFinanceChaind && \
    ${SupplyFinanceChaind_exe} --conf ${conf_file} \
...
echo "Use the following command on your local machine to download from your SupplyFinanceChaind instance: scp <remote_SupplyFinanceChaind_username>@<remote_host>:${tmp_loc}/info-package.tar.gz <path/to/local_machine/directory>"| tee /dev/fd/3
