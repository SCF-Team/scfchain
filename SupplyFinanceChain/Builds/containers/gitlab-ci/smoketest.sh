...
if [ "$use_private" -gt 0 ] ; then
    REPO_ROOT="https://SupplyFinanceChaind:${ARTIFACTORY_DEPLOY_KEY_SUPPLYFINANCECHAIND}@${ARTIFACTORY_HOST}/artifactory"
else
...
if [ "${pkgtype}" = "dpkg" ] ; then
    ...
    if [ "${install_from}" = "repo" ] ; then
        ...
        # apt-cache policy SupplyFinanceChaind
        apt-get -y install SupplyFinanceChaind=${dpkg_full_version}
    elif [ "${install_from}" = "local" ] ; then
        # cached pkg install
        updateWithRetry
        apt-get -y install libprotobuf-dev libssl-dev
        rm -f build/dpkg/packages/SupplyFinanceChaind-dbgsym*.*
    ...
    if [ "${install_from}" = "repo" ] ; then
        ...
        yum -y update
        # uncomment this next line if you want to see the available package versions
        # yum --showduplicates list SupplyFinanceChaind
        yum -y install ${rpm_version_release}
    elif [ "${install_from}" = "local" ] ; then
        # cached pkg install
        yum install -y yum-utils openssl-static zlib-static
        rm -f build/rpm/packages/SupplyFinanceChaind-debug*.rpm
...
INSTALLED=$(/opt/SupplyFinanceChain/bin/SupplyFinanceChaind --version | awk '{print $NF}')
if [ "${SupplyFinanceChaind_version}" != "${INSTALLED}" ] ; then
    echo "INSTALLED version ${INSTALLED} does not match ${SupplyFinanceChaind_version}"
    exit 1
fi
# run unit tests
/opt/SupplyFinanceChain/bin/SupplyFinanceChaind --unittest --unittest-jobs $(nproc)
/opt/SupplyFinanceChain/bin/validator-keys --unittest
