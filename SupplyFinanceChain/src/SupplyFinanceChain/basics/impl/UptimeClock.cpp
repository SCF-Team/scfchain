...
// On SupplyFinanceChaind shutdown, cancel and wait for the update thread
UptimeClock::update_thread::~update_thread()
{
    if (joinable())
    {
        ...
        // once at SupplyFinanceChaind shutdown.
        join();
    }
}
...
// This actually measures time since first use, instead of since SupplyFinanceChaind start.
...
UptimeClock::now()
{
    ...
    // Return the number of seconds since SupplyFinanceChaind start
