...
// the SupplyFinanceChaind server-server use case, where we only need MITM
...
disallowRenegotiation (SSL const* ssl, bool isNew)
{
    ...
    struct StaticData
    {
        ...
        StaticData()
            : set (SupplyFinanceChain::stopwatch())
        {
        }
    };
...
get_context (std::string const& cipherList)
{
    ...
    // These are the raw DH parameters that SupplyFinanceChain Labs has
    // chosen for SupplyFinanceChain, in the binary format needed by
    // d2i_DHparams.
    //
