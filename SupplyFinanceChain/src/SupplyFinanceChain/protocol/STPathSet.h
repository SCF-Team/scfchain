...
#ifndef SUPPLYFINANCECHAIN_PROTOCOL_STPATHSET_H_INCLUDED
#define SUPPLYFINANCECHAIN_PROTOCOL_STPATHSET_H_INCLUDED
...
public:
    ...
    STPathElement (
        ...
        , mIssuerID (issuer), is_offer_ (isSFC(mAccountID))
    {
        if (!is_offer_)
            mType |= typeAccount;

        if (forceCurrency || !isSFC(currency))
            mType |= typeCurrency;
