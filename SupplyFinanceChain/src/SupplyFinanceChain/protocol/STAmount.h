...
#ifndef SUPPLYFINANCECHAIN_PROTOCOL_STAMOUNT_H_INCLUDED
#define SUPPLYFINANCECHAIN_PROTOCOL_STAMOUNT_H_INCLUDED

#include <SupplyFinanceChain/basics/chrono.h>
#include <SupplyFinanceChain/basics/LocalValue.h>
...
private:
    ...
    bool mIsNative;      // A shorthand for isSFC(mIssue).
    bool mIsNegative;
...
public:
    ...
    STAmount (SFCAmount const& amount);
...
public:
    ...
    STAmount& operator= (SFCAmount const& amount)
    {
...
// IOUAmount and SFCAmount define toSTAmount, defining this
...
inline bool isSFC(STAmount const& amount)
{
    return isSFC (amount.issue().currency);
}
