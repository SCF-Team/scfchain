...
uint256
getSupplyFinanceChainStateIndex (AccountID const& a, AccountID const& b, Currency const& currency)
{
    if (a < b)
        return sha512Half(
            std::uint16_t(spaceSupplyFinanceChain),
            a,
            b,
            currency);
    return sha512Half(
...
uint256
getSupplyFinanceChainStateIndex (AccountID const& a, Issue const& issue)
{
    return getSupplyFinanceChainStateIndex (a, issue.account, issue.currency);
}
...
{
    return { ltSUPPLYFINANCECHAIN_STATE,
        getSupplyFinanceChainStateIndex(id0, id1, currency) };
}
...
{
    return { ltSUPPLYFINANCECHAIN_STATE,
        getSupplyFinanceChainStateIndex(id, issue) };
}
...
payChan (AccountID const& source, AccountID const& dst, std::uint32_t seq)
{
    sha512_half_hasher h;
    using beast::hash_append;
    hash_append(h, std::uint16_t(spaceSFCUChannel));
    hash_append(h, source);
    hash_append(h, dst);
    hash_append(h, seq);
