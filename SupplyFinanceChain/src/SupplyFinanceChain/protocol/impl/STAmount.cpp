    ...
    : STBase(name)
{
    ...
    if (isSFC (issue.currency))
        Throw<std::runtime_error> ("invalid native currency");

    issue.account = sit.get160();

    if (isSFC (issue.account))
...
STAmount::STAmount (SFCAmount const& amount)
    : mOffset (0)
    , mIsNative (true)
    , mIsNegative (amount < beast::zero)
{
...
//
//------------------------------------------------------------------------------
SFCAmount STAmount::sfc () const
{
    if (!mIsNative)
        Throw<std::logic_error> ("Cannot return non-native STAmount as SFCAmount");
...
//
// On the wire:
// - high bit is 0 for SFC, 1 for issued currency
...
//   (but SFC only needs 57 bits for the max value of 10^17 drops)
//
...
amountFromString (Issue const& issue, std::string const& amount)
{
    ...
    // Can't specify SFC using fractional representation
    if (isSFC(issue) && match[3].matched)
        Throw<std::runtime_error> ("SFC must be specified in integral drops.");
...
amountFromJson (SField const& name, Json::Value const& v)
{
    ...
    if (v.isNull())
    {
        Throw<std::runtime_error> ("SFC may not be specified with a null Json value");
    }
    else if (v.isObject())
    {
        value       = v[jss::value];
    ...
    {
        if (v.isObjectOrNull ())
            Throw<std::runtime_error> ("SFC may not be specified as an object");
        issue = sfcIssue ();
    }
    else
    {
...
multiply (STAmount const& v1, STAmount const& v2, Issue const& issue)
{
    ...
    if (v1.native() && v2.native() && isSFC (issue))
    {
    ...
    bool roundUp)
{
    ...
    bool const sfc = isSFC (issue);

    if (v1.native() && v2.native() && sfc)
    {
    ...
    if (resultNegative != roundUp)
        canonicalizeRound (sfc, amount, offset);
    ...
    {
        if (sfc && *stAmountCalcSwitchover2)
        {
    ...
    Issue const& issue, bool roundUp)
{
    ...
    if (resultNegative != roundUp)
        canonicalizeRound (isSFC (issue), amount, offset);

    STAmount result (issue, amount, offset, resultNegative);
    // Control when bugfixes that require switchover dates are enabled
    if (roundUp && !resultNegative && !result && *stAmountCalcSwitchover)
    {
        if (isSFC(issue) && *stAmountCalcSwitchover2)
        {
            // return the smallest value above zero
            amount = 1;
            offset = 0;
