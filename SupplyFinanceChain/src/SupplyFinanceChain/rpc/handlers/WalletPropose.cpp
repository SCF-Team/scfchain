...
#include <SupplyFinanceChain/crypto/KeyType.h>
...
#include <SupplyFinanceChain/protocol/Seed.h>
#include <SupplyFinanceChain/rpc/Context.h>
#include <SupplyFinanceChain/rpc/impl/RPCHelpers.h>
#include <SupplyFinanceChain/rpc/handlers/WalletPropose.h>
...
Json::Value walletPropose (Json::Value const& params)
{
    ...
    bool SupplyFinanceChainLibSeed = false;
    ...
    // SupplyFinanceChain-lib encodes seed used to generate an Ed25519 wallet in a
    // non-standard way. While we never encode seeds that way, we try
    // to detect such keys to avoid user confusion.
    {
        if (params.isMember(jss::passphrase))
            seed = RPC::parseSupplyFinanceChainLibSeed(params[jss::passphrase]);
        else if (params.isMember(jss::seed))
            seed = RPC::parseSupplyFinanceChainLibSeed(params[jss::seed]);

        if(seed)
        {
            SupplyFinanceChainLibSeed = true;
    ...
    if (!SupplyFinanceChainLibSeed && params.isMember (jss::passphrase))
    {
