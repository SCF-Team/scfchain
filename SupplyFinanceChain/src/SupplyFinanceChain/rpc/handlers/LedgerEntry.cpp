...
Json::Value doLedgerEntry (RPC::Context& context)
{
    ...
    else if (context.params.isMember (jss::SupplyFinanceChain_state))
    {
        expectedType = ltSUPPLYFINANCECHAIN_STATE;
        Currency         uCurrency;
        Json::Value     jvSupplyFinanceChainState   = context.params[jss::SupplyFinanceChain_state];

        if (!jvSupplyFinanceChainState.isObject()
            || !jvSupplyFinanceChainState.isMember (jss::currency)
            || !jvSupplyFinanceChainState.isMember (jss::accounts)
            || !jvSupplyFinanceChainState[jss::accounts].isArray()
            || 2 != jvSupplyFinanceChainState[jss::accounts].size ()
            || !jvSupplyFinanceChainState[jss::accounts][0u].isString ()
            || !jvSupplyFinanceChainState[jss::accounts][1u].isString ()
            || (jvSupplyFinanceChainState[jss::accounts][0u].asString ()
                == jvSupplyFinanceChainState[jss::accounts][1u].asString ())
           )
        {
            jvResult[jss::error] = "malformedRequest";
        }
        else
        {
            ...
            if (! id1 || ! id2)
            {
                ...
                jvSupplyFinanceChainState[jss::currency].asString()))
            {
                jvResult[jss::error] = "malformedCurrency";
            }
            else
            {
                uNodeIndex = getSupplyFinanceChainStateIndex(
                    *id1, *id2, uCurrency);
            }
        }
    }
