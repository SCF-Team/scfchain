...
void addLine (Json::Value& jsonLines, SupplyFinanceChainState const& line)
{
    ...
    if (line.getNoSupplyFinanceChain ())
        jPeer[jss::no_SupplyFinanceChain] = true;
    if (line.getNoSupplyFinanceChainPeer ())
        jPeer[jss::no_SupplyFinanceChain_peer] = true;
    if (line.getFreeze ())
...
Json::Value doAccountLines (RPC::Context& context)
{
    ...
    if (params.isMember (jss::marker))
    {
        ...
        auto const sleLine = ledger->read({ltSUPPLYFINANCECHAIN_STATE, startAfter});
        ...
        auto const line = SupplyFinanceChainState::makeItem (accountID, sleLine);
        if (line == nullptr)
            return rpcError (rpcINVALID_PARAMS);
...

    {
        if (! forEachItemAfter(*ledger, accountID,
            ...
            {
                auto const line =
                    SupplyFinanceChainState::makeItem (visitData.accountID, sleCur);
                if (line != nullptr &&
                    (! visitData.hasPeer ||
    ...
    if (visitData.items.size () == reserve)
    {
        ...
        SupplyFinanceChainState::pointer line (visitData.items.back ());
