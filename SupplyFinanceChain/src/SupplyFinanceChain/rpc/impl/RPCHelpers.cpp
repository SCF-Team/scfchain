...
boost::optional<Seed>
parseSupplyFinanceChainLibSeed(Json::Value const& value)
{
    // SupplyFinanceChain-lib encodes seed used to generate an Ed25519 wallet in a
    // non-standard way. While SupplyFinanceChaind never encode seeds that way, we
...
keypairForSignature(Json::Value const& params, Json::Value& error)
{
    ...
    {
        seed = RPC::parseSupplyFinanceChainLibSeed(params[secretType]);
...
chooseLedgerEntryType(Json::Value const& params)
{
    ...
    if (params.isMember(jss::type))
    {
        ...
        { {
            ...
            { jss::state,           ltSUPPLYFINANCECHAIN_STATE },
            { jss::ticket,          ltTICKET }
            } };
