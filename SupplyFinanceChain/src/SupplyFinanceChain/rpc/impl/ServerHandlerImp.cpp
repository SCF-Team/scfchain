...
#include <SupplyFinanceChain/rpc/json_body.h>
#include <SupplyFinanceChain/rpc/ServerHandler.h>
#include <SupplyFinanceChain/server/Server.h>
#include <SupplyFinanceChain/server/impl/JSONRPCUtil.h>
...
#include <SupplyFinanceChain/basics/make_SSLContext.h>
...
#include <SupplyFinanceChain/server/SimpleWriter.h>
            ...
            Json::Value const& jv)
{
    ...
    try
    {
             ...
             jv[jss::command].asString() != jv[jss::method].asString()))
        {
            ...
            if (jv.isMember(jss::SupplyFinanceChainrpc))
                jr[jss::SupplyFinanceChainrpc] = jv[jss::SupplyFinanceChainrpc];
        ...
        boost::string_view forwardedFor, boost::string_view user)
{
    ...
    for (unsigned i = 0; i < size; ++i)
    {
        ...
        std::string SupplyFinanceChainrpc = "1.0";
        if (params.isMember(jss::SupplyFinanceChainrpc))
        {
            if (!params[jss::SupplyFinanceChainrpc].isString())
            {
                usage.charge(Resource::feeInvalidRPC);
                if (!batch)
                {
                    HTTPReply(400, "SupplyFinanceChainrpc is not a string", output, rpcJ);
                    return;
                }

                Json::Value r = jsonRPC;
                r[jss::error] = make_json_error(
                    method_not_found, "SupplyFinanceChainrpc is not a string");
                reply.append(r);
                continue;
            }
            SupplyFinanceChainrpc = params[jss::SupplyFinanceChainrpc].asString();
        }
        ...
        if (params.isMember(jss::SupplyFinanceChainrpc))
           r[jss::SupplyFinanceChainrpc] = params[jss::SupplyFinanceChainrpc];
    ...
    http_request_type const& request) const
{
    ...
    if (app_.serverOkay(reason))
    {
        ...
        msg.body() = "<!DOCTYPE html><html><head><title>" + systemName() +
            " Test page for SupplyFinanceChaind</title></head><body><h1>" +
                systemName() + " Test</h1><p>This page shows SupplyFinanceChaind http(s) "
