...
#include <SupplyFinanceChain/rpc/impl/TransactionSign.h>
...
#include <SupplyFinanceChain/app/tx/apply.h>              // Validity::Valid
    ...
    bool doPath)
{
    ...
    if (!tx_json.isMember (jss::Paths) && params.isMember (jss::build_path))
    {
...

        {
            ...
            if (ledger)
            {
                Pathfinder pf(std::make_shared<SupplyFinanceChainLineCache>(ledger),
    ...
    std::shared_ptr<OpenView const> const& ledger)
{
    ...
    if (signingArgs.isMultiSigning ())
    {
        ...
        auto multisig = SupplyFinanceChain::sign (
            keypair.first,
            keypair.second,
            s.slice());
    ...
    ProcessTransactionFn const& processTransaction)
{
    ...
    // Validate the fields in the serialized transaction.
    {
        ...
        // The Fee field must be in SFC and greater than zero.
        ...
        if (!isLegalNet (fee))
        {
            ...
            err << "Invalid " << sfFee.fieldName
                << " field.  Fees must be specified in SFC.";
            return RPC::make_error (rpcINVALID_PARAMS, err.str ());
        }
        if (fee <= 0)
        {
