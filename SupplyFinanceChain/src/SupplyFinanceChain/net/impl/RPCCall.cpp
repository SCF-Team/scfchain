...
#include <SupplyFinanceChain/net/HTTPClient.h>
...
private:
    ...
    static Json::Value jvParseCurrencyIssuer (std::string const& strCurrencyIssuer)
    {
        ...
        if (boost::regex_match (strCurrencyIssuer, smMatch, reCurIss))
        {
            ...
            if (strIssuer.length ())
            {
                // Could confirm issuer is a valid SupplyFinanceChain address.
...
private:
    ...
    bool isValidJson2(Json::Value const& jv)
    {
        ...
        if (jv.isObject())
        {
            if (jv.isMember(jss::jsonrpc) && jv[jss::jsonrpc] == "2.0" &&
                jv.isMember(jss::SupplyFinanceChainrpc) && jv[jss::SupplyFinanceChainrpc] == "2.0" &&
    ...
    Json::Value parseJson2(Json::Value const& jvParams)
    {
        ...
        if (valid_parse && isValidJson2(jv))
        {
            if (jv.isObject())
            {
                ...
                jv1[jss::SupplyFinanceChainrpc] = jv[jss::SupplyFinanceChainrpc];
            ...
            for (Json::UInt j = 0; j < jv.size(); ++j)
            {
                ...
                jv1[j][jss::SupplyFinanceChainrpc] = jv[j][jss::SupplyFinanceChainrpc];
        ...
        if (jv.isMember(jss::SupplyFinanceChainrpc))
            jv_error[jss::SupplyFinanceChainrpc] = jv[jss::SupplyFinanceChainrpc];
    ...
    Json::Value parseSupplyFinanceChainPathFind (Json::Value const& jvParams)
    {
        Json::Reader    reader;
        Json::Value     jvRequest{Json::objectValue};
...
public:
    ...
    //      {   "profile",              &RPCParser::parseProfile,               1,  9   },
            ...
            {   "SupplyFinanceChain_path_find",     &RPCParser::parseSupplyFinanceChainPathFind,        1,  2   },
...
cmdLineToJSONRPC (std::vector<std::string> const& args, beast::Journal j)
{
    ...
    if (paramsObj.isMember(jss::SupplyFinanceChainrpc))
        jv[jss::SupplyFinanceChainrpc] = paramsObj[jss::SupplyFinanceChainrpc];
