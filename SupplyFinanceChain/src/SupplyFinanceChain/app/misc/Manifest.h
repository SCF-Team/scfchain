...
#ifndef SUPPLYFINANCECHAIN_APP_MISC_MANIFEST_H_INCLUDED
#define SUPPLYFINANCECHAIN_APP_MISC_MANIFEST_H_INCLUDED
...
/*
    ...
    Suppose the secret keys installed on a SupplyFinanceChain validator are compromised.  Not
    only do you have to generate and install new key pairs on each validator,
    EVERY SupplyFinanceChaind needs to have its config updated with the new public keys, and
    ...
    received from SupplyFinanceChaind peers.
    ...
    signed by the master key.  When a SupplyFinanceChaind peer receives the new manifest,
