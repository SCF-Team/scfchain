    ...
    TxQ::Setup const& setup)
{
    ...
    // happen.
    return mulDiv(tx[sfFee].sfc().drops(),
        baseRefLevel,
...
/*
    ...
    0. Is `featureFeeEscalation` enabled?
        ...
        No: Fallback to `SupplyFinanceChain::apply`. Stop.
        ...
        ApplyFlags flags, beast::Journal j)
{
    ...
    struct MultiTxn
    {
        ...
        SFCAmount fee = beast::zero;
        SFCAmount potentialSpend = beast::zero;
        ...
        if (sle)
        {
            ...
            if (multiTxn)
            {
                /* Check if the total fees in flight are greater
                    ...
                    Assume: Minimum account reserve is 20 SFC.
                    Example 1: If I have 1,000,000 SFC, I can queue
                        a transaction with a 1,000,000 SFC fee. In
                        the meantime, some other transaction may
                        lower my balance (eg. taking an offer). When
                        the transaction executes, I will either
                        spend the 1,000,000 SFC, or the transaction
                        will get stuck in the queue with a
                        `terINSUF_FEE_B`.
                    Example 2: If I have 1,000,000 SFC, and I queue
                        10 transactions with 0.1 SFC fee, I have 1 SFC
                        in flight. I can now queue another tx with a
                        999,999 SFC fee. When the first 10 execute,
                        ...
                        999,999 SFC, or get stuck in the queue.
                    Example 3: If I have 1,000,000 SFC, and I queue
                        7 transactions with 3 SFC fee, I have 21 SFC
                ...
                */
                auto const balance = (*sle)[sfBalance].sfc();
    ...
    */
    ...
    if (!multiTxn && feeLevelPaid >= requiredFeeLevel)
    {
        ...
        JLOG(j_.trace()) << "Applying transaction " <<
            transactionID <<
            " to open ledger.";
        SupplyFinanceChain::TER txnResult;
        bool didApply;
