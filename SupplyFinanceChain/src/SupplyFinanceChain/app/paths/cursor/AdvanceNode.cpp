...
TER PathCursor::advanceNode (STAmount const& amount, bool reverse, bool callerHasLiquidity) const
{
    ...
    PathCursor withMultiQuality {SupplyFinanceChainCalc_, pathState_, multi, j_, nodeIndex_};
        ...
        else
        {
            ...
            else
            {
                            ...
                            view().parentCloseTime().time_since_epoch().count()))
                {
                    ...
                    SupplyFinanceChainCalc_.permanentlyUnfundedOffers_.insert(
                        node().offerIndex_);
                    continue;
                }
                ...
                if (node().saTakerPays <= beast::zero || node().saTakerGets <= beast::zero)
                {
                    ...
                    else if (SupplyFinanceChainCalc_.permanentlyUnfundedOffers_.find (index)
                             != SupplyFinanceChainCalc_.permanentlyUnfundedOffers_.end ())
                    {
                ...
                auto itPast = SupplyFinanceChainCalc_.mumSource_.find (accountIssue);
                bool bFoundPast = (itPast != SupplyFinanceChainCalc_.mumSource_.end ());
                ...
                if (node().saOfferFunds <= beast::zero)
                {
                    ...
                    if (bReverse && !bFoundReverse && !bFoundPast)
                    {
                        ...
                        // still do deletions.
                        // Mark offer for always deletion.
                        SupplyFinanceChainCalc_.permanentlyUnfundedOffers_.insert (node().offerIndex_);
                    }
                    else
                    {
