...
#include <SupplyFinanceChain/app/paths/cursor/SupplyFinanceChainLiquidity.h>
...
// - If next node is an offer and output is non-SFC then we are the issuer and
//   do not need to push funds.
// - If next node is an offer and output is SFC then we need to deliver funds to
//   limbo.
TER PathCursor::forwardLiquidityForAccount () const
{
    TER resultCode   = tesSUCCESS;
    auto const lastNodeIndex       = pathState_.nodes().size () - 1;
    auto viewJ = SupplyFinanceChainCalc_.logs_.journal ("View");
    ...
    // SupplyFinanceChain through account.
    ...
    if (previousNode().isAccount() && nextNode().isAccount())
    {
        ...
        if (!nodeIndex_)
        {
            ...
            // For the first node, calculate amount to SupplyFinanceChain based on what is
            // available.
        ...
        else if (nodeIndex_ == lastNodeIndex)
        {
            ...
            {
                // Actually receive.
                resultCode = SupplyFinanceChainCredit(view(),
                    previousAccountID,
                    node().account_,
        ...
        else
        {
                ...
                // Previous wants to redeem.
            {
                ...
                SupplyFinanceChainLiquidity (
                    SupplyFinanceChainCalc_,
                    parityRate,
                    qualityOut,
    ...
    else if (previousNode().isAccount() && !nextNode().isAccount())
    {
        ...
        if (nodeIndex_)
        {
            // Non-SFC, current node is the issuer.
            JLOG (j_.trace())
                << "forwardLiquidityForAccount: account --> "
                << "ACCOUNT --> offer";
        ...
        else
        {
            ...
            if (pathState_.inReq() >= beast::zero)
            {
                ...
                // Limit SFC by available. No limit for non-SFC as issuer.
                if (isSFC (node().issue_))
                    node().saFwdDeliver = std::min (
                        node().saFwdDeliver,
                        accountHolds(view(),
                            node().account_,
                            sfcCurrency(),
                            sfcAccount(),
                            fhIGNORE_FREEZE, viewJ)); // SFC can't be frozen
            ...
            else if (!isSFC (node().issue_))
            {
                // Non-SFC, current node is the issuer.
                // We could be delivering to multiple accounts, so we don't know
                // which SupplyFinanceChain balance will be adjusted.  Assume just issuing.
            ...
            else
            {
                ...
                // Deliver SFC to limbo.
                resultCode = accountSend(view(),
                    node().account_, sfcAccount(), node().saFwdDeliver, viewJ);
            }
        }
    }
    else if (!previousNode().isAccount() && nextNode().isAccount())
