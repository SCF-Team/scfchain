...
void SupplyFinanceChainLiquidity (
    SupplyFinanceChainCalc& SupplyFinanceChainCalc,
...
{
    JLOG (SupplyFinanceChainCalc.j_.trace())
    ...
    if (qualityIn >= qualityOut)
    {
        ...
        JLOG (SupplyFinanceChainCalc.j_.trace()) << "SupplyFinanceChainLiquidity: No fees";
    ...
    else
    {
        ...
        JLOG (SupplyFinanceChainCalc.j_.trace()) << "SupplyFinanceChainLiquidity: Fee";
        ...
        if (!uRateMax || uRate <= uRateMax)
        {
            ...
            if (bPrvUnlimited || saCurIn <= saPrv)
            {
                ...
                saPrvAct += saCurIn;
                JLOG (SupplyFinanceChainCalc.j_.trace())
                    << "SupplyFinanceChainLiquidity:3c:"
                    << " saCurReq=" << saCurReq
                    << " saPrvAct=" << saPrvAct;
            }
            else
            {
                ...
                JLOG (SupplyFinanceChainCalc.j_.trace())
                    << "SupplyFinanceChainLiquidity:4: saCurReq=" << saCurReq;
...
quality_in (
    ...
    Currency const& currency)
{
    return SupplyFinanceChainQuality (view, uToAccountID, uFromAccountID, currency,
