...
#include <SupplyFinanceChain/app/paths/cursor/EffectiveRate.h>
    ...
    bool callerHasLiquidity) const
{
    ...
    while (resultCode == tesSUCCESS && saInAct + saInFees < saInReq)
    {
        ...
        else if (resultCode == tesSUCCESS)
        {
            ...
            if (!isSFC(nextNode().account_))
            {
                // ? --> OFFER --> account
                // Input fees: vary based upon the consumed offer's owner.
                // Output fees: none as SFC or the destination account is the
                // issuer.
                ...
                // Output: Debit offer owner, send SFC or non-XPR to next
                // account.
            ...
            else
            {
                ...
                // fees).
                auto const& id = isSFC(node().issue_) ?
                        sfcAccount() : node().issue_.account;
            ...
            // is redundant.
            if (isSFC (previousNode().issue_.currency)
                || uInAccountID != node().offerOwnerAccount_)
            {
                auto id = !isSFC(previousNode().issue_.currency) ?
                        uInAccountID : sfcAccount();
                resultCode = accountSend(view(),
                    id,
                    node().offerOwnerAccount_,
                    saInPassAct,
