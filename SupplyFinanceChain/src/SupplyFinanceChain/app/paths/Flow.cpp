...
#include <SupplyFinanceChain/app/paths/impl/StrandFlow.h>
...
auto finishFlow (PaymentSandbox& sb,
    ...
    FlowResult&& f)
{
    path::SupplyFinanceChainCalc::Output result;
    if (f.ter == tesSUCCESS)
        f.sandbox->apply (sb);
    else
    ...
    path::detail::FlowDebugInfo* flowDebugInfo)
{
    Issue const srcIssue = [&] {
        ...
        if (!isSFC (deliver.issue ().currency))
    ...
    const bool srcIsSFC = isSFC (srcIssue.currency);
    const bool dstIsSFC = isSFC (dstIssue.currency);

    auto const asDeliver = toAmountSpec (deliver);

    // The src account may send either sfc or iou. The dst account may receive
    // either sfc or iou. Since SFC and IOU amounts are represented by different
    // types, use templates to tell `flow` about the amount types.
    if (srcIsSFC && dstIsSFC)
    {
        return finishFlow (sb, srcIssue, dstIssue,
            flow<SFCAmount, SFCAmount> (
                sb, strands, asDeliver.sfc, partialPayment, offerCrossing,
    ...
    {
        return finishFlow (sb, srcIssue, dstIssue,
            flow<IOUAmount, SFCAmount> (
                sb, strands, asDeliver.sfc, partialPayment, offerCrossing,
                limitQuality, sendMax, j, flowDebugInfo));
    }

    assert (!srcIsSFC && !dstIsSFC);
    return finishFlow (sb, srcIssue, dstIssue,
