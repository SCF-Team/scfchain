...
#include <SupplyFinanceChain/app/paths/Pathfinder.h>
...
The pathfinding request is identified by category, SFC to SFC, SFC to
non-SFC, non-SFC to SFC, same currency non-SFC to non-SFC, cross-currency
non-SFC to non-SFC.  For each category, there is a table of paths that the
...
Pathfinder::Pathfinder (
    ...
    :   mSrcAccount (uSrcAccount),
        ...
        mEffectiveDst (isSFC(saDstAmount.getIssuer ()) ?
        ...
        mSrcAmount (srcAmount.value_or(STAmount({uSrcCurrency,
            uSrcIssuer.value_or(isSFC(uSrcCurrency) ?
                sfcAccount() : uSrcAccount)}, 1u, 0, true))),
...
{
    assert (! uSrcIssuer || isSFC(uSrcCurrency) == isSFC(uSrcIssuer.get()));
}
...
bool Pathfinder::findPaths (int searchLevel)
{
    ...
    auto currencyIsSFC = isSFC (mSrcCurrency);

    bool useIssuerAccount
            = mSrcIssuer && !currencyIsSFC && !isSFC (*mSrcIssuer);
    auto& account = useIssuerAccount ? *mSrcIssuer : mSrcAccount;
    auto issuer = currencyIsSFC ? AccountID() : account;
    ...
    bool bSrcSfc = isSFC (mSrcCurrency);
    bool bDstSfc = isSFC (mDstAmount.getCurrency());
    ...
    if (! mLedger->exists (keylet::account (mDstAccount)))
    {
        ...
        {
            JLOG (j_.debug())
                    << "New account not being funded in SFC ";
            return false;
        }
    ...
    {
        // SFC -> SFC
        JLOG (j_.debug()) << "SFC to SFC payment";
        paymentType = pt_SFC_to_SFC;
    }
    else if (bSrcSfc)
    {
        // SFC -> non-SFC
        JLOG (j_.debug()) << "SFC to non-SFC payment";
        paymentType = pt_SFC_to_nonSFC;
    }
    else if (bDstSfc)
    {
        // non-SFC -> SFC
        JLOG (j_.debug()) << "non-SFC to SFC payment";
        paymentType = pt_nonSFC_to_SFC;
    }
    else if (mSrcCurrency == mDstAmount.getCurrency ())
    {
        // non-SFC -> non-SFC - Same currency
        JLOG (j_.debug()) << "non-SFC to non-SFC - same currency";
        paymentType = pt_nonSFC_to_same;
    }
    else
    {
        // non-SFC to non-SFC - Different currency
        JLOG (j_.debug()) << "non-SFC to non-SFC - cross currency";
        paymentType = pt_nonSFC_to_nonSFC;
    }
    ...
    uint64_t& qualityOut) const    // OUT: The returned initial quality
{
    ...
    path::SupplyFinanceChainCalc::Input rcInput;
    ...
    try
    {
        ...
        auto rc = path::SupplyFinanceChainCalc::SupplyFinanceChainCalculate (
            sandbox,
            mSrcAmount,
            minDstAmount,
            mDstAccount,
        ...
        if (! convert_all_)
        {
            ...
            rcInput.partialPaymentAllowed = true;
            rc = path::SupplyFinanceChainCalc::SupplyFinanceChainCalculate (
                sandbox,
                mSrcAmount,
...
static bool isDefaultPath (STPath const& path)
{
    ...
    // https://forum.SupplyFinanceChain.com/viewtopic.php?f=2&t=8206&start=10#p57713
    //
    // JoelKatz writes:
    ...
    AccountID const& srcIssuer)
{
    ...
    const bool issuerIsSender = isSFC (mSrcCurrency) || (srcIssuer == mSrcAccount);
    ...
    AccountID const& dstAccount)
{
    ...
    if (!bFrozen)
    {
        ...
        for (auto const& item : mRLCache->getSupplyFinanceChainLines (account))
        {
            SupplyFinanceChainState* rspEntry = (SupplyFinanceChainState*) item.get ();
            ...
            else if (rspEntry->getNoSupplyFinanceChainPeer ())
            {
...
STPathSet& Pathfinder::addPathsForType (PathType const& pathType)
{
    ...
    case nt_SFC_BOOK:
        addLinks (parentPaths, pathsOut, afADD_BOOKS | afOB_SFC);
        break;
...
bool Pathfinder::isNoSupplyFinanceChain (
    AccountID const& fromAccount,
    AccountID const& toAccount,
    Currency const& currency)
{
    auto sleSupplyFinanceChain = mLedger->read(keylet::line(
        toAccount, fromAccount, currency));

    auto const flag ((toAccount > fromAccount)
                     ? lsfHighNoSupplyFinanceChain : lsfLowNoSupplyFinanceChain);

    return sleSupplyFinanceChain && (sleSupplyFinanceChain->getFieldU32 (sfFlags) & flag);
}

// Does this path end on an account-to-account link whose last account has
// set "no SupplyFinanceChain" on the link?
bool Pathfinder::isNoSupplyFinanceChainOut (STPath const& currentPath)
{
    ...
    // no SupplyFinanceChain on the output. A path with no SupplyFinanceChain on its output can't be
    // followed by a link with no SupplyFinanceChain on its input.
    auto const& fromAccount = (currentPath.size () == 1)
        ? mSrcAccount
        : (currentPath.end () - 2)->getAccountID ();
    auto const& toAccount = endElement.getAccountID ();
    return isNoSupplyFinanceChain (fromAccount, toAccount, endElement.getCurrency ());
}
    ...
    int addFlags)
{
    ...
    bool const bOnSFC = uEndCurrency.isZero ();
    ...
    if (addFlags & afADD_ACCOUNTS)
    {
        ...
        if (bOnSFC)
        {
            ...
            { // non-default path to SFC destination
                JLOG (j_.trace())
        ...
        else
        {
            ...
            if (sleEnd)
            {
                ...
                bool const bIsNoSupplyFinanceChainOut (
                    isNoSupplyFinanceChainOut (currentPath));
                bool const bDestOnly (
                    addFlags & afAC_LAST);

                auto& SupplyFinanceChainLines (mRLCache->getSupplyFinanceChainLines (uEndAccount));

                AccountCandidates candidates;
                candidates.reserve (SupplyFinanceChainLines.size ());

                for (auto const& item : SupplyFinanceChainLines)
                {
                    auto* rs = dynamic_cast<SupplyFinanceChainState const *> (item.get ());
                    if (!rs)
                    {
                        JLOG (j_.error())
                                << "Couldn't decipher SupplyFinanceChainState";
                        continue;
                    }
                        ...
                        !currentPath.hasSeen (acct, uEndCurrency, acct))
                    {
                        ...
                        else if (bIsNoSupplyFinanceChainOut && rs->getNoSupplyFinanceChain ())
                        {
    ...
    {
        // add order books
        if (addFlags & afOB_SFC)
        {
            // to SFC only
            if (!bOnSFC && app_.getOrderBookDB ().isBookToSFC (
            ...
            for (auto const& book : books)
            {
                     ...
                     (book->getCurrencyOut () == mDstAmount.getCurrency ())))
                {
                    ...
                    { // to SFC
                        ...
                        {
                            // destination is SFC, add account and path is
                            // complete
                            JLOG (j_.trace())
                        ...
                        book->getIssuerOut ()))
                    {
                            ...
                            (newPath[newPath.size() - 2].isOffer ()))
                        {
                            ...
                            newPath[newPath.size() - 1] = STPathElement (
                                ...
                                sfcAccount(), book->getCurrencyOut(),
                                book->getIssuerOut());
                        }
                        else
                        {
...
Pathfinder::PathType makePath (char const *string)
{
    ...
    while (true)
    {
        switch (*string++)
        {
            ...
            case 'x': // sfc book
                ret.push_back (Pathfinder::nt_SFC_BOOK);
                break;
...
void Pathfinder::initPathTable ()
{
    ...
    fillPaths(
        pt_nonSFC_to_SFC, {
            {1, "sxd"},       // gateway buys SFC
            {2, "saxd"},      // source -> gateway -> book(SFC) -> dest
            {6, "saaxd"},
            {7, "sbxd"},
            {8, "sabxd"},
            {9, "sabaxd"}
    ...
    // non-SFC to non-SFC (same currency)
    fillPaths(
        pt_nonSFC_to_same,  {
            ...
            {6, "sxfad"},
            {6, "safad"},
            {6, "saxfd"},   // source -> gateway -> book to SFC -> book ->
                            // destination
            {6, "saxfad"},
