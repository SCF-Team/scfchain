...
#include <SupplyFinanceChain/app/paths/impl/FlatSets.h>
#include <SupplyFinanceChain/app/paths/impl/Steps.h>
#include <SupplyFinanceChain/app/paths/Credit.h>
#include <SupplyFinanceChain/app/paths/NodeDirectory.h>
#include <SupplyFinanceChain/app/tx/impl/OfferStream.h>
...
public:
        ...
        bool prevStepRedeems) const
    {
        ...
        auto rate = [&](AccountID const& id) {
            if (isSFC(id) || id == this->strandDst_)
                return parityRate;
    ...
    Callback& callback) const
{
    ...
    while (offers.step ())
    {
        ...
        // An account can always own SFC or their own IOUs.
        if (flowCross &&
            (!isSFC (offer.issueIn().currency)) &&
...
template <class TIn, class TOut, class TDerived>
static
bool equalHelper (Step const& step, SupplyFinanceChain::Book const& book)
{
...
bool bookStepEqual (Step const& step, SupplyFinanceChain::Book const& book)
{
    bool const inSFC = isSFC (book.in.currency);
    bool const outSFC = isSFC (book.out.currency);
    if (inSFC && outSFC)
        return equalHelper<SFCAmount, SFCAmount,
            BookPaymentStep<SFCAmount, SFCAmount>> (step, book);
    if (inSFC && !outSFC)
        return equalHelper<SFCAmount, IOUAmount,
            BookPaymentStep<SFCAmount, IOUAmount>> (step, book);
    if (!inSFC && outSFC)
        return equalHelper<IOUAmount, SFCAmount,
            BookPaymentStep<IOUAmount, SFCAmount>> (step, book);
    if (!inSFC && !outSFC)
...
make_BookStepIX (
    ...
    Issue const& in)
{
    return make_BookStepHelper<IOUAmount, SFCAmount> (ctx, in, sfcIssue());
}
...
make_BookStepXI (
    ...
    Issue const& out)
{
    return make_BookStepHelper<SFCAmount, IOUAmount> (ctx, sfcIssue(), out);
}
