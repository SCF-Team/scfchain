...
#ifndef SUPPLYFINANCECHAIN_PATH_IMPL_FLOWDEBUGINFO_H_INCLUDED
#define SUPPLYFINANCECHAIN_PATH_IMPL_FLOWDEBUGINFO_H_INCLUDED
    ...
    to_string (bool writePassInfo) const
    {
        ...
        if (writePassInfo)
        {
            ...
            auto writeIouAmtList = [&write_list](
                std::vector<EitherAmount> const& amts, char delim=';') {
                auto get_val = [](EitherAmount const& a) -> std::string {
                    return SupplyFinanceChain::to_string (a.iou);
                };
                write_list (amts, get_val, delim);
            };
            ...
            auto writeNestedSfcAmtList = [&ostr, &writeSfcAmtList](
                std::vector<std::vector<EitherAmount>> const& amts) {
                ostr << '[';
                if (!amts.empty ())
                {
                    writeSfcAmtList(amts[0], '|');
                    for (size_t i = 1, e = amts.size (); i < e; ++i)
                    {
                        ostr << ';';
                        writeSfcAmtList(amts[i], '|');
                    }
                }
                ostr << ']';
            };
            ...
            if (passInfo.nativeIn)
                writeSfcAmtList (passInfo.in);
            else
                writeIouAmtList (passInfo.in);
            ostr << ", out_pass: ";
            if (passInfo.nativeOut)
                writeSfcAmtList (passInfo.out);
            else
                ...
                !passInfo.liquiditySrcIn.back ().empty ())
            {
                ostr << ", l_src_in: ";
                if (passInfo.nativeIn)
                    writeNestedSfcAmtList (passInfo.liquiditySrcIn);
                else
                    writeNestedIouAmtList (passInfo.liquiditySrcIn);
                ostr << ", l_src_out: ";
                if (passInfo.nativeOut)
                    writeNestedSfcAmtList (passInfo.liquiditySrcOut);
                else
...
{
    return {sb.balanceChanges (rv), sb.sfcDestroyed ()};
}
...
balanceDiffsToString (boost::optional<BalanceDiffs> const& bd)
{
    ...
    auto const& sfcDestroyed = bd->second;
    std::ostringstream ostr;
    ostr << ", sfcDestroyed: " << to_string (sfcDestroyed);
    ostr << ", balanceDiffs: ";
