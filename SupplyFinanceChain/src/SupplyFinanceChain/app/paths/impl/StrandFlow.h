...
#ifndef SUPPLYFINANCECHAIN_APP_PATHS_IMPL_STRANDFLOW_H_INCLUDED
#define SUPPLYFINANCECHAIN_APP_PATHS_IMPL_STRANDFLOW_H_INCLUDED
...
/**
   ...
   @param strand Steps of Accounts to SupplyFinanceChain through and offer books to use
    ...
    beast::Journal j)
{
    ...
    if (isDirectSfcToSfc<TInAmt, TOutAmt> (strand))
    {
        // The current implementation returns NO_LINE for SFC->SFC transfers.
        // Keep this behavior
            ...
            for (auto i = s; i--;)
            {
                ...
                else if (!strand[i]->equalOut (r.second, stepOut))
                {
                    ...
                    if (strand[i]->isZero(r.second))
                    {
                        ...
                        // I.e. 10^-80 IOU into an IOU -> SFC offer.
...
/**
   ...
   @param baseView Trust lines and balances
   @param strands Each strand contains the steps of accounts to SupplyFinanceChain through
