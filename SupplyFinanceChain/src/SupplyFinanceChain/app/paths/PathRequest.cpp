...
#include <SupplyFinanceChain/app/paths/PathRequest.h>
...
bool PathRequest::isValid (std::shared_ptr<SupplyFinanceChainLineCache> const& crCache)
{
    ...
    if (! sleDest)
    {
        ...
        {
            // Only SFC can be send to a non-existent account.
    ...
    {
        bool const disallowSFC (
            sleDest->getFlags() & lsfDisallowSFC);

        auto usDestCurrID = accountDestCurrencies (
                *raDstAccount, crCache, ! disallowSFC);
...
PathRequest::getPathFinder(std::shared_ptr<SupplyFinanceChainLineCache> const& cache,
...
bool
PathRequest::findPaths (std::shared_ptr<SupplyFinanceChainLineCache> const& cache,
    int const level, Json::Value& jvArray)
{
        ...
        for (auto const& c : currencies)
        {
            if (! sameAccount || c != saDstAmount.getCurrency())
            {
                ...
                sourceCurrencies.insert(
                    {c, c.isZero() ? sfcAccount() : *raSrcAccount});
            }
        }
    }
    ...
    for (auto const& issue : sourceCurrencies)
    {
        ...
        auto& sourceAccount = ! isSFC(issue.account)
            ? issue.account
        ...
        JLOG(m_journal.debug()) << iIdentifier
            << " Paths found, calling SupplyFinanceChainCalc";
        ...
        if (rc.result () == tesSUCCESS)
        {
            ...
            if (hasCompletion ())
            {
                // Old SupplyFinanceChain_path_find API requires this
        ...
        {
            JLOG(m_journal.debug()) << iIdentifier << " SupplyFinanceChainCalc returns "
                << transHuman(rc.result());
        }
    }
...
Json::Value PathRequest::doUpdate(
    std::shared_ptr<SupplyFinanceChainLineCache> const& cache, bool fast)
{
    using namespace std::chrono;
    JLOG(m_journal.debug()) << iIdentifier
    ...
    if (hasCompletion ())
    {
        // Old SupplyFinanceChain_path_find API gives destination_currencies
