...
class SupplyFinanceChainCalc; // for logging
...
void PathState::reset(STAmount const& in, STAmount const& out)
{
    ...
    {
        JLOG (j_.warn())
            << "SupplyFinanceChainCalc: ALREADY DONE:"
            << " saOutAct=" << outAct()
            << " saOutReq=" << outReq();
    }
...
// - A SupplyFinanceChain nodes output issuer must be the node's account or the next node's
//   account.
// - Offers can only go directly to another offer if the currency and issuer are
//   an exact match.
// - Real issuers must be specified for non-SFC.
    ...
    AccountID const& issuer)    // --> Delivering this issuer.
{
    ...
    if (nodes_.back ().issue_.currency != currency)
    {
        ...
        // book.  sfcAccount() does double duty as signaling "this is an order
        // book".

        // Corresponds to "Implies an offer directory" in the diagram, currently
        // at http://goo.gl/Uj3HAB.

        auto type = isSFC(currency) ? STPathElement::typeCurrency
            : STPathElement::typeCurrency | STPathElement::typeIssuer;

        // The offer's output is what is now wanted.
        // sfcAccount() is a placeholder for offers.
        resultCode = pushNode (type, sfcAccount(), currency, issuer);
    }


    // For SupplyFinanceChain, non-SFC, ensure the issuer is on at least one side of the
    // transaction.
    ...
    {
        // Need to SupplyFinanceChain through issuer's account.
    ...
    AccountID const& issuer)     // If not specified, default to previous.
{
    ...
    // true, iff node is a SupplyFinanceChain account. false, iff node is an offer node.
    ...
    else if (hasIssuer && isSFC (node.issue_))
    {
        JLOG (j_.debug()) << "pushNode: issuer specified for SFC.";
    ...
    else if (hasAccount)
    {
        ...
        node.issue_.account = hasIssuer ? issuer :
                (isSFC (node.issue_) ? sfcAccount() : account);
        ...
        else
        {
            ...
            resultCode = pushImpliedNodes (
                ...
                isSFC(node.issue_.currency) ? sfcAccount() : account);
        ...
        if (resultCode == tesSUCCESS && !nodes_.empty ())
        {
            ...
            {
                auto sleSupplyFinanceChainState = view().peek(
                    keylet::line(backNode.account_, node.account_, backNode.issue_.currency));

                // A "SupplyFinanceChainState" means a balance betweeen two accounts for a
                // specific currency.
                ...
                else
                {
                    ...
                    if (!sleBck)
                    {
                        ...
                        resultCode   = terNO_ACCOUNT;
                             ...
                             !(sleSupplyFinanceChainState->getFieldU32 (sfFlags) &
                                  (bHigh ? lsfHighAuth : lsfLowAuth)) &&
                             sleSupplyFinanceChainState->getFieldAmount(sfBalance) == beast::zero)
                    {
                        JLOG (j_.warn())
                                << "pushNode: delay: can't receive IOUs from "
                                << "issuer without auth.";
    ...
    else
    {
        ...
        else if (isSFC (node.issue_.currency))
            node.issue_.account = sfcAccount();
        else if (isSFC (backNode.issue_.account))
        ...
        else if (backNode.issue_ == node.issue_)
        {
            ...
            resultCode   = pushImpliedNodes (
                sfcAccount(), // Rippling, but offers don't have an account.
    ...
    AccountID const& uSenderID)
{
    ...
    AccountID const& uSenderIssuerID
        = isSFC(uMaxCurrencyID) ? sfcAccount() : uSenderID;
    // Sender is always issuer for non-SFC.
    ...
    // SFC with issuer is malformed.
    if ((isSFC (uMaxCurrencyID) && !isSFC (uMaxIssuerID))
        || (isSFC (currencyOutID) && !isSFC (issuerOutID)))
    {
        JLOG (j_.debug())
            << "expandPath> issuer with SFC";
        terStatus   = temBAD_PATH;
    }

    // Push sending node.
    // For non-SFC, issuer is always sending account.
    // - Trying to expand, not-compact.
    // - Every issuer will be traversed through.
    if (terStatus == tesSUCCESS)
    {
        terStatus   = pushNode (
            !isSFC(uMaxCurrencyID)
    ...
    if (tesSUCCESS == terStatus && uMaxIssuerID != uSenderIssuerID)
    {
        ...
        // - If it was SFC, then issuers would have matched.
        ...
        // account.
        if (!uNxtCurrencyID
            // Next is SFC, offer next. Must go through issuer.
    ...
    if (terStatus == tesSUCCESS
        && !isSFC(currencyOutID)               // Next is not SFC
...
/** Check if a sequence of three accounts violates the no SupplyFinanceChain constrains
    [first] -> [second] -> [third]
    Disallowed if 'second' set no SupplyFinanceChain on [first]->[second] and
    [second]->[third]
*/
TER PathState::checkNoSupplyFinanceChain (
    ...
    Currency const& currency)
{
    ...
    if (!sleIn || !sleOut)
    {
        ...
        sleIn->getFieldU32 (sfFlags) &
            ((secondAccount > firstAccount) ? lsfHighNoSupplyFinanceChain : lsfLowNoSupplyFinanceChain) &&
        sleOut->getFieldU32 (sfFlags) &
            ((secondAccount > thirdAccount) ? lsfHighNoSupplyFinanceChain : lsfLowNoSupplyFinanceChain))
    {
        JLOG (j_.info())
            << "Path violates noSupplyFinanceChain constraint between "
            << firstAccount << ", "
            << secondAccount << " and "
            << thirdAccount;

        terStatus = terNO_SUPPLYFINANCECHAIN;
    }
    return terStatus;
}

// Check a fully-expanded path to make sure it doesn't violate no-SupplyFinanceChain
// settings.
TER PathState::checkNoSupplyFinanceChain (
    AccountID const& uDstAccountID,
    AccountID const& uSrcAccountID)
{
    // There must be at least one node for there to be two consecutive SupplyFinanceChain
    // lines.
    if (nodes_.size() == 0)
       return terStatus;
    ...
    if (nodes_.size() == 1)
    {
            ...
            (nodes_[0].account_ != uDstAccountID))
        {
            ...
            {
                terStatus = checkNoSupplyFinanceChain (
    ...
    // These are the nodes whose no SupplyFinanceChain constraints could be violated
