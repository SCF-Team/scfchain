...
#include <SupplyFinanceChain/beast/clock/basic_seconds_clock.h>
...
void printHelp (const po::options_description& desc)
{
    std::cerr
        ...
        << "Commands: \n"
           ...
           "     random\n"
           "     SupplyFinanceChain ...\n"
           "     SupplyFinanceChain_path_find <json> [<ledger>]\n"
    ...
    char** argv)
{
    ...
    using namespace SupplyFinanceChain::test;

    SupplyFinanceChain::test::envUseIPv4 = (! ipv6);
...
int run (int argc, char** argv)
{
    ...
    beast::setCurrentThreadName ("SupplyFinanceChaind: main");
    ...
    {
        std::cerr << "SupplyFinanceChaind: Incorrect command line syntax." << std::endl;
    ...
    if (vm.count ("version"))
    {
        std::cout << "SupplyFinanceChaind version " <<
    ...
    else
    {
        if (vm.count("unittest-jobs"))
        {
            ...
            std::cerr << "SupplyFinanceChaind: '--unittest-jobs' specified without '--unittest'.\n";
    ...
    beast::setCurrentThreadName ("SupplyFinanceChaind: rpc");
...
int main (int argc, char** argv)
{
    ...
    SupplyFinanceChain::sha512_deprecatedMSVCWorkaround();
#endif
    ...
    static_assert (gccver >= 50100,
        "GCC version 5.1.0 or later is required to compile SupplyFinanceChaind.");
#endif

    atexit(&google::protobuf::ShutdownProtobufLibrary);

    auto const result (SupplyFinanceChain::run (argc, argv));
