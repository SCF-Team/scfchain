...
#ifndef SUPPLYFINANCECHAIN_TX_APPLYSTEPS_H_INCLUDED
#define SUPPLYFINANCECHAIN_TX_APPLYSTEPS_H_INCLUDED
...
struct TxConsequences
{
    ...
    SFCAmount const potentialSpend;

    /// Constructor
    TxConsequences(ConsequenceCategory const category_,
        SFCAmount const fee_, SFCAmount const spend_)
        : category(category_)
        , fee(fee_)
...
/** Determine the SFC balance consequences if a transaction
    consumes the maximum SFC allowed.
