...
SetAccount::preflight (PreflightContext const& ctx)
{
    ...
    // DisallowSFC
    //
    bool bSetDisallowSFC   = (uTxFlags & tfDisallowSFC) || (uSetFlag == asfDisallowSFC);
    bool bClearDisallowSFC = (uTxFlags & tfAllowSFC) || (uClearFlag == asfDisallowSFC);

    if (bSetDisallowSFC && bClearDisallowSFC)
    {
        JLOG(j.trace()) << "Malformed transaction: Contradictory flags set.";
        return temINVALID_FLAG;
    }
...
SetAccount::doApply ()
{
    ...
    bool const bSetDisallowSFC   {(uTxFlags & tfDisallowSFC) || (uSetFlag == asfDisallowSFC)};
    bool const bClearDisallowSFC {(uTxFlags & tfAllowSFC) || (uClearFlag == asfDisallowSFC)};
    ...
    // DisallowSFC
    //
    if (bSetDisallowSFC && !(uFlagsIn & lsfDisallowSFC))
    {
        JLOG(j_.trace()) << "Set lsfDisallowSFC.";
        uFlagsOut |= lsfDisallowSFC;
    }

    if (bClearDisallowSFC && (uFlagsIn & lsfDisallowSFC))
    {
        JLOG(j_.trace()) << "Clear lsfDisallowSFC.";
        uFlagsOut &= ~lsfDisallowSFC;
    }
    ...
    //
    // DefaultSupplyFinanceChain
    //
    if (uSetFlag == asfDefaultSupplyFinanceChain)
    {
        JLOG(j_.trace()) << "Set lsfDefaultSupplyFinanceChain.";
        uFlagsOut |= lsfDefaultSupplyFinanceChain;
    }
    else if (uClearFlag == asfDefaultSupplyFinanceChain)
    {
        JLOG(j_.trace()) << "Clear lsfDefaultSupplyFinanceChain.";
        uFlagsOut &= ~lsfDefaultSupplyFinanceChain;
    }
