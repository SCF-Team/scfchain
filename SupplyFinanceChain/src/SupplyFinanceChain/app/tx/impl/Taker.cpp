    ...
    , journal_ (journal)
{
    ...
    // flavor we expect:
    assert (cross_type != CrossType::SfcToIou ||
        (isSFC (issue_in ()) && !isSFC (issue_out ())));

    assert (cross_type != CrossType::IouToSfc ||
        (!isSFC (issue_in ()) && isSFC (issue_out ())));

    // And make sure we're not crossing SFC for SFC
    assert (!isSFC (issue_in ()) || !isSFC (issue_out ()));
    ...
    Rate const& rate_out)
{
    ...
    // of an autobridge.
    if (cross_type_ == CrossType::SfcToIou && (remaining_.in < f.order.in))
    {
    ...
    Rate const& rate_in)
{
    ...
    if (!sell_ && cross_type_ == CrossType::IouToSfc)
    {
...
BasicTaker::do_cross (Amounts offer, Quality quality, AccountID const& owner)
{
    ...
    if (cross_type_ == CrossType::SfcToIou)
    {
        result = flow_sfc_to_iou (offer, quality, owner_funds, taker_funds,
            out_rate (owner, account ()));
    }
    else if (cross_type_ == CrossType::IouToSfc)
    {
        result = flow_iou_to_sfc (offer, quality, owner_funds, taker_funds,
    ...
    Amounts offer2, Quality quality2, AccountID const& owner2)
{
    ...
    // The amount available to flow via SFC is the amount that the owner of the
    ...
    // of SFC that can flow between the two legs is, essentially, infinite
    // since all the owner is doing is taking out SFC of his left pocket
    // and putting it in his right pocket. In that case, we set the available
    // SFC to the largest of the two offers.
    auto sfc_funds = get_funds (owner1, offer1.out);

    if (owner1 == owner2)
    {
        JLOG(journal_.trace()) <<
            "The bridge endpoints are owned by the same account.";
        sfc_funds = std::max (offer1.out, offer2.in);
    }
    ...
    if (auto stream = journal_.debug())
    {
        ...
        stream << "      sfc: " << format_amount (sfc_funds);
    }
    ...
    auto flow1 = flow_iou_to_sfc (offer1, quality1, sfc_funds, leg1_in_funds, leg1_rate);

    if (!flow1.sanity_check ())
        Throw<std::logic_error> ("Computed flow1 fails sanity check.");

    auto flow2 = flow_sfc_to_iou (offer2, quality2, leg2_out_funds, sfc_funds, leg2_rate);
    ...
    // equalize them, so that the amount of SFC that flows out of the first leg
    // is the same as the amount of SFC that flows into the second leg. We take
...
TER Taker::transferSFC (
    AccountID const& from,
    AccountID const& to,
    STAmount const& amount)
{
    if (!isSFC (amount))
        Throw<std::logic_error> ("Using transferSFC with IOU");
    ...
    return SupplyFinanceChain::transferSFC (view_, from, to, amount, journal_);
}
...
{
    if (isSFC (amount))
        Throw<std::logic_error> ("Using redeemIOU with SFC");
    ...
    auto ret = SupplyFinanceChain::redeemIOU (view_, account, amount, issue, journal_);
...
{
    if (isSFC (amount))
        Throw<std::logic_error> ("Using issueIOU with SFC");
    ...
    return SupplyFinanceChain::issueIOU (view_, account, amount, issue, journal_);
}
...
Taker::fill (BasicTaker::Flow const& flow, Offer& offer)
{
    ...
    if (cross_type () != CrossType::SfcToIou)
    {
        assert (!isSFC (flow.order.in));
    ...
    else
    {
        ...
        if (result == tesSUCCESS)
            result = transferSFC (account (), offer.owner (), flow.order.in);
    }

    // Now send funds from the account whose offer we're taking
    if (cross_type () != CrossType::IouToSfc)
    {
        assert (!isSFC (flow.order.out));
    ...
    else
    {
        ...
        if (result == tesSUCCESS)
            result = transferSFC (offer.owner (), account (), flow.order.out);
    }
    ...
    BasicTaker::Flow const& flow2, Offer& leg2)
{
    ...
    // leg1 to leg2: bridging over SFC
    if (result == tesSUCCESS)
        result = transferSFC (leg1.owner (), leg2.owner (), flow1.order.out);
...
{
    // In direct crossings, at least one leg must not be SFC.
    if (isSFC (offer.amount ().in) && isSFC (offer.amount ().out))
        return tefINTERNAL;
...
{
    // In bridged crossings, SFC must can't be the input to the first leg
    // or the output of the second leg.
    if (isSFC (leg1.amount ().in) || isSFC (leg2.amount ().out))
        return tefINTERNAL;
...
Taker::calculateRate (
    ApplyView const& view,
        AccountID const& issuer,
            AccountID const& account)
{
    return isSFC (issuer) || (account == issuer)
        ? parityRate
