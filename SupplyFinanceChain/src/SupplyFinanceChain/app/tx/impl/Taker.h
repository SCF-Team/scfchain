...
#ifndef SUPPLYFINANCECHAIN_APP_BOOK_TAKER_H_INCLUDED
#define SUPPLYFINANCECHAIN_APP_BOOK_TAKER_H_INCLUDED

#include <SupplyFinanceChain/app/tx/impl/Offer.h>
...
protected:
    ...
    struct Flow
    {
        ...
        bool sanity_check () const
        {
            ...
            if (isSFC (order.in) && isSFC (order.out))
                return false;
...
private:
    ...
    Flow
    flow_sfc_to_iou (Amounts const& offer, Quality quality,
        STAmount const& owner_funds, STAmount const& taker_funds,
        Rate const& rate_out);

    Flow
    flow_iou_to_sfc (Amounts const& offer, Quality quality,
        STAmount const& owner_funds, STAmount const& taker_funds,
        Rate const& rate_in);
...
private:
    ...
    TER
    transferSFC (AccountID const& from, AccountID const& to, STAmount const& amount);
...
private:
    ...
    // The amount of SFC that flowed if we were autobridging
    STAmount sfc_flow_;
