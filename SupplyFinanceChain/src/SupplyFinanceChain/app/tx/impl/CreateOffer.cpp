...
#include <SupplyFinanceChain/app/tx/impl/CreateOffer.h>
...
CreateOffer::preflight (PreflightContext const& ctx)
{
    ...
    {
        JLOG(j.debug()) <<
            "Malformed offer: redundant (SFC for SFC)";
        return temBAD_OFFER;
    }
    ...
    // We don't allow a non-native currency to use the currency code SFC.
...
CreateOffer::reachedOfferCrossingLimit (Taker const& taker) const
{
    ...
    // The crossing limit is part of the SupplyFinanceChain protocol and
    ...
    NetClock::time_point const when)
{
    ...
    assert (!isSFC (takerAmount.in) && !isSFC (takerAmount.out));

    if (isSFC (takerAmount.in) || isSFC (takerAmount.out))
        Throw<std::logic_error> ("Bridging with SFC and an endpoint.");
    ...
    OfferStream offers_leg1 (view, view_cancel,
        Book (taker.issue_in (), sfcIssue ()),
        when, stepCounter_, j_);

    OfferStream offers_leg2 (view, view_cancel,
        Book (sfcIssue (), taker.issue_out ()),
        when, stepCounter_, j_);
    ...
    Amounts const& takerAmount)
{
    ...
    //
    // We check this in preclaim, but when selling SFC
    ...
    Amounts const& takerAmount)
{
    try
    {
        ...
        //
        // We check this in preclaim, but when selling SFC charged fees can
        ...
        // of the takerAmount currencies are SFC then we cross through an
        // additional path with SFC as the intermediate between two books.
...
static std::string to_string (SBoxCmp c)
{
    ...
    case SBoxCmp::sfcRound:
        return "SFC round to zero";
    case SBoxCmp::diff:
        return "different";
    }
    return {};
    ...
    beast::Journal j)
{
    ...
    if (diff.hasDiff())
    {
        ...
        // There is a special case of an offer with SFC on one side where
        // the SFC gets rounded to zero.  It mostly looks like dust-level
        // differences.  It is easier to detect if we look for it before
        // removing the dust differences.
        if (int const side = diff.sfcRoundToZero())
        {
            char const* const whichSide = side > 0 ? "; Flow" : "; Taker";
            j.stream (kWarning) << "FlowCross: " << name << " different" <<
                whichSide << " SFC rounded to zero.  tx: " <<
                ctx.tx.getTransactionID();
            return SBoxCmp::sfcRound;
        }
...
CreateOffer::preCompute()
{
    ...
    if (pays_sfc && !gets_sfc)
        cross_type_ = CrossType::IouToSfc;
    else if (gets_sfc && !pays_sfc)
        cross_type_ = CrossType::SfcToIou;
...
CreateOffer::applyGuts (Sandbox& sb, Sandbox& sbCancel)
{
    ...
    {
        SFCAmount reserve = ctx_.view().fees().accountReserve(
