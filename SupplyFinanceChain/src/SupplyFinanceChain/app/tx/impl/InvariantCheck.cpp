...
#include <SupplyFinanceChain/app/tx/impl/InvariantCheck.h>
    ...
    beast::Journal const& j)
{
    ...
    if (fee > tx.getFieldAmount(sfFee).sfc())
    {
    ...
    std::shared_ptr <SLE const> const& after)
{
     ...
     * previous SFC values and add to the total any new SFC values. The net
    ...
    if(before)
    {
        ...
        {
        case ltACCOUNT_ROOT:
            drops_ -= (*before)[sfBalance].sfc().drops();
            break;
        case ltPAYCHAN:
            drops_ -= ((*before)[sfAmount] - (*before)[sfBalance]).sfc().drops();
            break;
        case ltESCROW:
            drops_ -= (*before)[sfAmount].sfc().drops();
            break;
        default:
            break;
        }
    ...
    if(after)
    {
        ...
        {
        case ltACCOUNT_ROOT:
            drops_ += (*after)[sfBalance].sfc().drops();
            break;
        case ltPAYCHAN:
            if (! isDelete)
                drops_ += ((*after)[sfAmount] - (*after)[sfBalance]).sfc().drops();
            break;
        case ltESCROW:
            if (! isDelete)
                drops_ += (*after)[sfAmount].sfc().drops();
            break;
        default:
            break;
        }
    ...
    beast::Journal const& j)
{
    ...
    // transaction created SFC out of thin air. That's not possible.
    if (drops_ > 0)
    {
        JLOG(j.fatal()) <<
            "Invariant failed: SFC net change was positive: " << drops_;
        return false;
    }
    ...
    {
        JLOG(j.fatal()) <<
            "Invariant failed: SFC net change of " << drops_ <<
    ...
    std::shared_ptr <SLE const> const& after)
{
    auto isBad = [](STAmount const& balance)
    {
        ...
        auto const drops = balance.sfc().drops();
...
bool
SFCBalanceChecks::finalize(STTx const&, TER const, SFCAmount const, beast::Journal const& j)
{
    if (bad_)
    {
        JLOG(j.fatal()) << "Invariant failed: incorrect account SFC balance";
        return false;
    }
    ...
    std::shared_ptr <SLE const> const& after)
{
    auto isBad = [](STAmount const& pays, STAmount const& gets)
    {
        ...
        // Can't have an SFC to SFC offer:
...
bool
NoBadOffers::finalize(STTx const& tx, TER const, SFCAmount const, beast::Journal const& j)
{
    if (bad_)
    {
    ...
    std::shared_ptr <SLE const> const& after)
{
    auto isBad = [](STAmount const& amount)
    {
        ...
        if (amount.sfc().drops() <= 0)
            return true;

        if (amount.sfc().drops() >= SYSTEM_CURRENCY_START)
            return true;
...
bool
NoZeroEscrow::finalize(STTx const& tx, TER const, SFCAmount const, beast::Journal const& j)
{
    if (bad_)
    {
...
bool
AccountRootsNotDeleted::finalize(STTx const&, TER const, SFCAmount const, beast::Journal const& j)
{
    if (! accountDeleted_)
        return true;
    ...
    std::shared_ptr <SLE const> const& after)
{
    ...
    if (after)
    {
        ...
        case ltACCOUNT_ROOT:
        case ltDIR_NODE:
        case ltSUPPLYFINANCECHAIN_STATE:
        case ltTICKET:
        case ltSIGNER_LIST:
        case ltOFFER:
...
bool
LedgerEntryTypesMatch::finalize(STTx const&, TER const, SFCAmount const, beast::Journal const& j)
{
...
void
NoSFCTrustLines::visitEntry(
    uint256 const&,
    bool,
    std::shared_ptr <SLE const> const&,
    std::shared_ptr <SLE const> const& after)
{
    if (after && after->getType() == ltSUPPLYFINANCECHAIN_STATE)
    {
        // checking the issue directly here instead of
        // relying on .native() just in case native somehow
        // were systematically incorrect
        sfcTrustLine_ =
            after->getFieldAmount (sfLowLimit).issue() == sfcIssue() ||
            after->getFieldAmount (sfHighLimit).issue() == sfcIssue();
    }
}

bool
NoSFCTrustLines::finalize(STTx const&, TER const, SFCAmount const, beast::Journal const& j)
{
    if (! sfcTrustLine_)
        return true;

    JLOG(j.fatal()) << "Invariant failed: an SFC trust line was created";
    return false;
}
