...
#include <SupplyFinanceChain/app/tx/impl/Payment.h>
...
Payment::calculateMaxSpend(STTx const& tx)
{
    if (tx.isFieldPresent(sfSendMax))
    {
        ...
        return sendMax.native() ? sendMax.sfc() : beast::zero;
    }
    /* If there's no sfSendMax in SFC, and the sfAmount isn't
    in SFC, then the transaction can not send SFC. */
    auto const& saDstAmount = tx.getFieldAmount(sfAmount);
    return saDstAmount.native() ? saDstAmount.sfc() : beast::zero;
}
...
Payment::preflight (PreflightContext const& ctx)
{
    ...
    bool const defaultPathsAllowed = !(uTxFlags & tfNoSupplyFinanceChainDirect);
    ...
    // isZero() is SFC.  FIX!
    bool const bSFCDirect = uSrcCurrency.isZero () && uDstCurrency.isZero ();
    ...
    if (bSFCDirect && bMax)
    {
        ...
        JLOG(j.trace()) << "Malformed transaction: " <<
            "SendMax specified for SFC to SFC.";
        return temBAD_SEND_SFC_MAX;
    }
    if (bSFCDirect && bPaths)
    {
        // SFC is sent without paths.
        JLOG(j.trace()) << "Malformed transaction: " <<
            "Paths specified for SFC to SFC.";
        return temBAD_SEND_SFC_PATHS;
    }
    if (bSFCDirect && partialPaymentAllowed)
    {
        // Consistent but redundant transaction.
        JLOG(j.trace()) << "Malformed transaction: " <<
            "Partial payment specified for SFC to SFC.";
        return temBAD_SEND_SFC_PARTIAL;
    }
    if (bSFCDirect && limitQuality)
    {
        // Consistent but redundant transaction.
        JLOG(j.trace()) << "Malformed transaction: " <<
            "Limit quality specified for SFC to SFC.";
        return temBAD_SEND_SFC_LIMIT;
    }
    if (bSFCDirect && !defaultPathsAllowed)
    {
        // Consistent but redundant transaction.
        JLOG(j.trace()) << "Malformed transaction: " <<
            "No SupplyFinanceChain direct specified for SFC to SFC.";
        return temBAD_SEND_SFC_NO_DIRECT;
    }
...
{
    // SupplyFinanceChain if source or destination is non-native or if there are paths.
    ...
    {
        // SupplyFinanceChain payment with at least one intermediate step and uses
        // transitive balances.
...
Payment::doApply ()
{
    ...
    bool const bSupplyFinanceChain = paths || sendMax || !saDstAmount.native ();

    // If the destination has lsfDepositAuth set, then only direct SFC
    // payments (no intermediate steps) are allowed to the destination.
    if (!depositPreauth && bSupplyFinanceChain && reqDepositAuth)
        return tecNO_PERMISSION;

    if (bSupplyFinanceChain)
    {
        // SupplyFinanceChain payment with at least one intermediate step and uses
        // transitive balances.
        ...
        path::SupplyFinanceChainCalc::Output rc;
        {
            PaymentSandbox pv(&view());
            JLOG(j_.debug())
                << "Entering SupplyFinanceChainCalc in payment: "
        ...
        // Because of its overhead, if SupplyFinanceChainCalc
    ...
    auto const mmm = std::max(reserve,
        ctx_.tx.getFieldAmount (sfFee).sfc ());

    if (mPriorBalance < saDstAmount.sfc () + mmm)
    {
        // Vote no. However the transaction might succeed, if applied in
        // a different order.
        JLOG(j_.trace()) << "Delay transaction: Insufficient funds: " <<
            " " << to_string (mPriorBalance) <<
            " / " << to_string (saDstAmount.sfc () + mmm) <<
            " (" << to_string (reserve) << ")";
    ...
    {
        // If depositPreauth is enabled, then an account that requires
        // authorization has three ways to get an SFC Payment in:
        //  1. If Account == Destination, or
        //  2. If Account is deposit preauthorized by destination, or
        //  3. If the destination's SFC balance is
        ...
        // then consumes all of its SFC.  Without the rule if an
        // account with lsfDepositAuth set spent all of its SFC, it
        // would be unable to acquire more SFC required to pay fees.
        //
        ...
        if (uDstAccountID != account_)
        {
                ...
                keylet::depositPreauth (uDstAccountID, account_)))
            {
                ...
                SFCAmount const dstReserve {view().fees().accountReserve (0)};
