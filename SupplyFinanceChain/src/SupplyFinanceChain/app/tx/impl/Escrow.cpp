...
#include <SupplyFinanceChain/app/tx/impl/Escrow.h>
...
#include <SupplyFinanceChain/conditions/Condition.h>
#include <SupplyFinanceChain/conditions/Fulfillment.h>
...
/*
    ...
    Escrow is a feature of the SFC Ledger that allows you to send conditional
    SFC payments. These conditional payments, called escrows, set aside SFC and
    ...
    The SFC set aside in an escrow is locked up. No one can use or destroy the
    SFC until the escrow has been successfully finished or canceled. Before the
    expiration time, only the intended receiver can get the SFC. After the
    expiration time, the SFC can only be returned to the sender.

    For more details on escrow, including examples, diagrams and more please
    visit https://SupplyFinanceChain.com/build/escrow/#escrow
    ...
    `EscrowCreate`
    --------------
        See: https://SupplyFinanceChain.com/build/transactions/#escrowcreate

    `EscrowFinish`
    --------------
        See: https://SupplyFinanceChain.com/build/transactions/#escrowfinish

    `EscrowCancel`
    --------------
        See: https://SupplyFinanceChain.com/build/transactions/#escrowcancel
*/
...
EscrowCreate::preflight (PreflightContext const& ctx)
{
    ...
    {
        using namespace SupplyFinanceChain::cryptoconditions;
...
EscrowCreate::doApply()
{
    ...
    {
        auto const balance = STAmount((*sle)[sfBalance]).sfc();
        ...
        if (balance < reserve + STAmount(ctx_.tx[sfAmount]).sfc())
            return tecUNFUNDED;
    }
    ...
    // Check destination account
    {
        ...
        // Obeying the lsfDissalowSFC flag was a bug.  Piggyback on
