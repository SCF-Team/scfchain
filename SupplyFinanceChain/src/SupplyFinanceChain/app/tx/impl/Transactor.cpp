...
preflight1 (PreflightContext const& ctx)
{
    ...
    if (!fee.native () || fee.negative () || !isLegalAmount (fee.sfc ()))
    {
...
TER Transactor::payFee ()
{
    ...
    // VFALCO Should we call view().rawDestroySFC() here as well?
...
TER Transactor::apply ()
{
    ...
    if (sle)
    {
        mPriorBalance   = STAmount ((*sle)[sfBalance]).sfc ();
...
SFCAmount
Transactor::reset(SFCAmount fee)
{
    ctx_.discard();
    ...
    auto const balance = txnAcct->getFieldAmount (sfBalance).sfc ();
...
Transactor::operator()()
{
    ...
    auto fee = ctx_.tx.getFieldAmount(sfFee).sfc ();
