...
#include <SupplyFinanceChain/app/tx/impl/PayChan.h>
#include <SupplyFinanceChain/basics/chrono.h>
#include <SupplyFinanceChain/basics/Log.h>
#include <SupplyFinanceChain/ledger/ApplyView.h>
...
/*
    PaymentChannel
        ...
        Payment channels permit off-ledger checkpoints of SFC payments flowing
        in a single direction. A channel sequesters the owner's SFC in its own
    ...
    PaymentChannelCreate
        ...
        Amount
            The amount of SFC to deposit in the channel immediately.
        SettleDelay
        ...
        DestinationTag (optional)
            ...
            Wallet to be mapped back onto the SupplyFinanceChain ledger. The destination tag
        ...
        SourceTag (optional)
            ...
            to be mapped back onto the SupplyFinanceChain ledger. Source tags are similar to
    ...
    PaymentChannelClaim
        ...
        Channel
            The 256-bit ID of the channel.
        Balance (optional)
            The total amount of SFC delivered after this claim is processed (optional, not
            needed if just closing).
        Amount (optional)
            The amount of SFC the signature is for (not needed if equal to Balance or just
            closing the line).
        Signature (optional)
...
PayChanCreate::preflight (PreflightContext const& ctx)
{
    ...
    if (!isSFC (ctx.tx[sfAmount]) || (ctx.tx[sfAmount] <= beast::zero))
...
PayChanCreate::preclaim(PreclaimContext const &ctx)
{
...

    {
        ...
        // Obeying the lsfDisallowSFC flag was a bug.  Piggyback on
        // featureDepositAuth to remove the bug.
        if (!ctx.view.rules().enabled(featureDepositAuth) &&
            ((*sled)[sfFlags] & lsfDisallowSFC))
            return tecNO_TARGET;
    }
...
//  bool const noTecs = ctx.rules.enabled(fix1512);
    ...
    if (bal && (!isSFC (*bal) || *bal <= beast::zero))
        return temBAD_AMOUNT;

    auto const amt = ctx.tx[~sfAmount];
    if (amt && (!isSFC (*amt) || *amt <= beast::zero))
        return temBAD_AMOUNT;
    ...
    if (auto const sig = ctx.tx[~sfSignature])
    {
        ...
        auto const reqBalance = bal->sfc ();
        auto const authAmt = amt ? amt->sfc() : reqBalance;
...
PayChanClaim::doApply()
{
    ...
    {
        auto const chanBalance = slep->getFieldAmount (sfBalance).sfc ();
        auto const chanFunds = slep->getFieldAmount (sfAmount).sfc ();
        auto const reqBalance = ctx_.tx[sfBalance].sfc ();
        ...
        if (!depositAuth &&
            (txAccount == src && (sled->getFlags() & lsfDisallowSFC)))
            return tecNO_TARGET;
        ...
        SFCAmount const reqDelta = reqBalance - chanBalance;
