...
CashCheck::preclaim (PreclaimContext const& ctx)
{
    ...
    }
    {
        ...
        // less than value the check cannot be cashed.
        {
            ...
            // Note that src will have one reserve's worth of additional SFC
            // once the check is cashed, since the check's reserve will no
            // longer be required.  So, if we're dealing in SFC, we add one
            // reserve's worth to the available funds.
            if (value.native())
                availableFunds += SFCAmount (ctx.view.fees().increment);
...
CashCheck::doApply ()
{
    ...
    if (srcId != account_)
    {
        ...
        // Flow() doesn't do SFC to SFC transfers.
        if (sendMax.native())
        {
            // Here we need to calculate the amount of SFC sleSrc can send.
            ...
            // argument.
            STAmount const srcLiquid {sfcLiquid (psb, srcId, -1, viewJ)};

            // Now, how much do they need in order to be successful?
            STAmount const sfcDeliver {optDeliverMin ?
                std::max (*optDeliverMin, std::min (sendMax, srcLiquid)) :
                ctx_.tx.getFieldAmount (sfAmount)};

            if (srcLiquid < sfcDeliver)
            {
                // Vote no. However the transaction might succeed if applied
                // in a different order.
                JLOG(j_.trace()) << "Cash Check: Insufficient SFC: "
                    << srcLiquid.getFullText()
                    << " < " << sfcDeliver.getFullText();
                return tecUNFUNDED_PAYMENT;
            }

            if (optDeliverMin && doFix1623)
                // Set the DeliveredAmount metadata.
                ctx_.deliver (sfcDeliver);

            // The source account has enough SFC so make the ledger change.
            transferSFC (psb, srcId, account_, sfcDeliver, viewJ);
        }
        else
        {
