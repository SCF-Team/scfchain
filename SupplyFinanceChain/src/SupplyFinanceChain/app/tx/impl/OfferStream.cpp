...
static
SFCAmount accountFundsHelper (ReadView const& view,
    AccountID const& id,
    SFCAmount const& amtDefault,
    Issue const& issue,
    FreezeHandling freezeHandling,
    beast::Journal j)
{
    return toAmount<SFCAmount> (
        accountHolds (view, id, issue.currency, issue.account, freezeHandling, j));
}
...
template class FlowOfferStream<SFCAmount, IOUAmount>;
template class FlowOfferStream<IOUAmount, SFCAmount>;

template class TOfferStreamBase<STAmount, STAmount>;
template class TOfferStreamBase<IOUAmount, IOUAmount>;
template class TOfferStreamBase<SFCAmount, IOUAmount>;
template class TOfferStreamBase<IOUAmount, SFCAmount>;
}
