...
{
    bool toSFC = isSFC (book.out);
    std::lock_guard <std::recursive_mutex> sl (mLock);

    if (toSFC)
    {
        // We don't want to search through all the to-SFC or from-SFC order
        // books!
        for (auto ob: mSourceMap[book.in])
        {
            if (isSFC (ob->getCurrencyOut ())) // also to SFC
                return;
        }
    }
    else
...
bool OrderBookDB::isBookToSFC(Issue const& issue)
{
    std::lock_guard <std::recursive_mutex> sl (mLock);
    return mSFCBooks.count(issue) > 0;
}
