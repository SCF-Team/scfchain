...
#ifndef SUPPLYFINANCECHAIN_LEDGER_CASHDIFF_H_INCLUDED
#define SUPPLYFINANCECHAIN_LEDGER_CASHDIFF_H_INCLUDED
...
public:
    ...
    // Checks for the SFC round-to-zero case.  Returns zero if not detected.
    // Otherwise returns -1 if seen on lhs, +1 if seen on rhs.
    //
    // For tiny offers of TakerPays IOU and TakerGets SFC, cases have been
    // observed where SFC rounding allows a tiny amount of IOU to be
    // removed from an Offer while returning no SFC to the offer owner.
    // That's because the SFC amount was rounded down to zero drops.
    //
...
// If both v1 and v2 are SFC, consider any diff of 2 drops or less to be dust.
