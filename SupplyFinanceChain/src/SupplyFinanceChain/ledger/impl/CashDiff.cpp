...
#include <SupplyFinanceChain/ledger/CashDiff.h>
#include <SupplyFinanceChain/ledger/detail/ApplyStateTable.h>
...
struct CashSummary
{
    ...
    std::vector<std::pair<
        AccountID, SFCAmount>> sfcChanges;
    ...
    void sort()
    {
        std::sort (sfcChanges.begin(), sfcChanges.end());
    ...
    std::shared_ptr<SLE const> const& after)
{
    if (isDelete)
    {
        ...
        {
        case ltACCOUNT_ROOT:
            result.sfcChanges.push_back (
                std::make_pair (prev[sfAccount], SFCAmount {0}));
            return true;
    ...
    else
    {
        ...
        case ltACCOUNT_ROOT:
        {
            auto const curSfc = cur[sfBalance].sfc();
            if (!before || (*before)[sfBalance].sfc() != curSfc)
                result.sfcChanges.push_back (
                    std::make_pair (cur[sfAccount], curSfc));
            return true;
        }
        case ltSUPPLYFINANCECHAIN_STATE:
        {
...
{
private:
    // Note differences in destroyed SFC between two ApplyStateTables.
    struct DropsGone
    {
        SFCAmount lhs;
        SFCAmount rhs;
...
countKeys (detail::CashSummary const& lhs, detail::CashSummary const& rhs)
{
    ...
    addIn (countKeys(lhs.sfcChanges,     rhs.sfcChanges));
...
int CashDiff::Impl::sfcRoundToZero () const
{
    // The case has one OfferChange that is present on both lhs_ and rhs_.
    // That OfferChange should have SFC for TakerGets.  There should be a 1
    ...
    // The side with the smaller SFC balance in the OfferChange should have
    // two SFC differences.  The other side should have no SFC differences.
    if (smaller.sfcChanges.size() != 2)
        return 0;
    if (! bigger.sfcChanges.empty())
        return 0;
...
bool CashDiff::Impl::rmDust ()
{
    ...
    // amount differences: sfcChanges, trustChanges, offerChanges, and
    // offerDeletions.  Rifle through those containers and remove any
    // entries that are _almost_ the same between lhs and rhs.

    // sfcChanges.  We call a difference of 2 drops or less dust.
    removedDust |= rmVecDust (lhsDiffs_.sfcChanges, rhsDiffs_.sfcChanges,
        [](SFCAmount const& lhs, SFCAmount const& rhs)
        {
        ...
        CashFilter rhsFilter, detail::ApplyStateTable const& rhs)
{
    ...
    // sfcChanges:
    setDiff (lhsDiffs.sfcChanges, rhsDiffs.sfcChanges, lhsDiffs_.sfcChanges);
    setDiff (rhsDiffs.sfcChanges, lhsDiffs.sfcChanges, rhsDiffs_.sfcChanges);
...
int CashDiff::sfcRoundToZero() const
{
    return impl_->sfcRoundToZero();
}
...
bool diffIsDust (STAmount const& v1, STAmount const& v2, std::uint8_t e10)
{
    ...
    // Handling SFC is different from IOU.
    if (v1.native())
    {
