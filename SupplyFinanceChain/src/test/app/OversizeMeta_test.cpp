...
BEAST_DEFINE_TESTSUITE_MANUAL_PRIO(PlumpBook,tx,SupplyFinanceChain,5);
...
BEAST_DEFINE_TESTSUITE(ThinBook, tx, SupplyFinanceChain);
...
BEAST_DEFINE_TESTSUITE_MANUAL_PRIO(OversizeMeta,tx,SupplyFinanceChain,3);
...
public:
    ...
    oversize(std::size_t n)
    {
        ...
        createOffers(env, USD, n);
        env(pay("alice", gw, USD(billion)));
        env(offer("alice", USD(1), SFC(1)), ter(std::ignore));
...
BEAST_DEFINE_TESTSUITE_MANUAL_PRIO(FindOversizeCross,tx,SupplyFinanceChain,50);
