...
class Ticket_test : public beast::unit_test::suite
{
    ...
    void testTicketCreatePreflightFail ()
    {
        ...
        env (ticket::create (env.master), fee (SFC (-1)), ter (temBAD_FEE));
        env (ticket::cancel (env.master, idOne), fee (SFC (-1)), ter (temBAD_FEE));
    }
...
BEAST_DEFINE_TESTSUITE (Ticket, tx, SupplyFinanceChain);
