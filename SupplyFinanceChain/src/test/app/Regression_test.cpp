...
struct Regression_test : public beast::unit_test::suite
{
    ...
    void testOffer1()
    {
        ...
        env(offer("alice", USD(10), SFC(10)), require(owners("alice", 1)));
        env(offer("alice", USD(20), SFC(10)), json(R"raw(
                { "OfferSequence" : 2 }
    ...
    {
        testcase("Account balance < fee destroys correct amount of SFC");
        using namespace jtx;
        Env env(*this);
        env.memoize("alice");
        ...
        auto const aliceSFC = 400;
        auto const aliceAmount = SFC(aliceSFC);
            ...
            env.app().timeKeeper().closeTime());
        {
            ...
            auto const result = SupplyFinanceChain::apply(env.app(),
        ...
        }
        {
            ...
            BEAST_EXPECT(balance == SFC(0));
        }
        expectedDrops -= aliceSFC * dropsPerSFC<int>::value;
        BEAST_EXPECT(next->info().drops == expectedDrops);
    }
...
BEAST_DEFINE_TESTSUITE(Regression,app,SupplyFinanceChain);
