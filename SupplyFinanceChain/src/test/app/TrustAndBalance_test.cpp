...
class TrustAndBalance_test : public beast::unit_test::suite
{
    ...
    testPayNonexistent (FeatureBitset features)
    {
        ...
        env (pay (env.master, "alice", SFC(1)), ter(tecNO_DST_INSUF_SFC));
        env.close();
    }
    ...
    void
    testDirectSupplyFinanceChain (FeatureBitset features)
    {
        testcase ("Direct Payment, SupplyFinanceChain");
        using namespace test::jtx;
    ...
    testIndirectMultiPath (bool with_rate, FeatureBitset features)
    {
        ...
        env.fund (SFC(10000), gw, amazon, alice, bob, carol);
        env.close();
    ...
    testInvoiceID (FeatureBitset features)
    {
        ...
        Json::Value jv;
        auto tx = env.jt (
            pay (env.master, alice, SFC(10000)),
...
public:
    void run () override
    {
        ...
        auto testWithFeatures = [this](FeatureBitset features) {
            ...
            testDirectSupplyFinanceChain(features);
...
BEAST_DEFINE_TESTSUITE_PRIO (TrustAndBalance, app, SupplyFinanceChain, 1);
