...
/*
    ...
    Copyright (c) 2017 SupplyFinanceChain Labs Inc.
...
struct DepositAuth_test : public beast::unit_test::suite
{
    ...
    void testPayIOU()
    {
        // Exercise IOU payments and non-direct SFC payments to an account
        ...
        env (offer (carol, USD(100), SFC(100)));
        env.close();
        ...
        auto failedIouPayments = [this, &env, &alice, &bob, &USD] ()
        {
            ...
            PrettyAmount const bobSfcBalance {env.balance (bob, SFC)};
            PrettyAmount const bobUsdBalance {env.balance (bob, USD)};

            env (pay (alice, bob, USD (50)), ter (tecNO_PERMISSION));
            env.close();

            // Note that even though alice is paying bob in SFC, the payment
            ...
            BEAST_EXPECT (bobSfcBalance == env.balance (bob, SFC));
            BEAST_EXPECT (bobUsdBalance == env.balance (bob, USD));
        };

        //  Test when bob has an SFC balance > base reserve.
        failedIouPayments();

        // Set bob's SFC balance == base reserve.  Also demonstrate that
        // bob can make payments while his lsfDepositAuth flag is set.
        env (pay (bob, alice, USD(25)));
        env.close();

        {
            STAmount const bobPaysSFC {
                env.balance (bob, SFC) - reserve (env, 1)};
            SFCAmount const bobPaysFee {reserve (env, 1) - reserve (env, 0)};
            env (pay (bob, alice, bobPaysSFC), fee (bobPaysFee));
            env.close();
        }

        // Test when bob's SFC balance == base reserve.
        BEAST_EXPECT (env.balance (bob, SFC) == reserve (env, 0));
        BEAST_EXPECT (env.balance (bob, USD) == USD(25));
        failedIouPayments();

        // Test when bob has an SFC balance == 0.
        env (noop (bob), fee (reserve (env, 0)));
        env.close ();

        BEAST_EXPECT (env.balance (bob, SFC) == SFC (0));
        failedIouPayments();

        // Give bob enough SFC for the fee to clear the lsfDepositAuth flag.
    ...
    void testPaySFC()
    {
        // Exercise direct SFC payments to an account that has the
        ...
        env.close();
        BEAST_EXPECT (env.balance (bob, SFC) == SFC (10000) - drops(10));

        // bob has more SFC than the base reserve.  Any SFC payment should fail.
        env (pay (alice, bob, drops(1)), ter (tecNO_PERMISSION));
        env.close();
        BEAST_EXPECT (env.balance (bob, SFC) == SFC (10000) - drops(10));

        // Change bob's SFC balance to exactly the base reserve.
        {
            STAmount const bobPaysSFC {
                env.balance (bob, SFC) - reserve (env, 1)};
            SFCAmount const bobPaysFee {reserve (env, 1) - reserve (env, 0)};
        ...
        // bob has exactly the base reserve.  A small enough direct SFC
        ...
        BEAST_EXPECT (env.balance (bob, SFC) == reserve (env, 0) + drops(1));
        env (pay (alice, bob, drops(1)), ter (tecNO_PERMISSION));
        env.close();

        // Take bob down to a balance of 0 SFC.
        env (noop (bob), fee (reserve (env, 0) + drops(1)));
        env.close ();
        BEAST_EXPECT (env.balance (bob, SFC) == drops(0));
        ...
        env (pay (alice, bob, reserve (env, 0) + drops(0)));
        env.close();
        BEAST_EXPECT (env.balance (bob, SFC) ==
        ...
        // Take bob back down to a zero SFC balance.
        env (noop (bob), fee (env.balance (bob, SFC)));
        env.close();
        BEAST_EXPECT (env.balance (bob, SFC) == drops(0));
        ...
        env (pay (alice, bob, drops(1)));
        env.close();
        BEAST_EXPECT (env.balance (bob, SFC) == drops(1));
    ...
    void testNoSupplyFinanceChain()
    {
        // It its current incarnation the DepositAuth flag does not change
        // any behaviors regarding rippling and the NoSupplyFinanceChain flag.
        // Demonstrate that.
        testcase ("No SupplyFinanceChain");
        ...
        auto testIssuer = [&] (FeatureBitset const& features,
            bool noSupplyFinanceChainPrev,
            bool noSupplyFinanceChainNext,
            bool withDepositAuth)
        {
            ...
            env.fund(SFC(10000), gw1, alice, bob);
            env (trust (gw1, alice["USD"](10), noSupplyFinanceChainPrev ? tfSetNoSupplyFinanceChain : 0));
            env (trust (gw1, bob["USD"](10), noSupplyFinanceChainNext ? tfSetNoSupplyFinanceChain : 0));
            ...
            TER const result = (noSupplyFinanceChainNext && noSupplyFinanceChainPrev)
                ? TER {tecPATH_DRY}
                : TER {tesSUCCESS};
            ...
            bool withDepositAuth)
        {
            ...
            env.fund(SFC(10000), gw1, gw2, alice);
            env (trust (alice, USD1(10), noSupplyFinanceChainPrev ? tfSetNoSupplyFinanceChain : 0));
            env (trust (alice, USD2(10), noSupplyFinanceChainNext ? tfSetNoSupplyFinanceChain : 0));
        ...
        // Test every combo of noSupplyFinanceChainPrev, noSupplyFinanceChainNext, and withDepositAuth
        for (int i = 0; i < 8; ++i)
        {
            auto const noSupplyFinanceChainPrev = i & 0x1;
            auto const noSupplyFinanceChainNext = i & 0x2;
            auto const withDepositAuth = i & 0x4;
            testIssuer(
                supported_amendments() | featureDepositAuth,
                noSupplyFinanceChainPrev,
                noSupplyFinanceChainNext,
                withDepositAuth);
...
struct DepositPreauth_test : public beast::unit_test::suite
{
    void testEnable()
    {
        ...
        Account const becky {"becky"};
        {
            ...
            env.fund (SFC (10000), alice, becky);
            env.close();
    ...
    void testInvalid()
    {
        ...
        }
        {
            ...
            Json::Value tx {deposit::auth (alice, becky)};
            tx[sfAuthorize.jsonName] = to_string (sfcAccount());
        ...
        // carol gets enough SFC to (barely) meet the reserve.
    ...
    void testPayment (FeatureBitset features)
    {
...

        {
            ...
            Env env (*this, features);
            env.fund (SFC (5000), alice, becky, gw);
            env.close();
            ...
            env (offer (alice, SFC (100), USD (100), tfPassive),
            ...
            env (pay (becky, becky, USD (10)),
                path (~USD), sendmax (SFC (10)), ter (expect));
            env.close();
        }
        ...
        if (supportsPreauth)
        {
            ...
            Env env (*this, features);
            env.fund (SFC (5000), alice, becky, carol, gw);
            env.close();
            ...
            // Make SFC and IOU payments from alice to becky.  Should be fine.
            env (pay (alice, becky, SFC (100)));
            ...
            env (pay (alice, becky, SFC (100)), ter (tecNO_PERMISSION));
            ...
            env (pay (becky, alice, SFC (100)), ter (tecNO_PERMISSION));
...
BEAST_DEFINE_TESTSUITE(DepositAuth,app,SupplyFinanceChain);
BEAST_DEFINE_TESTSUITE(DepositPreauth,app,SupplyFinanceChain);
