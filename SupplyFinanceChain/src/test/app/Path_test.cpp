...
#include <SupplyFinanceChain/app/paths/AccountCurrencies.h>
...
rpf(jtx::Account const& src, jtx::Account const& dst, std::uint32_t num_src)
{
    ...
    jv[jss::command] = "SupplyFinanceChain_path_find";
...
auto IPE(Issue const& iss)
{
    return STPathElement (
        ...
        sfcAccount (), iss.currency, iss.account);
};
...
public:
                    ...
                    boost::optional<Currency> const& saSrcCurrency = boost::none)
    {
        ...
        params[jss::command] = "SupplyFinanceChain_path_find";
    ...
    path_find_consume_all()
    {
...

        {
            ...
            env(offer("carol", SFC(100), USD(100)));
            ...
            std::tie(st, sa, da) = find_paths(env,
                "alice", "bob", Account("bob")["AUD"](-1),
                    boost::optional<STAmount>(SFC(100000000)));
            BEAST_EXPECT(st.empty());
            std::tie(st, sa, da) = find_paths(env,
                "alice", "bob", Account("bob")["USD"](-1),
                    boost::optional<STAmount>(SFC(100000000)));
            BEAST_EXPECT(sa == SFC(100));
    ...
    alternative_path_consume_both()
    {
        ...
        env.fund(SFC(10000), "alice", "bob", gw, gw2);
        env.trust(USD(600), "alice");
    ...
    alternative_paths_limit_returned_paths_to_best_quality()
    {
        ...
        auto const gw2 = Account("gateway2");
        auto const gw2_USD = gw2["USD"];
        env.fund(SFC(10000), "alice", "bob", "carol", "dan", gw, gw2);
        env(rate("carol", 1.1));
    ...
    void
    issues_path_negative_SupplyFinanceChain_client_issue_23_smaller()
    {
        testcase("path negative: SupplyFinanceChain-client issue #23: smaller");
        using namespace jtx;
        Env env(*this);
        env.fund(SFC(10000), "alice", "bob", "carol", "dan");
    ...
    void
    issues_path_negative_SupplyFinanceChain_client_issue_23_larger()
    {
        testcase("path negative: SupplyFinanceChain-client issue #23: larger");
        using namespace jtx;
        Env env(*this);
        env.fund(SFC(10000), "alice", "bob", "carol", "dan", "edward");
    ...
    // carol holds gateway AUD, sells gateway AUD for SFC
    // bob will hold gateway AUD
    // alice pays bob gateway AUD using SFC
    void
    via_offers_via_gateway()
    {
        ...
        env(offer("carol", SFC(50), AUD(50)));
        env(pay("alice", "bob", AUD(10)), sendmax(SFC(100)), paths(SFC));
    ...
    quality_paths_quality_set_and_test()
    {
        ...
        Json::Reader().parse(R"({
                ...
                "LedgerEntryType" : "SupplyFinanceChainState",
                "LowLimit" : {
                    "currency" : "USD",
    ...
    void path_find_01()
    {
        testcase("Path Find: SFC -> SFC and SFC -> IOU");
        using namespace jtx;
        Env env(*this);
        Account A1 {"A1"};
        Account A2 {"A2"};
        ...
        env.fund(SFC(100000), A1);
        env.fund(SFC(10000), A2);
        env.fund(SFC(1000), A3, G1, G2, G3, M1);
        env.close();
        ...
        env(offer(M1, SFC(10000), G3["ABC"](1000)));

        STPathSet st;
        STAmount sa, da;

        {
            auto const& send_amt = SFC(10);
            std::tie(st, sa, da) = find_paths(env, A1, A2, send_amt,
                boost::none, sfcCurrency());
    ...
    void path_find_02()
    {
        testcase("Path Find: non-SFC -> SFC");
        using namespace jtx;
        Env env(*this);
        Account A1 {"A1"};
        Account A2 {"A2"};
        ...
        env.fund(SFC(1000), A1, A2, G3);
        env.fund(SFC(11000), M1);
        env.close();
        ...
        env(offer(M1, G3["ABC"](1000), SFC(10000)));
        ...
        BEAST_EXPECT(same(st, stpath(G3, IPE(sfcIssue()))));
    }
    ...
    void path_find_03()
    {
        ...
        env.fund(SFC(4999.999898), SRC);
        env.fund(SFC(10846.168060), GATEWAY_DST);
        env.fund(SFC(4291.430036), MONEY_MAKER_1);
        env.fund(SFC(106839.375770), MONEY_MAKER_2);
        env.fund(SFC(1240.997150), A1);
        env.fund(SFC(14115.046893), A2);
        env.fund(SFC(512087.883181), A3);
        env.close();
        ...
        env(offer(MONEY_MAKER_2, SFC(1), GATEWAY_DST["CNY"](1)));
        env(offer(MONEY_MAKER_2, GATEWAY_DST["CNY"](1), SFC(1)));
        env(offer(MONEY_MAKER_2, GATEWAY_DST["CNY"](318000), SFC(53000)));
        env(offer(MONEY_MAKER_2, SFC(209), MONEY_MAKER_2["CNY"](4.18)));
        env(offer(MONEY_MAKER_2, MONEY_MAKER_1["CNY"](990000), SFC(10000)));
        env(offer(MONEY_MAKER_2, MONEY_MAKER_1["CNY"](9990000), SFC(10000)));
        env(offer(MONEY_MAKER_2, GATEWAY_DST["CNY"](8870000), SFC(10000)));
        env(offer(MONEY_MAKER_2, SFC(232), MONEY_MAKER_2["CNY"](5.568)));
        env(offer(A2, SFC(2000), MONEY_MAKER_1["CNY"](66.8)));
        env(offer(A2, SFC(1200), GATEWAY_DST["CNY"](42)));
        env(offer(A2, MONEY_MAKER_1["CNY"](43.2), SFC(900)));
        env(offer(A3, MONEY_MAKER_1["CNY"](2240), SFC(50000)));
        ...
        std::tie(st, sa, da) = find_paths(env, SRC, GATEWAY_DST, send_amt,
            boost::none, sfcCurrency());
        BEAST_EXPECT(equal(da, send_amt));
        BEAST_EXPECT(equal(sa, SFC(288.571429)));
    ...
    void path_find_04()
    {
        ...
        env.fund(SFC(1000), G1BS, G2SW, A1, A2);
        env.fund(SFC(11000), M1);
        env.close();
        ...
        // can charge a fee when transactions SupplyFinanceChain across. Liquidity
    ...
    void path_find_05()
    {
        testcase("Path Find: non-SFC -> non-SFC, same currency");
        using namespace jtx;
        Env env(*this);
        Account A1 {"A1"};
        Account A2 {"A2"};
        ...
        env.fund(SFC(1000), A1, A2, A3, G1, G2, G3, G4);
        env.fund(SFC(10000), A4);
        env.fund(SFC(11000), M1, M2);
        env.close();
        ...
        env(offer(M2, SFC(10000), G2["HKD"](1000)));
        env(offer(M2, G1["HKD"](1000), SFC(10000)));
...

        {
            ...
            BEAST_EXPECT(same(st,
                ...
                stpath(M1),
                stpath(M2),
                stpath(IPE(sfcIssue()), IPE(G2["HKD"]))
            ));
        }

        {
            ...
            BEAST_EXPECT(same(st,
                ...
                stpath(G1, IPE(sfcIssue()), IPE(G2["HKD"]))
            ));
        }

        {
            //I4) SFC bridge" --
            //  Source -> AC -> OB to SFC -> OB from SFC -> AC -> Destination
            auto const& send_amt = A2["HKD"](10);
            std::tie(st, sa, da) = find_paths(env, A1, A2, send_amt,
                boost::none, G1["HKD"].currency);
            BEAST_EXPECT(equal(da, send_amt));
            ...
            BEAST_EXPECT(same(st,
                ...
                stpath(G1, IPE(sfcIssue()), IPE(G2["HKD"]), G2)
            ));
        }
    }
    ...
    void path_find_06()
    {
        ...
        env.fund(SFC(11000), M1);
        env.fund(SFC(1000), A1, A2, A3, G1, G2);
        env.close();
...
BEAST_DEFINE_TESTSUITE(Path,app,SupplyFinanceChain);
