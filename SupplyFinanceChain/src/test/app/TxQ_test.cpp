...
#include <SupplyFinanceChain/app/tx/apply.h>
...
class TxQ_test : public beast::unit_test::suite
{
        ...
        std::uint32_t units, std::uint32_t reserve, std::uint32_t increment)
    {
        ...
        auto const flagPerLedger = 1 +
            SupplyFinanceChain::detail::supportedAmendments().size();
...
public:
    void testQueue()
    {
        ...
        env.fund(SFC(50000), noSupplyFinanceChain(alice, bob, charlie, daria));
        ...
        env.fund(SFC(50000), noSupplyFinanceChain(elmo, fred, gwen, hank));
    ...
    void testTecResult()
    {
        ...
        // Create accounts
        env.fund(SFC(50000), noSupplyFinanceChain(alice, gw));
        ...
        env(offer(alice, SFC(1000), USD(1000)), ter(tecUNFUNDED_OFFER));
        ...
        env(offer(alice, SFC(1000), USD(1000)), ter(terQUEUED));
    ...
    void testLocalTxRetry()
    {
        ...
        // Create several accounts while the fee is cheap so they all apply.
        env.fund(SFC(50000), noSupplyFinanceChain(alice, bob, charlie));
    ...
    void testLastLedgerSeq()
    {
        ...
        env.fund(SFC(1000), noSupplyFinanceChain(alice, bob));
        env.close(env.now() + 5s, 10000ms);
        env.fund(SFC(1000), noSupplyFinanceChain(charlie, daria));
        env.close(env.now() + 5s, 10000ms);
        env.fund(SFC(1000), noSupplyFinanceChain(edgar, felicia));
        env.close(env.now() + 5s, 10000ms);
    ...
    void testZeroFeeTxn()
    {
        ...
        env.fund(SFC(1000), noSupplyFinanceChain(alice, bob));
        env.close(env.now() + 5s, 10000ms);
        env.fund(SFC(1000), noSupplyFinanceChain(carol));
        env.close(env.now() + 5s, 10000ms);
    ...
    void testPreclaimFailures()
    {
        ...
        env.fund(SFC(1000), noSupplyFinanceChain(alice));
        ...
        // Fail in preflight
        env(pay(alice, bob, SFC(-1000)),
            ter(temBAD_AMOUNT));

        // Fail in preclaim
        env(noop(alice), fee(SFC(100000)),
            ter(terINSUF_FEE_B));
    }
            ...
            env.app().openLedger().modify(
                [&](OpenView& view, beast::Journal j)
                {
                    std::tie(ter, didApply) =
                        SupplyFinanceChain::apply(env.app(),
                            view, *jt.stx, tapNONE,
                                env.journal);
                    return didApply;
                }
    ...
    void testMultiTxnPerAccount()
    {
        ...
        // Create several accounts while the fee is cheap so they all apply.
        env.fund(drops(2000), noSupplyFinanceChain(alice));
        env.fund(SFC(500000), noSupplyFinanceChain(bob, charlie, daria));
        ...
        aliceFee = env.le(alice)->getFieldAmount(sfBalance).sfc().drops()
            - (59);
        env(noop(alice), seq(aliceSeq),
        ...
        // Alice is broke
        env.require(balance(alice, SFC(0)));
    ...
    void testAcctTxnID()
    {
        ...
        env.fund(SFC(50000), noSupplyFinanceChain(alice));
    ...
    void testUnexpectedBalanceChange()
    {
        ...
        env.fund(drops(5000), noSupplyFinanceChain(alice));
        env.fund(SFC(50000), noSupplyFinanceChain(bob));
        ...
        /* At this point, Alice's transaction is indefinitely
            ...
            will get more SFC, and it'll process.
        */
    ...
    void testBlockers()
    {
        ...
        env.fund(SFC(50000), noSupplyFinanceChain(alice, bob));
        env.memoize(charlie);
        env.memoize(daria);
    ...
    void testInFlightBalance()
    {
        ...
        env.fund(SFC(50000), noSupplyFinanceChain(alice, charlie), gw);
        ...
        //////////////////////////////////////////
        // Offer with high SFC out and low fee doesn't block
        auto aliceSeq = env.seq(alice);
        auto aliceBal = env.balance(alice);

        env.require(balance(alice, SFC(50000)),
            owners(alice, 0));

        // If this offer crosses, all of alice's
        // SFC will be taken (except the reserve).
        env(offer(alice, BUX(5000), SFC(50000)),
            queued);
        ...
        // has plenty of SFC, because the offer didn't
        // cross (of course).
        ...
        //////////////////////////////////////////
        // Offer with high SFC out and high total fee blocks later txs
        fillQueue(env, alice);
        ...
        env(offer(alice, BUX(5000), SFC(50000)), fee(drops(100)),
            queued);
        ...
        //////////////////////////////////////////
        // Offer with high SFC out and super high fee blocks later txs
        fillQueue(env, alice);
        checkMetrics(env, 0, limit * 2, limit + 1, limit, 256);
        aliceSeq = env.seq(alice);
        aliceBal = env.balance(alice);
        ...
        env(offer(alice, BUX(5000), SFC(50000)), fee(drops(300)),
            queued);
        checkMetrics(env, 1, limit * 2, limit + 1, limit, 256);
        ...
        //////////////////////////////////////////
        // Offer with low SFC out allows later txs
        fillQueue(env, alice);
        checkMetrics(env, 0, limit * 2, limit + 1, limit, 256);
        aliceSeq = env.seq(alice);
        aliceBal = env.balance(alice);
        ...
        // of alice's SFC will be taken.
        env(offer(alice, BUX(50), SFC(500)),
            queued);
        ...
        //////////////////////////////////////////
        // Large SFC payment doesn't block later txs
        fillQueue(env, alice);
        checkMetrics(env, 0, limit * 2, limit + 1, limit, 256);
        ...
        // (minus the reserve).
        env(pay(alice, charlie, SFC(50000)),
            queued);
        ...
        //////////////////////////////////////////
        // Small SFC payment allows later txs
        fillQueue(env, alice);
        checkMetrics(env, 0, limit * 2, limit + 1, limit, 256);
        ...
        env.require(balance(alice, aliceBal - SFC(500) - drops(20)),
            owners(alice, 0));
        ...
        // alice's SFC balance (other than the fee, of course).
        env(noop(alice), seq(aliceSeq + 1), queued);
        checkMetrics(env, 2, limit * 2, limit + 1, limit, 256);
        ...
        // SFC balance, but her USD balance went to charlie.
        env.require(balance(alice, aliceBal - drops(20)),
            balance(alice, USD(0)),
        ...
        //////////////////////////////////////////
        // Large SFC to IOU payment doesn't block later txs.

        env(offer(gw, SFC(500000), USD(50000)));
        // Close so we don't have to deal
        // with tx ordering in consensus.
        env.close();
        ...
        // entire SFC balance to charlie in the
        // form of USD.
        BEAST_EXPECT(SFC(60000) > aliceBal);
        env(pay(alice, charlie, USD(1000)),
            sendmax(SFC(60000)), queued);
        ...
        // to charlie using only a portion of her SFC balance
        env.require(balance(alice, aliceBal - SFC(10000) - drops(20)),
            balance(alice, USD(0)),
        ...
        //////////////////////////////////////////
        // Small SFC to IOU payment allows later txs.
        ...
        // a portion of her SFC balance to charlie
        // in the form of USD.
        BEAST_EXPECT(aliceBal > SFC(6001));
        env(pay(alice, charlie, USD(500)),
            sendmax(SFC(6000)), queued);
        ...
        // So once we close the ledger, alice sent a payment
        // to charlie using only a portion of her SFC balance
        env.require(balance(alice, aliceBal - SFC(5000) - drops(20)),
            balance(alice, USD(0)),
    ...
    void testExpirationReplacement()
    {
        ...
        */
        ...
        env.fund(SFC(500000), noSupplyFinanceChain(alice, bob));
        checkMetrics(env, 0, boost::none, 2, 1, 256);
    ...
    void testServerSubscribe()
    {
        ...
        env.fund(SFC(50000), noSupplyFinanceChain(a,b,c,d));
        checkMetrics(env, 0, boost::none, 4, 3, 256);
        ...
        env.fund(SFC(50000), noSupplyFinanceChain(e,f,g,h,i));
    ...
    void testClearQueuedAccountTxs()
    {
        ...
        checkMetrics(env, 0, boost::none, 0, 3, 256);
        env.fund(SFC(50000000), alice, bob);
        ...
        testcase("clear queue failure (load)");
        {
            ...
            env(noop(alice), fee(SFC(1)), seq(aliceSeq++), ter(terQUEUED));
...
BEAST_DEFINE_TESTSUITE_PRIO(TxQ,app,SupplyFinanceChain,1);
