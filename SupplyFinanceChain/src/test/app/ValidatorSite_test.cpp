...
/*
    ...
    Copyright 2016 SupplyFinanceChain Labs Inc.
...
private:
    ...
    testConfigLoad ()
    {
        ...
        std::vector<std::string> cfgSites({
            "http://SupplyFinanceChain.com/",
            "http://SupplyFinanceChain.com/validators",
            "http://SupplyFinanceChain.com:8080/validators",
            "http://207.261.33.37/validators",
            "http://207.261.33.37:8080/validators",
            "https://SupplyFinanceChain.com/validators",
            "https://SupplyFinanceChain.com:443/validators",
            "file:///etc/opt/SupplyFinanceChain/validators.txt",
        ...
        std::vector<std::string> badSites(
            {"ftp://SupplyFinanceChain.com/validators"});
        BEAST_EXPECT(!trustedSites->load (badSites));

        badSites[0] = "wss://SupplyFinanceChain.com/validators";
        BEAST_EXPECT(!trustedSites->load (badSites));

        badSites[0] = "SupplyFinanceChain.com/validators";
        BEAST_EXPECT(!trustedSites->load (badSites));

        // Host names are not supported for file URLs
        badSites[0] = "file://SupplyFinanceChain.com/vl.txt";
        BEAST_EXPECT(!trustedSites->load (badSites));
...
BEAST_DEFINE_TESTSUITE(ValidatorSite, app, SupplyFinanceChain);
