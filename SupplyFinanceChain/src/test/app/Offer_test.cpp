...
{
    SFCAmount reserve(jtx::Env& env, std::uint32_t count)
    {
    ...
    static auto
    sfcMinusFee (jtx::Env const& env, std::int64_t sfcAmount)
        -> jtx::PrettyAmount
    {
        using namespace jtx;
        auto feeDrops = env.current()->fees().base;
        return drops (dropsPerSFC<std::int64_t>::value * sfcAmount - feeDrops);
    }
        ...
        jtx::Account const & acct_b, std::string const& currency)
    {
        ...
        jvParams[jss::SupplyFinanceChain_state][jss::currency] = currency;
        jvParams[jss::SupplyFinanceChain_state][jss::accounts] = Json::arrayValue;
        jvParams[jss::SupplyFinanceChain_state][jss::accounts].append(acct_a.human());
        jvParams[jss::SupplyFinanceChain_state][jss::accounts].append(acct_b.human());
...
public:
    void testRmFundedOffer (FeatureBitset features)
    {
        ...
        // be "taker gets" SFC. Old, broken would remove the first
        // "taker gets" sfc offer, even though the offer is still funded and
        ...
        // "taker gets" must be SFC
        // (Different amounts so I can distinguish the offers)
        env (offer (carol, BTC (49), SFC (49)));
        env (offer (carol, BTC (51), SFC (51)));

        // Offers for the poor quality path
        // Must be two offers at the same quality
        env (offer (carol, SFC (50), USD (50)));
        env (offer (carol, SFC (50), USD (50)));

        // Offers for the good quality path
        env (offer (carol, BTC (1), USD (100)));

        PathSet paths (Path (SFC, USD), Path (USD));
        ...
        BEAST_EXPECT(!isOffer (env, carol, BTC (1), USD (100)) &&
            isOffer (env, carol, BTC (49), SFC (49)));
    }
    ...
    void testCanceledOffer (FeatureBitset features)
    {
        ...
        env (offer (alice, SFC (500), USD (100)),
            require (offers (alice, 1)));
        env.close();

        BEAST_EXPECT(isOffer (env, alice, SFC (500), USD (100)));

        // cancel the offer above and replace it with a new offer
        auto const offer2Seq = env.seq (alice);

        env (offer (alice, SFC (300), USD (100)),
             json (jss::OfferSequence, offer1Seq),
             require (offers (alice, 1)));
        env.close();

        BEAST_EXPECT(isOffer (env, alice, SFC (300), USD (100)) &&
            !isOffer (env, alice, SFC (500), USD (100)));

        // Test canceling non-existent offer.
//      auto const offer3Seq = env.seq (alice);

        env (offer (alice, SFC (400), USD (200)),
             json (jss::OfferSequence, offer1Seq),
             require (offers (alice, 2)));
        env.close();

        BEAST_EXPECT(isOffer (env, alice, SFC (300), USD (100)) &&
            isOffer (env, alice, SFC (400), USD (200)));

        // Test cancellation now with OfferCancel tx
        auto const offer4Seq = env.seq (alice);
        env (offer (alice, SFC (222), USD (111)),
            require (offers (alice, 3)));
        env.close();

        BEAST_EXPECT(isOffer (env, alice, SFC (222), USD (111)));
        {
        ...
        env.require (offers (alice, 2));
        BEAST_EXPECT( isOffer (env, alice, SFC (300), USD (100))); // offer2
        BEAST_EXPECT(!isOffer (env, alice, SFC   (5), USD   (2))); // expired
    }
    ...
    void testSFCTinyPayment (FeatureBitset features)
    {
        testcase ("SFC Tiny payments");

        // Regression test for tiny sfc payments
        // In some cases, when the payment code calculates
        // the amount of sfc needed as input to an sfc->iou offer
        ...
        // 1) When we calculate the amount of input sfc needed for an offer
        //    from sfc->iou, the amount is less than 1 drop (after rounding
        ...
        for (auto withFix : {false, true})
        {
            ...
            env.fund (SFC (10000), alice, bob, carol, dan, erin, gw);
    ...
    void testEnforceNoSupplyFinanceChain (FeatureBitset features)
    {
        testcase ("Enforce No SupplyFinanceChain");
        ...
        {
            // No SupplyFinanceChain with an implied account step after an offer
            Env env {*this, features};
            auto const closeTime =
                fix1449Time () +
                    100 * env.closed ()->info ().closeTimeResolution;
            ...
            env.fund (SFC (10000), alice, noSupplyFinanceChain (bob), carol, dan, gw1, gw2);
            env.trust (USD1 (1000), alice, carol, dan);
            env(trust (bob, USD1 (1000), tfSetNoSupplyFinanceChain));
            env.trust (USD2 (1000), alice, carol, dan);
            env(trust (bob, USD2 (1000), tfSetNoSupplyFinanceChain));

            env (pay (gw1, dan, USD1 (50)));
            env (pay (gw1, bob, USD1 (50)));
            env (pay (gw2, bob, USD2 (50)));

            env (offer (dan, SFC (50), USD1 (50)));

            env (pay (alice, carol, USD2 (50)), path (~USD1, bob),
                sendmax (SFC (50)), txflags (tfNoSupplyFinanceChainDirect),
                ter(tecPATH_DRY));
        }
        {
            ...
            env.fund (SFC (10000), alice, bob, carol, dan, gw1, gw2);
            ...
            env.require (balance (alice, sfcMinusFee (env, 10000 - 50)));
    ...
    testInsufficientReserve (FeatureBitset features)
    {
        ...
        // No crossing:
        {
            ...
            env (offer (alice, sfcOffer, usdOffer),
        ...
        // Partial cross:
        {
            ...
            auto const sfcOffer2 = SFC (500);

            env.fund (r + f + sfcOffer, bob);
            env (offer (bob, usdOffer2, sfcOffer2),   ter(tesSUCCESS));
            env.fund (r + f, alice);
            env (trust (alice, usdOffer),             ter(tesSUCCESS));
            env (pay (gw, alice, usdOffer),           ter(tesSUCCESS));
            env (offer (alice, sfcOffer, usdOffer),   ter(tesSUCCESS));

            env.require (
                balance (alice, r - f + sfcOffer2),
                balance (alice, usdOffer2),
                owners (alice, 1),
                balance (bob, r + sfcOffer2),
        ...
        // buy SFC. If it fully crosses, we succeed.
        {
            Env env {*this, features};
            auto const closeTime =
                fix1449Time () +
            ...
            env.fund (r + f + sfcOffer, bob, carol);
            env (offer (bob, usdOffer2, sfcOffer2),    ter(tesSUCCESS));
            env (offer (carol, usdOffer, sfcOffer),    ter(tesSUCCESS));
            ...
            env.require (
                balance (alice, r - f + sfcOffer),
                balance (alice, USD(0)),
                owners (alice, 1),
                balance (bob, r + sfcOffer2),
                balance (bob, usdOffer2),
                owners (bob, 1),
                balance (carol, r + sfcOffer2),
    ...
    testFillModes (FeatureBitset features)
    {
        ...
        auto const startBalance = SFC (1000000);
        auto const gw = Account {"gateway"};
        auto const alice = Account {"alice"};
        auto const bob = Account {"bob"};
        auto const USD = gw["USD"];
            ...
            {features - fix1578, features | fix1578})
        {
            ...
            env (offer (bob, USD (500), SFC (500)), ter(tesSUCCESS));
            env.close();
            env.require (
                owners (bob, 2),
                offers (bob, 2));
            ...
            // Order that can't be filled but will remove bob's expired offer:
            {
                ...
                env (offer (alice, SFC (1000), USD (1000)),
            ...
            env (offer (alice, SFC (500), USD (500)),
                txflags (tfFillOrKill),              ter(tesSUCCESS));

            env.require (
                balance (alice, startBalance - (f * 3) + SFC (500)),
                balance (alice, USD (500)),
                owners (alice, 1),
                offers (alice, 0),
                balance (bob, startBalance - (f * 2) - SFC (500)),
                balance (bob, USD (500)),
                owners (bob, 1),
                offers (bob, 0));
        }
        ...
        // and add nothing on the books:
        {
            ...
            // Partially cross:
            env (offer (bob, USD (50), SFC (50)),           ter(tesSUCCESS));
            env (offer (alice, SFC (1000), USD (1000)),
                txflags (tfImmediateOrCancel),              ter(tesSUCCESS));

            env.require (
                balance (alice, startBalance - f - f - f + SFC (50)),
                balance (alice, USD (950)),
                owners (alice, 1),
                offers (alice, 0),
                balance (bob, startBalance - f - SFC (50)),
                balance (bob, USD (50)),
                owners (bob, 1),
                offers (bob, 0));
            ...
            // Fully cross:
            env (offer (bob, USD (50), SFC (50)),           ter(tesSUCCESS));
            env (offer (alice, SFC (50), USD (50)),
                txflags (tfImmediateOrCancel),              ter(tesSUCCESS));

            env.require (
                balance (alice, startBalance - f - f - f - f + SFC (100)),
                balance (alice, USD (900)),
                owners (alice, 1),
                offers (alice, 0),
                balance (bob, startBalance - f - f - SFC (100)),
                balance (bob, USD (100)),
                owners (bob, 1),
                offers (bob, 0));
        }
        ...
        // tfPassive -- place the offer without crossing it.
        {
            ...
            env (offer (alice, USD(1000), SFC(2000)));
            env.close();
            ...
            for (auto offerPtr : aliceOffers)
            {
                ...
                BEAST_EXPECT (offer[sfTakerGets] == SFC (2000));
            ...
            env (offer (bob, SFC(2000), USD(1000), tfPassive));
            env.close();
            ...
            for (auto offerPtr : bobOffers)
            {
                ...
                BEAST_EXPECT (offer[sfTakerPays] == SFC (2000));
            }

            // It should be possible for gw to cross both of those offers.
            env (offer (gw, SFC(2000), USD(1000)));
            env.close();
            env.require (offers (alice, 0));
            env.require (offers (gw, 0));
            env.require (offers (bob, 1));

            env (offer (gw, USD(1000), SFC(2000)));
            env.close();
        ...
        // tfPassive -- cross only offers of better quality.
        {
            ...
            env (pay (gw, "bob", USD(1000)));
            env (offer ("alice", USD(500), SFC(1001)));
            env.close();

            env (offer ("alice", USD(500), SFC(1000)));
            env.close();
            ...
            for (auto offerPtr : bobOffers)
            {
                ...
                BEAST_EXPECT (offer[sfTakerPays] == SFC (999));
            }
        }
    }
    ...
    testMalformed(FeatureBitset features)
    {
        ...
        env (offer (alice, USD (1000), SFC (1000)),
        ...
        {
            // Alice tries an SFC to SFC order:
            env (offer (alice, SFC (1000), SFC (1000)),   ter(temBAD_OFFER));
            env.require (
                owners (alice, 0),
                offers (alice, 0));
        ...
        {
            env (offer (alice, -USD (1000), SFC (1000)),  ter(temBAD_OFFER));
            env.require (
                owners (alice, 1),
                offers (alice, 0));
        ...
        // Use SFC as a currency code
        {
            auto const BAD = IOU(gw, badCurrency());

            env (offer (alice, SFC (1000), BAD (1000)),   ter(temBAD_CURRENCY));
            env.require (
                owners (alice, 1),
                offers (alice, 0));
        }
    ...
    testExpiration(FeatureBitset features)
    {
        ...
        env (offer (bob, usdOffer, sfcOffer),     ter(tesSUCCESS));
        env.require (
            balance (alice, startBalance - f - f - f),
            balance (alice, usdOffer),
            offers (alice, 0),
    ...
    testUnfundedCross(FeatureBitset features)
    {
        ...
        env (offer ("alice", usdOffer, sfcOffer),     ter(tecUNFUNDED_OFFER));
        env.require (
        ...
        env (offer ("bob", usdOffer, sfcOffer),       ter(tecUNFUNDED_OFFER));
        env.require (
        ...
        env.fund (reserve (env, 0) + f + SFC(1), "carol");
        env (offer ("carol", usdOffer, sfcOffer), ter(tecINSUF_RESERVE_OFFER));
        env.require (
            balance ("carol", reserve (env, 0) + SFC(1)),
            owners ("carol", 0));

        // Account has enough for the reserve plus one
        // offer, and the fee.
        env.fund (reserve (env, 1) + f, "dan");
        env (offer ("dan", usdOffer, sfcOffer),       ter(tesSUCCESS));
        env.require (
        ...
        env.fund (reserve (env, 1) + f + sfcOffer, "eve");
        env (offer ("eve", usdOffer, sfcOffer),       ter(tesSUCCESS));
        env.require (
            balance ("eve", reserve (env, 1) + sfcOffer),
            owners ("eve", 1));
    }
    ...
    testSelfCross(bool use_partner, FeatureBitset features)
    {
        ...
        env.fund (SFC (10000), gw);
        if (use_partner)
        {
            env.fund (SFC (10000), partner);
        ...
        // through SFC
        env (offer (account_to_test, BTC (250), SFC (1000)));
        env.require (offers(account_to_test, 1));

        // validate that the book now shows a BTC for SFC offer
        BEAST_EXPECT(isOffer(env, account_to_test, BTC(250), SFC(1000)));

        auto const secondLegSeq = env.seq(account_to_test);
        env (offer (account_to_test, SFC(1000), USD (50)));
        env.require (offers (account_to_test, 2));

        // validate that the book also shows a SFC for USD offer
        BEAST_EXPECT(isOffer(env, account_to_test, SFC(1000), USD(50)));
        ...
        // old and the new behavior.
        {
            ...
            for (auto const& offerPtr : acctOffers)
            {
                ...
                BEAST_EXPECT (offer[sfTakerPays] == SFC (0));
            }
        }
    ...
    testNegativeBalance(FeatureBitset features)
    {
        ...
        env (offer (alice, USD (50), SFC (150000)));
        ...
        // small_amount and transfer no SFC.  The new offer crossing transfers
        ...
        BEAST_EXPECT(jrr[jss::node][sfBalance.fieldName][jss::value] == "50");
        BEAST_EXPECT(env.balance (alice, sfcIssue()) ==
        ...
        BEAST_EXPECT(env.balance (bob, sfcIssue()) ==
            bob_initial_balance -
    ...
    void
    testOfferCrossWithSFC(bool reverse_order, FeatureBitset features)
    {
        testcase (std::string("Offer Crossing with SFC, ") +
        ...
        env.fund (SFC (10000), gw, alice, bob);
        ...
        if (reverse_order)
            env (offer (bob, USD (1), SFC (4000)));

        env (offer (alice, SFC (150000), USD (50)));
        ...
        BEAST_EXPECT(
            ...
            std::to_string(
                SFC (10000).value ().mantissa () -
                SFC (reverse_order ? 4000 : 3000).value ().mantissa () -
    ...
    testOfferCrossWithLimitOverride(FeatureBitset features)
    {
        ...
        env.fund (SFC (100000), gw, alice, bob);
        ...
        BEAST_EXPECT(
            jrr[jss::node][sfBalance.fieldName] ==
            std::to_string(
                SFC (100000).value ().mantissa () -
                SFC (3000).value ().mantissa () -
    ...
    testOfferAcceptThenCancel(FeatureBitset features)
    {
        ...
        env (offer (env.master, SFC (500), USD (100)));
        env.close ();
    ...
    testCurrencyConversionEntire(FeatureBitset features)
    {
        ...
        BEAST_EXPECT(
            jro[jss::node][jss::TakerGets] == SFC (500).value ().getText ());
        BEAST_EXPECT(
            jro[jss::node][jss::TakerPays] ==
            USD (100).value ().getJson (JsonOptions::none));

        env (pay (alice, alice, SFC (500)), sendmax (USD (100)));
    ...
    testCurrencyConversionInParts(FeatureBitset features)
    {
        ...
        env (pay (alice, alice, SFC (200)), sendmax (USD (100)));

        // The previous payment reduced the remaining offer amount by 200 SFC
        auto jro = ledgerEntryOffer (env, bob, bobOfferSeq);
        BEAST_EXPECT(
            jro[jss::node][jss::TakerGets] == SFC (300).value ().getText ());
        BEAST_EXPECT(
            jro[jss::node][jss::TakerPays] ==
        ...
        // alice now has 200 more SFC from the payment
        jrr = ledgerEntryRoot (env, alice);
        BEAST_EXPECT(
            jrr[jss::node][sfBalance.fieldName] ==
            std::to_string(
                SFC (10000).value ().mantissa () +
                SFC (200).value ().mantissa () -
                env.current ()->fees ().base * 2)
        );
        ...
        // Alice converts USD to SFC which should fail
        // due to PartialPayment.
        env (pay (alice, alice, SFC (600)), sendmax (USD (100)),
            ter(tecPATH_PARTIAL));

        // Alice converts USD to SFC, should succeed because
        ...
        // only 300 SFC should be have been payed since that's all
    ...
    void
    testCrossCurrencyStartSFC(FeatureBitset features)
    {
        testcase ("Cross Currency Payment: Start with SFC");
        ...
        env (offer (carol, SFC (500), USD (50)));

        env (pay (alice, bob, USD (25)), sendmax (SFC (333)));
        ...
        BEAST_EXPECT(
            jro[jss::node][jss::TakerPays] == SFC (250).value ().getText ());
    }

    void
    testCrossCurrencyEndSFC(FeatureBitset features)
    {
        testcase ("Cross Currency Payment: End with SFC");
        ...
        auto const carolOfferSeq = env.seq (carol);
        env (offer (carol, USD (50), SFC (500)));

        env (pay (alice, bob, SFC (250)), sendmax (USD (333)));
        ...
        BEAST_EXPECT(
            ...
            std::to_string(
                SFC (10000).value ().mantissa () +
                SFC (250).value ().mantissa ())
        );

        auto jro = ledgerEntryOffer (env, carol, carolOfferSeq);
        BEAST_EXPECT(
            jro[jss::node][jss::TakerGets] == SFC (250).value ().getText ());
        BEAST_EXPECT(
            jro[jss::node][jss::TakerPays] ==
            USD (25).value ().getJson (JsonOptions::none));
    }
    ...
    testCrossCurrencyBridged(FeatureBitset features)
    {
        ...
        env.fund (SFC (10000), gw1, gw2, alice, bob, carol, dan);
        ...
        env (offer (dan, SFC (500), EUR (50)));

        Json::Value jtp {Json::arrayValue};
        jtp[0u][0u][jss::currency] = "SFC";
        ...
        auto jro = ledgerEntryOffer (env, carol, carolOfferSeq);
        BEAST_EXPECT(
            jro[jss::node][jss::TakerGets] == SFC (200).value ().getText ());
        BEAST_EXPECT(
            jro[jss::node][jss::TakerPays] ==
        ...
        BEAST_EXPECT(
            jro[jss::node][jss::TakerPays] == SFC (200).value ().getText ());
    }
    ...
    testBridgedSecondLegDry(FeatureBitset features)
    {
        ...
        env.fund(SFC(100000000), alice, bob, carol, gw);
        ...
        env (offer (alice, EUR(2), SFC(1)));
        env (offer (alice, EUR(2), SFC(1)));

        env (offer (alice, SFC(1), USD(4)));
        env (offer (carol, SFC(1), USD(3)));
        env.close();

        // Bob offers to buy 10 USD for 10 EUR.
        //  1. He spends 2 EUR taking Alice's auto-bridged offers and
        //     gets 4 USD for that.
        //  2. He spends another 2 EUR taking Alice's last EUR->SFC offer and
        //     Carol's SFC-USD offer.  He gets 3 USD for that.
        // The key for this test is that Alice's SFC->USD leg goes dry before
        // Alice's EUR->SFC.  The SFC->USD leg is the second leg which showed
        // some sensitivity.
    ...
    testOfferFeesConsumeFunds(FeatureBitset features)
    {
        ...
        //  1 for payment          == 4
        auto const starting_sfc = SFC (100) +
            env.current ()->fees ().accountReserve (3) +
            env.current ()->fees ().base * 4;

        env.fund (starting_sfc, gw1, gw2, gw3, alice, bob);
        ...
        env (offer (bob, SFC (200), USD1 (200)));
        // Alice has 350 fees - a reserve of 50 = 250 reserve = 100 available.
        // Ask for more than available to prove reserve works.
        env (offer (alice, USD1 (200), SFC (200)));

        auto jrr = ledgerEntryState (env, alice, gw1, "USD");
        BEAST_EXPECT(jrr[jss::node][sfBalance.fieldName][jss::value] == "100");
        jrr = ledgerEntryRoot (env, alice);
        BEAST_EXPECT(
            jrr[jss::node][sfBalance.fieldName] == SFC (350).value ().getText ()
        );
    ...
    testSellFlagBasic(FeatureBitset features)
    {
        ...
        env.fund (starting_sfc, gw, alice, bob);
        ...
        env (offer (bob, SFC (200), USD (200)), json(jss::Flags, tfSell));
        // Alice has 350 + fees - a reserve of 50 = 250 reserve = 100 available.
        // Alice has 350 + fees - a reserve of 50 = 250 reserve = 100 available.
        // Ask for more than available to prove reserve works.
        env (offer (alice, USD (200), SFC (200)), json(jss::Flags, tfSell));

        auto jrr = ledgerEntryState (env, alice, gw, "USD");
        BEAST_EXPECT(jrr[jss::node][sfBalance.fieldName][jss::value] == "-100");
        jrr = ledgerEntryRoot (env, alice);
        BEAST_EXPECT(
            jrr[jss::node][sfBalance.fieldName] == SFC (250).value ().getText ()
        );
    ...
    testSellFlagExceedLimit(FeatureBitset features)
    {
        ...
        env (offer (bob, SFC (100), USD (200)));
        // Alice has 350 fees - a reserve of 50 = 250 reserve = 100 available.
        // Ask for more than available to prove reserve works.
        // Taker pays 100 USD for 100 SFC.
        // Selling SFC.
        // Will sell all 100 SFC and get more USD than asked for.
        env (offer (alice, USD (100), SFC (100)), json(jss::Flags, tfSell));
    ...
    testGatewayCrossCurrency(FeatureBitset features)
    {
        ...
        auto const starting_sfc = SFC (100.1) +
            env.current ()->fees ().accountReserve (1) +
            env.current ()->fees ().base * 2;
        ...
        if (wsc->version() == 2)
        {
            ...
            BEAST_EXPECT(jrr.isMember(jss::SupplyFinanceChainrpc) && jrr[jss::SupplyFinanceChainrpc] == "2.0");
    ...
    void testPartialCross (FeatureBitset features)
    {
        ...
        env.fund (SFC(10000000), gw);

        // The fee that's charged for transactions
        auto const f = env.current ()->fees ().base;

        // To keep things simple all offers are 1 : 1 for SFC : USD.
        enum preTrustType {noPreTrust, gwPreTrust, acctPreTrust};
        struct TestData
        {
            std::string account;       // Account operated on
            STAmount fundSfc;          // Account funded with
            int bookAmount;            // USD -> SFC offer on the books
            preTrustType preTrust;     // If true, pre-establish trust line
            int offerAmount;           // Account offers this much SFC -> USD
            TER tec;                   // Returned tec code
            STAmount spentSfc;         // Amount removed from fundSfc
        ...
        {
//acct                             fundSfc  bookAmt      preTrust  offerAmt                     tec       spentSfc    balanceUSD  offers  owners
...
{"flo",             reserve (env, 1) + 0*f,       1,   noPreTrust,     1000,             tesSUCCESS, SFC(   1) + f, USD(      1),      0,     1},
{"gay",             reserve (env, 1) + 1*f,    1000,   noPreTrust,     1000,             tesSUCCESS, SFC(  50) + f, USD(     50),      0,     1},
{"hye", SFC(1000)                    + 1*f,    1000,   noPreTrust,     1000,             tesSUCCESS, SFC( 800) + f, USD(    800),      0,     1},
{"ivy", SFC(   1) + reserve (env, 1) + 1*f,       1,   noPreTrust,     1000,             tesSUCCESS, SFC(   1) + f, USD(      1),      0,     1},
{"joy", SFC(   1) + reserve (env, 2) + 1*f,       1,   noPreTrust,     1000,             tesSUCCESS, SFC(   1) + f, USD(      1),      1,     2},
{"kim", SFC( 900) + reserve (env, 2) + 1*f,     999,   noPreTrust,     1000,             tesSUCCESS, SFC( 999) + f, USD(    999),      0,     1},
{"liz", SFC( 998) + reserve (env, 0) + 1*f,     999,   noPreTrust,     1000,             tesSUCCESS, SFC( 998) + f, USD(    998),      0,     1},
{"meg", SFC( 998) + reserve (env, 1) + 1*f,     999,   noPreTrust,     1000,             tesSUCCESS, SFC( 999) + f, USD(    999),      0,     1},
{"nia", SFC( 998) + reserve (env, 2) + 1*f,     999,   noPreTrust,     1000,             tesSUCCESS, SFC( 999) + f, USD(    999),      1,     2},
{"ova", SFC( 999) + reserve (env, 0) + 1*f,    1000,   noPreTrust,     1000,             tesSUCCESS, SFC( 999) + f, USD(    999),      0,     1},
{"pam", SFC( 999) + reserve (env, 1) + 1*f,    1000,   noPreTrust,     1000,             tesSUCCESS, SFC(1000) + f, USD(   1000),      0,     1},
{"rae", SFC( 999) + reserve (env, 2) + 1*f,    1000,   noPreTrust,     1000,             tesSUCCESS, SFC(1000) + f, USD(   1000),      0,     1},
{"sue", SFC(1000) + reserve (env, 2) + 1*f,       0,   noPreTrust,     1000,             tesSUCCESS,             f, USD(      0),      1,     1},
...
{"eli", SFC(  20) + reserve (env, 0) + 1*f,    1000,   gwPreTrust,     1000,             tesSUCCESS, SFC(20) + 1*f, USD(     20),      0,     0},
{"fyn",             reserve (env, 1) + 0*f,       0,   gwPreTrust,     1000,             tesSUCCESS,             f, USD(      0),      1,     1},
{"gar",             reserve (env, 1) + 0*f,       1,   gwPreTrust,     1000,             tesSUCCESS, SFC(   1) + f, USD(      1),      1,     1},
{"hal",             reserve (env, 1) + 1*f,       1,   gwPreTrust,     1000,             tesSUCCESS, SFC(   1) + f, USD(      1),      1,     1},
...
{"ted", SFC(  20) + reserve (env, 1) + 2*f,    1000, acctPreTrust,     1000,             tesSUCCESS, SFC(20) + 2*f, USD(     20),      0,     1},
{"uli",             reserve (env, 2) + 0*f,       0, acctPreTrust,     1000, tecINSUF_RESERVE_OFFER,           2*f, USD(      0),      0,     1},
{"vic",             reserve (env, 2) + 0*f,       1, acctPreTrust,     1000,             tesSUCCESS, SFC( 1) + 2*f, USD(      1),      0,     1},
{"wes",             reserve (env, 2) + 1*f,       0, acctPreTrust,     1000,             tesSUCCESS,           2*f, USD(      0),      1,     2},
{"xan",             reserve (env, 2) + 1*f,       1, acctPreTrust,     1000,             tesSUCCESS, SFC( 1) + 2*f, USD(      1),      1,     2},
};

        for (auto const& t : tests)
        {
            auto const acct = Account(t.account);
            env.fund (t.fundSfc, acct);
            env.close();
            ...
            if (book)
                env (offer (gw, SFC (book), USD (book)));
            env.close();
            ...
            // to be enough SFC reserve for acct to create the trust line.
            ...
            env (offer (acct, USD (acctOffer), SFC (acctOffer)), ter (t.tec));
            env.close();
            std::uint32_t const acctOfferSeq = env.seq (acct) - 1;

            BEAST_EXPECT (env.balance (acct, USD.issue()) == t.balanceUsd);
            BEAST_EXPECT (
                env.balance (acct, sfcIssue()) == t.fundSfc - t.spentSfc);
            ...
            if (acctOffers.size() && t.offers)
            {
                ...
                BEAST_EXPECT (acctOffer[sfTakerGets] == SFC (leftover));
    ...
    void
    testSFCDirectCross (FeatureBitset features)
    {
        testcase ("SFC Direct Crossing");
        ...
        // The scenario:
        //   o alice has USD but wants SFC.
        //   o bob has SFC but wants USD.
        auto const alicesSFC = env.balance (alice);
        auto const bobsSFC = env.balance (bob);

        env (offer (alice, sfcOffer, usdOffer));
        env.close();
        env (offer (bob, usdOffer, sfcOffer));

        env.close();
        env.require (
            balance (alice, USD(0)),
            balance (bob, usdOffer),
            balance (alice, alicesSFC + sfcOffer - fee),
            balance (bob,   bobsSFC   - sfcOffer - fee),
            offers (alice, 0),
            offers (bob, 0));
        ...
        env (offer (alice, USD(999), SFC(999)));
        env (offer (bob, sfcOffer, usdOffer));
        ...
        verifyDefaultTrustline (env, bob, USD(1));
        {
            ...
            BEAST_EXPECT (bobsOffer[sfTakerPays] == SFC (1));
        }
   }
    ...
    testBridgedCross (FeatureBitset features)
    {
        ...
        env.fund (SFC(1000000), gw, alice, bob, carol);
        env.close();
        ...
        env (offer (alice, SFC(1000), usdOffer));
        env (offer (bob, eurOffer, SFC(1000)));
        auto const bobSfcBalance = env.balance (bob);
        env.close();
        ...
        env.require (
            ...
            balance (bob, bobSfcBalance - SFC(400)),
            offers (carol, 0));
        ...
        verifyDefaultTrustline (env, carol, USD(400));
        {
            ...
            BEAST_EXPECT (alicesOffer[sfTakerPays] == SFC (600));
        }
        {
            auto const bobsOffers = offersOnAccount (env, bob);
            BEAST_EXPECT (bobsOffers.size() == 1);
            ...
            BEAST_EXPECT (bobsOffer[sfLedgerEntryType] == ltOFFER);
            BEAST_EXPECT (bobsOffer[sfTakerGets] == SFC (600));
        ...
        env.require (
            ...
            balance (bob, bobSfcBalance - SFC(1000)),
            offers (bob, 0),
            offers (carol, 0));
        ...
        if (alicesOffers.size() != 0)
        {
            ...
            BEAST_EXPECT (alicesOffer[sfTakerPays] == SFC (0));
        }
    }
    ...
    testSellOffer (FeatureBitset features)
    {
        ...
        {
            std::string account;       // Account operated on
            STAmount fundSfc;          // SFC acct funded with
            ...
            TestData (
                ...
                STAmount const& fundSfc_,       // SFC acct funded with
                ...
                STAmount const& spentSfc_,      // Amount removed from fundSfc
            ...
            TestData (
                ...
                int offers_,                    // Offers on acct
                int owners_)                    // Owners on acct
                : TestData (std::move(account_), fundSfc_, fundUSD_, gwGets_,
                  gwPays_, acctGets_, acctPays_, tec_, spentSfc_, finalUsd_,
        ...
        {
// acct pays SFC
//acct                           fundSfc  fundUSD   gwGets   gwPays  acctGets  acctPays                      tec         spentSfc  finalUSD  offers  owners  takerGets  takerPays
{"ann", SFC(10) + reserve (env, 0) + 1*f, USD( 0), SFC(10), USD( 5),  USD(10),  SFC(10), tecINSUF_RESERVE_OFFER, SFC( 0) + (1*f),  USD( 0),      0,     0},
{"bev", SFC(10) + reserve (env, 1) + 1*f, USD( 0), SFC(10), USD( 5),  USD(10),  SFC(10),             tesSUCCESS, SFC( 0) + (1*f),  USD( 0),      1,     1,     SFC(10),  USD(10)},
{"cam", SFC(10) + reserve (env, 0) + 1*f, USD( 0), SFC(10), USD(10),  USD(10),  SFC(10),             tesSUCCESS, SFC(10) + (1*f),  USD(10),      0,     1},
{"deb", SFC(10) + reserve (env, 0) + 1*f, USD( 0), SFC(10), USD(20),  USD(10),  SFC(10),             tesSUCCESS, SFC(10) + (1*f),  USD(20),      0,     1},
{"eve", SFC(10) + reserve (env, 0) + 1*f, USD( 0), SFC(10), USD(20),  USD( 5),  SFC( 5),             tesSUCCESS, SFC( 5) + (1*f),  USD(10),      0,     1},
{"flo", SFC(10) + reserve (env, 0) + 1*f, USD( 0), SFC(10), USD(20),  USD(20),  SFC(20),             tesSUCCESS, SFC(10) + (1*f),  USD(20),      0,     1},
{"gay", SFC(20) + reserve (env, 1) + 1*f, USD( 0), SFC(10), USD(20),  USD(20),  SFC(20),             tesSUCCESS, SFC(10) + (1*f),  USD(20),      0,     1},
{"hye", SFC(20) + reserve (env, 2) + 1*f, USD( 0), SFC(10), USD(20),  USD(20),  SFC(20),             tesSUCCESS, SFC(10) + (1*f),  USD(20),      1,     2,     SFC(10),  USD(10)},
// acct pays USD
{"meg",           reserve (env, 1) + 2*f, USD(10), USD(10), SFC( 5),  SFC(10),  USD(10), tecINSUF_RESERVE_OFFER, SFC(  0) + (2*f),  USD(10),      0,     1},
{"nia",           reserve (env, 2) + 2*f, USD(10), USD(10), SFC( 5),  SFC(10),  USD(10),             tesSUCCESS, SFC(  0) + (2*f),  USD(10),      1,     2,     USD(10),  SFC(10)},
{"ova",           reserve (env, 1) + 2*f, USD(10), USD(10), SFC(10),  SFC(10),  USD(10),             tesSUCCESS, SFC(-10) + (2*f),  USD( 0),      0,     1},
{"pam",           reserve (env, 1) + 2*f, USD(10), USD(10), SFC(20),  SFC(10),  USD(10),             tesSUCCESS, SFC(-20) + (2*f),  USD( 0),      0,     1},
{"qui",           reserve (env, 1) + 2*f, USD(10), USD(20), SFC(40),  SFC(10),  USD(10),             tesSUCCESS, SFC(-20) + (2*f),  USD( 0),      0,     1},
{"rae",           reserve (env, 2) + 2*f, USD(10), USD( 5), SFC( 5),  SFC(10),  USD(10),             tesSUCCESS, SFC( -5) + (2*f),  USD( 5),      1,     2,     USD( 5),  SFC( 5)},
{"sue",           reserve (env, 2) + 2*f, USD(10), USD( 5), SFC(10),  SFC(10),  USD(10),             tesSUCCESS, SFC(-10) + (2*f),  USD( 5),      1,     2,     USD( 5),  SFC( 5)},
};
    ...
    testSellWithFillOrKill (FeatureBitset features)
    {
        ...
        }
        {
            ...
            env (offer(alice, USD(20), SFC(2000), tfSell | tfFillOrKill));
            env.close();
        ...
        }
        {
            ...
            env (offer(bob, SFC(2000), USD(20)));
            env.close();
            env (offer(alice, USD(10), SFC(1500), tfSell | tfFillOrKill));
            env.close();
        ...
        }
        {
            ...
            // We're using bob's left-over offer for SFC(500), USD(5)
            env (offer(alice, USD(1), SFC(501),
                tfSell | tfFillOrKill), ter (killedCode));
            env.close();
            env.require (balance (alice, USD(35)));
            env.require (offers (alice, 0));
        ...
        }
        {
            ...
            // We're using bob's left-over offer for SFC(500), USD(5)
            env (offer(alice, USD(1), SFC(500), tfSell | tfFillOrKill));
            env.close();
    ...
    testTransferRateOffer (FeatureBitset features)
    {
        ...
        env(rate(gw1, 1.25));
        {
            ...
            auto const bob = Account("bob");
            env.fund (SFC(100) + reserve(env, 2) + (fee*2), ann, bob);
            env.close();
            ...
            // bob offers to sell USD(100) for SFC.  alice takes bob's offer.
            ...
            env (offer (bob, SFC(1), USD(100)));
            env.close();

            env (offer (ann, USD(100), SFC(1)));
            env.close();

            env.require (balance (ann, USD(100)));
            env.require (balance (ann, SFC( 99) + reserve(env, 2)));
            env.require (offers (ann, 0));

            env.require (balance (bob, USD(  0)));
            env.require (balance (bob, SFC(101) + reserve(env, 2)));
            env.require (offers (bob, 0));
        }
        {
            // Reverse the order, so the offer in the books is to sell SFC
            // in return for USD.  Gateway rate should still apply identically.
            auto const che = Account("che");
            auto const deb = Account("deb");
            env.fund (SFC(100) + reserve(env, 2) + (fee*2), che, deb);
            env.close();
            ...
            env (offer (che, USD(100), SFC(1)));
            env.close();

            env (offer (deb, SFC(1), USD(100)));
            env.close();

            env.require (balance (che, USD(100)));
            env.require (balance (che, SFC( 99) + reserve(env, 2)));
            env.require (offers (che, 0));

            env.require (balance (deb, USD(  0)));
            env.require (balance (deb, SFC(101) + reserve(env, 2)));
        ...
        }
        {
            ...
            env.fund (SFC(20000) + fee*2, eve, fyn);
            env.close();
            ...
            env (offer (eve, USD(10), SFC(4000)));
            env.close();
            std::uint32_t const eveOfferSeq = env.seq (eve) - 1;

            env (offer (fyn, SFC(2000), USD(5)));
            env.close();

            env.require (balance (eve, USD(105)));
            env.require (balance (eve, SFC(18000)));
            ...
            if (evesOffers.size() != 0)
            {
                ...
                BEAST_EXPECT (evesOffer[sfTakerGets] == SFC (2000));
            ...
            env.require (balance (fyn, SFC(22000)));
        ...
        env.fund (SFC(100000), gw2);
        env.close();

        env(rate(gw2, 1.5));
        {
            // Remove SFC from the equation.  Give the two currencies two
        ...
        }
        {
            ...
            env.fund (SFC(2) + reserve(env, 3) + (fee*3), ova, pat, qae);
            env.close();

            //   o ova has USD but wants XPR.
            //   o pat has SFC but wants EUR.
            ...
            env (offer (ova, SFC(2), USD(100)));
            env (offer (pat, EUR(100), SFC(2)));
            env.close();
            ...
            env.require (balance (ova, SFC(4) + reserve(env, 3)));
    ...
    testSelfCrossOffer1 (FeatureBitset features)
    {
        ...
        for (auto const& offerPtr : gwOffers)
        {
            auto const& offer = *offerPtr;
            BEAST_EXPECT (offer[sfLedgerEntryType] == ltOFFER);
            BEAST_EXPECT (offer[sfTakerGets] == SFC (600));
        ...
        for (auto const& offerPtr : gwOffers)
        {
            ...
            BEAST_EXPECT (offer[sfTakerPays] == SFC (1000));
        }
    }
    ...
    testSelfCrossOffer2 (FeatureBitset features)
    {
        ...
        TestData const tests[]
        {
// acct               fundSFC    fundUSD    fundEUR           firstOfferTec          secondOfferTec
...
{"flo", reserve(env, 3) +   0, USD(1000), EUR(1000), tecINSUF_RESERVE_OFFER, tecINSUF_RESERVE_OFFER},
    ...
    testSelfIssueOffer (FeatureBitset features)
    {
        ...
        env.fund(SFC(50000) + f, alice, bob);
        env.close();

        env(offer(alice, USD(5000), SFC(50000)));
        env.close();

        // This offer should take alice's offer up to Alice's reserve.
        env(offer(bob, SFC(50000), USD(5000)));
        env.close();

        // alice's offer should have been removed, since she's down to her
        // SFC reserve.
        env.require (balance (alice, SFC(250)));
        ...
        for (auto const& offerPtr : bobOffers)
        {
            ...
            BEAST_EXPECT(offer[sfTakerPays] == SFC (250));
        }
    }
    ...
    void testSelfCrossLowQualityOffer (FeatureBitset features)
    {
        ...
        // for more SFC than the offering account held.  This unit test
    ...
    void testTinyOffer (FeatureBitset features)
    {
        ...
        auto const CNY = gw["CNY"];
        auto const fee = env.current()->fees().base;
        auto const startSfcBalance = drops (400000000000) + (fee * 2);

        env.fund (startSfcBalance, gw, alice, bob);
        env.close();
        ...
        env.require (balance (alice, startSfcBalance - fee - drops(1)));
        env.require (balance (bob, bobsCnyStartBalance - alicesCnyOffer));
        env.require (balance (bob, startSfcBalance - (fee * 2) + drops(1)));
    }
    ...
    void testSelfPayXferFeeOffer (FeatureBitset features)
    {
        ...
        //
        //  2. alice makes an offer to buy SFC and sell USD.
        //  3. bob makes an offer to buy BTC and sell SFC.
        //
        ...
        //  o bob' offer which takes alice's BTC and sells SFC to...
        //  o alice's first offer which takes bob's SFC and sells USD to...
        ...
        auto const BTC = gw["BTC"];
        auto const USD = gw["USD"];
        auto const startSfcBalance = SFC (4000000);

        env.fund (startSfcBalance, gw);
        env.close();
        ...
        struct Actor
        {
            ...
            PrettyAmount sfc;  // final expected after crossing
...
// { 0, 0, 0, BTC( 5), { {"flo", 0, drops(3999999999950), BTC( 5.0), USD(2000)}                                                      } }  // no xfer fee
        ...
        for (auto const& t : tests)
        {
            ...
            {
                env.fund (SFC (4000000), actor.acct);
                env.close();
            ...
            env (offer (leg0, BTC(10),     SFC(100000), tfPassive));
            env.close();
            std::uint32_t const leg0OfferSeq = env.seq (leg0) - 1;

            env (offer (leg1, SFC(100000), USD(1000),   tfPassive));
            env.close();
            ...
            for (auto const& actor : t.actors)
            {
                ...
                env.require (balance (actor.acct, actor.sfc));
...
{ 0, 0, 0, BTC( 5), { {"hye", 2, drops(3999999999950), BTC(5), USD(2000)}                                                      } }  // no xfer fee
    ...
    void testRequireAuth (FeatureBitset features)
    {
        ...
        env.fund (SFC(400000), gw, alice, bob);
        env.close();
        ...
        env (offer (alice, gwUSD(40), SFC(4000)));
        env.close();
        ...
        env (offer (bob, SFC(4000), gwUSD(40)));
        env.close();
    ...
    void testMissingAuth (FeatureBitset features)
    {
        ...
        // untouched.
        env (offer (alice, gwUSD(40), SFC(4000)), ter(tecNO_LINE));
        env.close();
        ...
        env (offer (alice, gwUSD(40), SFC(4000)), ter(tecNO_AUTH));
        env.close();
    ...
    {
        testcase("SupplyFinanceChainConnect Smoketest payment flow");
        using namespace jtx;
        ...
        // This test mimics the payment flow used in the SupplyFinanceChain Connect
        ...
        env.fund (SFC(100000), hotUS, coldUS, hotEU, coldEU, mm);
        env.close();

        // Cold wallets require trust but will SupplyFinanceChain by default
        for (auto const& cold : {coldUS, coldEU})
        {
            env(fset (cold, asfRequireAuth));
            env(fset (cold, asfDefaultSupplyFinanceChain));
        }
        env.close();

        // Each hot wallet trusts the related cold wallet for a large amount
        env (trust(hotUS, USD(10000000)), txflags (tfSetNoSupplyFinanceChain));
        env (trust(hotEU, EUR(10000000)), txflags (tfSetNoSupplyFinanceChain));
        // Market maker trusts both cold wallets for a large amount
        env (trust(mm, USD(10000000)), txflags (tfSetNoSupplyFinanceChain));
        env (trust(mm, EUR(10000000)), txflags (tfSetNoSupplyFinanceChain));
        env.close();
        ...
        // There should be a path available from hot US to cold EUR
        {
            ...
            Json::Value const jrr {env.rpc(
                "json", "SupplyFinanceChain_path_find", to_string(jvParams))[jss::result]};
    ...
    void testSelfAuth (FeatureBitset features)
    {
        ...
        env.fund (SFC(400000), gw, alice);
        env.close();

        // Test that gw can create an offer to buy gw's currency.
        env (offer (gw, gwUSD(40), SFC(4000)));
        env.close();
        std::uint32_t const gwOfferSeq = env.seq (gw) - 1;
        env.require (offers (gw, 1));
        ...
        env (offer (alice, SFC(4000), gwUSD(40)));
        env.close();
    ...
    void testAll(FeatureBitset features)
    {
        ...
        testSFCTinyPayment(features);
        testEnforceNoSupplyFinanceChain(features);
        ...
        testOfferCrossWithSFC(true, features);
        testOfferCrossWithSFC(false, features);
        ...
        testCrossCurrencyStartSFC(features);
        testCrossCurrencyEndSFC(features);
...
BEAST_DEFINE_TESTSUITE_PRIO (Offer, tx, SupplyFinanceChain, 4);
BEAST_DEFINE_TESTSUITE_MANUAL_PRIO (Offer_manual, tx, SupplyFinanceChain, 20);
