...
struct PayChan_test : public beast::unit_test::suite
{
    ...
    {
        Serializer msg;
        serializePayChanAuthorization (msg, channel, authAmt.sfc ());
    ...
    testSimple ()
    {
        ...
        env (create (alice, bob, SFC (1000), settleDelay, pk));
        auto const chan = channel (*env.current (), alice, bob);
        BEAST_EXPECT (channelBalance (*env.current (), chan) == SFC (0));
        BEAST_EXPECT (channelAmount (*env.current (), chan) == SFC (1000));

        {
            auto const preAlice = env.balance (alice);
            env (fund (alice, chan, SFC (1000)));
            auto const feeDrops = env.current ()->fees ().base;
            BEAST_EXPECT (env.balance (alice) == preAlice - SFC (1000) - feeDrops);
        }

        auto chanBal = channelBalance (*env.current (), chan);
        auto chanAmt = channelAmount (*env.current (), chan);
        BEAST_EXPECT (chanBal == SFC (0));
        BEAST_EXPECT (chanAmt == SFC (2000));

        {
            // bad amounts (non-sfc, negative amounts)
        ...
        // invalid account
        env (create (alice, "noAccount", SFC (1000), settleDelay, pk),
            ter (tecNO_DST));
        // can't create channel to the same account
        env (create (alice, alice, SFC (1000), settleDelay, pk),
             ter (temDST_IS_SRC));
        // invalid channel
        env (fund (alice, channel (*env.current (), alice, "noAccount"), SFC (1000)),
            ter (tecNO_ENTRY));
        // not enough funds
        env (create (alice, bob, SFC (10000), settleDelay, pk),
            ter (tecUNFUNDED));

        {
            // No signature claim with bad amounts (negative and non-sfc)
            auto const iou = USDA (100).value ();
            auto const negSFC = SFC (-100).value ();
            auto const posSFC = SFC (100).value ();
            env (claim (alice, chan, iou, iou), ter (temBAD_AMOUNT));
            env (claim (alice, chan, posSFC, iou), ter (temBAD_AMOUNT));
            env (claim (alice, chan, iou, posSFC), ter (temBAD_AMOUNT));
            env (claim (alice, chan, negSFC, negSFC), ter (temBAD_AMOUNT));
            env (claim (alice, chan, posSFC, negSFC), ter (temBAD_AMOUNT));
            env (claim (alice, chan, negSFC, posSFC), ter (temBAD_AMOUNT));
        }
        {
            // No signature claim more than authorized
            auto const delta = SFC (500);
            auto const reqBal = chanBal + delta;
            auto const authAmt = reqBal + SFC (-100);
        ...
        }
        {
            ...
            auto const preBob = env.balance (bob);
            STAmount const authAmt = chanBal + SFC (500);
        ...
        env (fund (bob, chan, SFC (1000)), ter (tecNO_PERMISSION));
        BEAST_EXPECT (channelBalance (*env.current (), chan) == chanBal);
        BEAST_EXPECT (channelAmount (*env.current (), chan) == chanAmt);

        {
            // Wrong signing key
            auto const sig =
                signClaimAuth (bob.pk (), bob.sk (), chan, SFC (1500));
            env (claim (bob, chan, SFC (1500).value (), SFC (1500).value (),
                     Slice (sig), bob.pk ()),
                ter (temBAD_SIGNER));
            BEAST_EXPECT (channelBalance (*env.current (), chan) == chanBal);
            BEAST_EXPECT (channelAmount (*env.current (), chan) == chanAmt);
    ...
    testCancelAfter ()
    {
        ...
        auto const carol = Account ("carol");
        {
            ...
            auto const channelFunds = SFC (1000);
            env (create (
    ...
    testExpiration ()
    {
        ...
        env (fund (alice, chan, SFC (1),
            ...
            ter (temBAD_EXPIRATION));
        BEAST_EXPECT (!channelExpiration (*env.current (), chan));
        env (fund (alice, chan, SFC (1), NetClock::time_point{minExpiration}));
        env.close (minExpiration);
        // Try to extend the expiration after the expiration has already passed
        env (fund (
            alice, chan, SFC (1), NetClock::time_point{minExpiration + 1000s}));
        BEAST_EXPECT (!channelExists (*env.current (), chan));
    }
    ...
    testDisallowSFC ()
    {
        ...
        auto const bob = Account ("bob");
        {
            // Create a channel where dst disallows SFC
            Env env (*this, supported_amendments() - featureDepositAuth);
            env.fund (SFC (10000), alice, bob);
            env (fset (bob, asfDisallowSFC));
            env (create (alice, bob, SFC (1000), 3600s, alice.pk()),
                ter (tecNO_TARGET));
            auto const chan = channel (*env.current (), alice, bob);
            BEAST_EXPECT (!channelExists (*env.current (), chan));
        }
        {
            // Create a channel where dst disallows SFC.  Ignore that flag,
        ...
        {
            // Claim to a channel where dst disallows SFC
            // (channel is created before disallow sfc is set)
            Env env (*this, supported_amendments() - featureDepositAuth);
            env.fund (SFC (10000), alice, bob);
            env (create (alice, bob, SFC (1000), 3600s, alice.pk()));
            auto const chan = channel (*env.current (), alice, bob);
            ...
            env (fset (bob, asfDisallowSFC));
            auto const reqBal = SFC (500).value();
            env (claim (alice, chan, reqBal, reqBal), ter(tecNO_TARGET));
        }
        {
            // Claim to a channel where dst disallows SFC (channel is
            // created before disallow sfc is set).  Ignore that flag
    ...
    testDepositAuth ()
    {
        ...
        auto USDA = alice["USD"];
        {
            ...
            env (claim (alice, chan,
                SFC (500).value(), SFC (500).value()), ter (tecNO_PERMISSION));
            env.close();
            ...
            auto const preBob = env.balance (bob);
            {
                auto const delta = SFC (500).value();
            ...
            }
            {
                ...
                auto const delta = SFC (600).value();
                auto const sig = signClaimAuth (pk, alice.sk (), chan, delta);
            ...
            }
            {
                ...
                auto const delta = SFC (800).value();
                ...
                env.close();
                BEAST_EXPECT (
                    env.balance (bob) == preBob + SFC (800) - (5 * baseFee));
            }
        }
    }
    ...
    testMalformedPK ()
    {
        ...
        auto jv = create (alice, bob, SFC (1000), settleDelay, pk);
...
BEAST_DEFINE_TESTSUITE (PayChan, app, SupplyFinanceChain);
}  // test
}  // SupplyFinanceChain
