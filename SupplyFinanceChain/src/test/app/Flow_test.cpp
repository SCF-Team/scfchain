...
bool getNoSupplyFinanceChainFlag (jtx::Env const& env,
    ...
    Currency const& cur)
{
    ...
    {
        auto const flag = (src.id() > dst.id()) ? lsfHighNoSupplyFinanceChain : lsfLowNoSupplyFinanceChain;
...
sfcMinusFee (jtx::Env const& env, std::int64_t sfcAmount)
{
    ...
    return drops (
        dropsPerSFC<std::int64_t>::value * sfcAmount - feeDrops);
};
...
struct Flow_test : public beast::unit_test::suite
{
    void testDirectStep (FeatureBitset features)
    {
        ...
        }
        {
            ...
            env.fund (SFC (10000), alice, bob);
            env (pay (alice, bob, SFC (100)));
            env.require (balance (bob, SFC (10000 + 100)));
            env.require (balance (alice, sfcMinusFee (env, 10000 - 100)));
        }
        {
            // Partial payments
            Env env (*this, features);
        ...
        }
        {
            ...
            env.fund (SFC (10000), alice, bob, carol, dan);
        ...
        }
        {
            ...
            env (pay (alice, dan, USDC (5)), path (bob, carol),
                sendmax (USDA (6)), txflags (tfNoSupplyFinanceChainDirect));
        ...
        }
        {
            ...
            env.fund (SFC (10000), alice, bob, carol, dan, erin);
            ...
            env (pay (alice, erin, USDD (5)), path (carol, dan),
                path (bob, dan), txflags (tfNoSupplyFinanceChainDirect));
        ...
        for (auto bobDanQIn : {80, 100, 120})
            for (auto bobAliceQOut : {80, 100, 120})
            {
                ...
                env(pay(dan, carol, USDA(10)),
                    path(bob), sendmax(USDD(100)), txflags(tfNoSupplyFinanceChainDirect));
    ...
    void testBookStep (FeatureBitset features)
    {
        ...
        }
        {
            ...
            env (offer (bob, BTC (50), SFC (50)));
            env (offer (bob, SFC (50), USD (50)));

            env (pay (alice, carol, USD (50)), path (~SFC, ~USD),
                sendmax (BTC (50)));
            ...
            env.require (balance (bob, USD (0)));
            env.require (balance (carol, USD (50)));
            BEAST_EXPECT(!isOffer (env, bob, SFC (50), USD (50)));
            BEAST_EXPECT(!isOffer (env, bob, BTC (50), SFC (50)));
        }
        {
            // simple SFC -> USD through offer and sendmax
            Env env (*this, features);
            ...
            env.require (balance (bob, sfcMinusFee (env, 10000 + 50)));
            env.require (balance (bob, USD (0)));
            env.require (balance (carol, USD (50)));
            BEAST_EXPECT(!isOffer (env, bob, SFC (50), USD (50)));
        }
        {
            // simple USD -> SFC through offer and sendmax
            Env env (*this, features);
            ...
            env (offer (bob, USD (50), SFC (50)));

            env (pay (alice, carol, SFC (50)), path (~SFC),
                 sendmax (USD (50)));

            env.require (balance (alice, USD (0)));
            env.require (balance (bob, sfcMinusFee (env, 10000 - 50)));
            env.require (balance (bob, USD (50)));
            env.require (balance (carol, SFC (10000 + 50)));
            BEAST_EXPECT(!isOffer (env, bob, USD (50), SFC (50)));
        }
        {
        ...
        }
        {
            ...
            env (pay (alice, carol, EUR (1)), path (~SFC, ~EUR),
                sendmax (USD (0.4)), txflags (tfNoSupplyFinanceChainDirect|tfPartialPayment));
    ...
    void testTransferRate (FeatureBitset features)
    {
        ...
        }
        {
            ...
            env (pay (alice, carol, USD (50)), path (~USD), sendmax (SFC (50)));
            env.require (
                balance (alice, sfcMinusFee (env, 10000 - 50)),
...

        {
            ...
            env (pay (alice, carol, EUR (40)), path (~USD, ~EUR), sendmax (SFC (40)));
            env.require (
                balance (alice,  sfcMinusFee (env, 10000 - 40)),
                balance (bob, USD (40)),
                balance (bob, EUR (0)),
        ...
        }
        {
            ...
            env (pay (alice, carol, EUR (50)), path (bob, gw, ~EUR),
                sendmax (USDA (60)), txflags (tfNoSupplyFinanceChainDirect));
            env.require (
                balance (bob, USD (-10)),
                balance (bob, USDA (60)),
                balance (dan, USD (50)),
...

        {
            ...
            env(rate(gw, 1.25));
            env.trust (USD (1000), alice, bob);
            env (offer (gw, SFC (100), USD (100)));
            env (pay (alice, bob, USD (100)),
                 sendmax (SFC (100)));
            env.require (
                balance (alice, sfcMinusFee(env, 10000-100)),
                balance (bob, USD (100)));
        }
        if (!features[featureOwnerPaysFee])
        {
            ...
            env(rate(gw, 1.25));
            env.trust (USD (1000), alice, bob);
            env (offer (gw, SFC (125), USD (125)));
            env (pay (alice, bob, USD (100)),
                 sendmax (SFC (200)));
            env.require (
                balance (alice, sfcMinusFee(env, 10000-125)),
                balance (bob, USD (100)));
        }
    }
    ...
    testFalseDry(FeatureBitset features)
    {
        ...
        env.fund (SFC(10000), alice, carol, gw);
        ...
        // Bob has _just_ slightly less than 50 sfc available
        // If his owner count changes, he will have more liquidity.
        // This is one error case to test (when Flow is used).
        // Computing the incomming sfc to the SFC/USD offer will require two
        // recursive calls to the EUR/SFC offer. The second call will return
        // tecPATH_DRY, but the entire path should not be marked as dry. This
        // is the second error case to test (when flowV1 is used).
        env (offer (bob, EUR (50), SFC (50)));
        env (offer (bob, SFC (50), USD (50)));

        env (pay (alice, carol, USD (1000000)), path (~SFC, ~USD),
            sendmax (EUR (500)),
    ...
    testLimitQuality ()
    {
        ...
        for (auto const& d : {-100 * timeDelta, +100 * timeDelta})
        {
            ...
            env (offer (bob, SFC (50), USD (50)));
            env (offer (bob, SFC (100), USD (50)));

            TER const expectedResult = closeTime < fix1141Time ()
                ? TER {tecPATH_DRY}
                : TER {tesSUCCESS};
            env (pay (alice, carol, USD (100)), path (~USD), sendmax (SFC (100)),
                txflags (tfNoSupplyFinanceChainDirect | tfPartialPayment | tfLimitQuality),
                ter (expectedResult));
    ...
    static SFCAmount reserve(jtx::Env& env, std::uint32_t count)
    {
    ...
    void testSelfFundedSFCEndpoint (bool consumeOffer, FeatureBitset features)
    {
        // Test that the deferred credit table is not bypassed for
        // SFCEndpointSteps. If the account in the first step is sending SFC and
        // that account also owns an offer that receives SFC, it should not be
        // possible for that step to use the SFC received in the offer as part
        // of the payment.
        testcase("Self funded SFCEndpoint");
        ...
        env(offer(alice, SFC(50000), USD(10)));
        ...
        env(pay(alice, alice, toSend), path(~USD), sendmax(SFC(20000)),
            txflags(tfPartialPayment | tfNoSupplyFinanceChainDirect));
    }
    ...
    void testUnfundedOffer (bool withFix, FeatureBitset features)
    {
        ...
        using namespace jtx;
        {
            ...
            env.fund(SFC(100000), alice, bob, gw);
            ...
            env(pay(alice, bob, tinyAmt1), path(~USD),
                sendmax(drops(9000000000)), txflags(tfNoSupplyFinanceChainDirect));

            if (withFix)
                BEAST_EXPECT(!isOffer(env, gw, SFC(0), USD(0)));
            else
                BEAST_EXPECT(isOffer(env, gw, SFC(0), USD(0)));
        }
        {
            ...
            env(pay(alice, bob, drops(9000000000)), path(~SFC),
                sendmax(USD(1)), txflags(tfNoSupplyFinanceChainDirect));

            if (withFix)
                BEAST_EXPECT(!isOffer(env, gw, USD(0), SFC(0)));
            else
                BEAST_EXPECT(isOffer(env, gw, USD(0), SFC(0)));
        }
    }
    ...
    testReexecuteDirectStep(FeatureBitset features)
    {
        ...
        BEAST_EXPECT(!getNoSupplyFinanceChainFlag(env, gw, alice, usdC));
        ...
        env(pay(alice, bob, SFC(10000)), path(~SFC), sendmax(USD(100)),
            txflags(tfPartialPayment | tfNoSupplyFinanceChainDirect));
    }
    ...
    testRIPD1443(bool withFix)
    {
        ...
        env.fund(SFC(100000000), alice, noSupplyFinanceChain(bob), carol, gw);
        env.trust(gw["USD"](10000), alice, carol);
        env(trust(bob, gw["USD"](10000), tfSetNoSupplyFinanceChain));
        env.trust(gw["USD"](10000), bob);
        env.close();

        // set no SupplyFinanceChain between bob and the gateway

        env(pay(gw, alice, gw["USD"](1000)));
        env.close();

        env(offer(alice, bob["USD"](1000), SFC(1)));
        env.close();

        env(pay(alice, alice, SFC(1)), path(gw, bob, ~SFC),
            sendmax(gw["USD"](1000)), txflags(tfNoSupplyFinanceChainDirect),
        ...
        env(offer(alice, SFC(1000), bob["USD"](1000)));
        env.close();

        env(pay (carol, carol, gw["USD"](1000)), path(~bob["USD"], gw),
            sendmax(SFC(100000)), txflags(tfNoSupplyFinanceChainDirect),
            ter(withFix ? TER {tecPATH_DRY} : TER {tesSUCCESS}));
        env.close();
    ...
    testRIPD1449(bool withFix)
    {
        ...
        // pay alice -> sfc -> USD/bob -> bob -> gw -> alice
        // set no SupplyFinanceChain on bob's side of the bob/gw trust line
        ...
        env(trust(bob, USD(10000), tfSetNoSupplyFinanceChain));
        ...
        env(offer(carol, SFC(1), bob["USD"](1000)));
        env.close();

        env(pay(alice, alice, USD(1000)), path(~bob["USD"], bob, gw),
            sendmax(SFC(1)), txflags(tfNoSupplyFinanceChainDirect),
            ter(withFix ? TER {tecPATH_DRY} : TER {tesSUCCESS}));
        env.close();
    }
    ...
    testSelfPayLowQualityOffer (FeatureBitset features)
    {
        ...
        // SFC than the offering account held.  This unit test reproduces
        // that failing case.
    ...
    testZeroOutputStep()
    {
        ...
        env(offer(bob, EUR(100), SFC(0.000001)));
        env.close();

        env(pay(alice, carol, SFC(1)),
            path(~EUR, ~SFC),
            sendmax(USD(1)),
            txflags(tfPartialPayment),
            ter(tecPATH_DRY));
    ...
    void testWithFeats(FeatureBitset features)
    {
        ...
        testSelfFundedSFCEndpoint(false, features);
        testSelfFundedSFCEndpoint(true, features);
...
BEAST_DEFINE_TESTSUITE_PRIO(Flow,app,SupplyFinanceChain,2);
BEAST_DEFINE_TESTSUITE_MANUAL_PRIO(Flow_manual,app,SupplyFinanceChain,4);
