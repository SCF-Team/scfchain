...
class Check_test : public beast::unit_test::suite
{
    ...
    void testEnabled()
    {
        ...
        Account const alice {"alice"};
        {
            ...
            env.fund (SFC(1000), alice);

            uint256 const checkId {
                getCheckIndex (env.master, env.seq (env.master))};
            env (check::create (env.master, alice, SFC(100)),
                ter(temDISABLED));
            env.close();

            env (check::cash (alice, checkId, SFC(100)),
                ter (temDISABLED));
            env.close();
        ...
        }
        {
            ...
            env (check::cash (alice, checkId1, SFC(100)));
            env.close();
    ...
    void testCreateValid()
    {
        ...
        STAmount const startBalance {SFC(1000).value()};
            ...
            [&env, &USD, this] (Account const& from, Account const& to)
        {
            ...
            env (check::create (from, to, SFC(2000)));
            env.close();
    ...
    void testCreateInvalid()
    {
        ...
        // Check to self.
        env (check::create (alice, alice, SFC(10)), ter (temREDUNDANT));
        env.close();
    ...
    void testCashSFC()
    {
        // Explore many of the valid ways to cash a check for SFC.
        testcase ("Cash SFC");
        ...
        env.fund (startBalance, alice, bob);
        {
            ...
            env (check::cash (bob, chkId, SFC(10)));
            env.close();
            env.require (balance (alice,
                startBalance - SFC(10) - drops (baseFeeDrops)));
            ...
            env (pay (env.master, alice, SFC(10) + drops (baseFeeDrops)));
            env (pay (bob, env.master,   SFC(10) - drops (baseFeeDrops * 2)));
            env.close();
    ...
    void testCashIOU ()
    {
        ...
        IOU const USD {gw["USD"]};
        {
            ...
            env.fund (SFC(1000), gw, alice, bob);
    ...
    void testCashInvalid()
    {
        ...
        env.fund (SFC(1000), gw, alice, bob, zoe);
        ...
        env (check::create (alice, bob, SFC(10)), expiration (env.now() + 1s));
        env.close();
        ...
        // Same set of failing cases for both IOU and SFC check cashing.
        ...
        failingCases (chkIdX, SFC(10));
        failingCases (chkIdU, USD(20));

        // Verify that those two checks really were cashable.
        env (check::cash (bob, chkIdU, USD(20)));
        env.close();
        env (check::cash (bob, chkIdX, check::DeliverMin (SFC(10))));
        verifyDeliveredAmount (env, SFC(10));

        // Try to cash an expired check.
        env (check::cash (bob, chkIdExp, SFC(10)), ter (tecEXPIRED));
        env.close();
    ...
    void testCancelValid()
    {
                ...
                allSupported | featureMultiSignReserve})
        {
            ...
            env (check::create (alice, bob, SFC(10)), expiration (env.now()+600s));
            env.close();
    ...
    void testFix1623Enable ()
    {
        ...
        auto testEnable = [this] (FeatureBitset const& features, bool hasFields)
        {
            ...
            uint256 const chkId {getCheckIndex (alice, env.seq (alice))};
            env (check::create (alice, bob, SFC(200)));
            env.close();

            env (check::cash (bob, chkId, check::DeliverMin (SFC(100))));
...
BEAST_DEFINE_TESTSUITE (Check, tx, SupplyFinanceChain);
