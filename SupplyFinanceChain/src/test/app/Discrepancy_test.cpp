...
class Discrepancy_test : public beast::unit_test::suite
{
    ...
    void
    testSFCDiscrepancy (FeatureBitset features)
    {
        testcase ("Discrepancy test : SFC Discrepancy");
        ...
        env.fund(SFC(2000), A1);
        env.fund(SFC(1000), A2, A6, A7);
        env.fund(SFC(5000), A3);
        env.fund(SFC(1000000), A4);
        env.fund(SFC(600000), A5);
        env.close();
        ...
        env(offer(A4, SFC(49147), A2["JPY"](34501)));
        env(offer(A5, A3["CNY"](3150), SFC(80086)));
        env(offer(A7, SFC(1233), A6["CNY"](25)));
        env.close();

        test::PathSet payPaths {
            test::Path {A2["JPY"], A2},
            test::Path {SFC, A2["JPY"], A2},
            test::Path {A6, SFC, A2["JPY"], A2} };
...
public:
    void run () override
    {
        ...
        testSFCDiscrepancy (sa - featureFlow - fix1373 - featureFlowCross);
        testSFCDiscrepancy (sa               - fix1373 - featureFlowCross);
        testSFCDiscrepancy (sa                         - featureFlowCross);
        testSFCDiscrepancy (sa);
    }
};

BEAST_DEFINE_TESTSUITE (Discrepancy, app, SupplyFinanceChain);
