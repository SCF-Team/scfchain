...
#include <test/jtx.h>
#include <SupplyFinanceChain/app/tx/applySteps.h>
#include <SupplyFinanceChain/ledger/Directory.h>
...
struct Escrow_test : public beast::unit_test::suite
{
    ...
    testEnablement()
    {
        ...
        { // Escrow not enabled
            ...
            env.fund(SFC(5000), "alice", "bob");
            env(escrow("alice", "bob", SFC(1000)),
        ...
        { // Escrow enabled
            Env env(*this);
            env.fund(SFC(5000), "alice", "bob");
            env(escrow("alice", "bob", SFC(1000)), finish_time(env.now() + 1s));
            env.close();

            auto const seq1 = env.seq("alice");

            env(escrow("alice", "bob", SFC(1000)), condition (cb1),
            ...
            env(escrow("alice", "bob", SFC(1000)), condition(cb2),
    ...
    testTiming()
    {
...

        {
            ...
            env(escrow("alice", "bob", SFC(1000)), finish_time(ts));
...

        {
            ...
            auto const seq = env.seq("alice");
            env(escrow("alice", "bob", SFC(1000)), condition(cb1), cancel_time(ts));
...

        {
            ...
            auto const seq = env.seq("alice");
            env(escrow("alice", "bob", SFC(1000)), finish_time(fts), cancel_time(cts));
    ...
    testTags()
    {
        ...
        env(escrow(alice, bob, SFC(1000)), finish_time(env.now() + 1s), ter(tecDST_TAG_NEEDED));

        // set source and dest tags
        auto const seq = env.seq(alice);

        env(escrow(alice, bob, SFC(1000)), finish_time(env.now() + 1s), stag(1), dtag(2));
    ...
    testDisallowSFC()
    {
        testcase ("Disallow SFC");

        using namespace jtx;
        using namespace std::chrono;

        {
            // Respect the "asfDisallowSFC" account flag:
            Env env(*this, supported_amendments() - featureDepositAuth);

            env.fund(SFC(5000), "bob", "george");
            env(fset("george", asfDisallowSFC));
            env(escrow("bob", "george", SFC(10)), finish_time(env.now() + 1s),
                ter (tecNO_TARGET));
        }
        {
            // Ignore the "asfDisallowSFC" account flag, which we should
    ...
    test1571()
    {
...

        {
            ...
            env.fund(SFC(5000), "alice", "bob", "carol");
            env.close();

            // Creating an escrow without a finish time and finishing it
            // is allowed without fix1571:
            auto const seq1 = env.seq("alice");
            env(escrow("alice", "bob", SFC(100)), cancel_time(env.now() + 1s), fee(1500));
            env.close();
            env(finish("carol", "alice", seq1), fee(1500));
            BEAST_EXPECT (env.balance ("bob") == SFC(5100));
            ...
            auto const seq2 = env.seq("alice");
            env(escrow("alice", "bob", SFC(100)), cancel_time(env.now() + 1s),
                condition(cb1), fee(1500));
            env.close();
            env(finish("carol", "alice", seq2), condition(cb1),
                fulfillment(fb1), fee(1500));
            BEAST_EXPECT (env.balance ("bob") == SFC(5200));
        }

        {
            ...
            env (escrow("alice", "bob", SFC(100)), cancel_time(env.now() + 90s),
    ...
    testFails()
    {
        ...
        env(escrow("alice", "carol", SFC(1)), condition(cb1),
        ...
        { // Fail if the sender wants to send more than he has:
            ...
            env.fund (accountReserve + accountIncrement + SFC(50), "daniel");
            env(escrow("daniel", "bob", SFC(51)),
                finish_time(env.now() + 1s), ter(tecUNFUNDED));

            env.fund (accountReserve + accountIncrement + SFC(50), "evan");
            env(escrow("evan", "bob", SFC(50)),
                finish_time(env.now() + 1s), ter(tecUNFUNDED));

            env.fund (accountReserve, "frank");
            env(escrow("frank", "bob", SFC(1)),
                finish_time(env.now() + 1s), ter(tecINSUFFICIENT_RESERVE));
        }

        { // Specify incorrect sequence number
            env.fund (SFC(5000), "hannah");
            auto const seq = env.seq("hannah");
            env(escrow("hannah", "hannah", SFC(10)),
                finish_time(env.now() + 1s), fee(1500));
            env.close();
        ...
        { // Try to specify a condition for a non-conditional payment
            env.fund (SFC(5000), "ivan");
            auto const seq = env.seq("ivan");

            env(escrow("ivan", "ivan", SFC(10)), finish_time(env.now() + 1s));
            env.close();
    ...
    testLockup()
    {
...

        {
            ...
            env.fund(SFC(5000), "alice", "bob");
            auto const seq = env.seq("alice");
            env(escrow("alice", "alice", SFC(1000)), finish_time(env.now() + 5s));
            env.require(balance("alice", SFC(4000) - drops(10)));
            ...
            env.require(balance("alice", SFC(5000) - drops(10)));
        }
        {
            ...
            Env env(*this);
            env.fund(SFC(5000), "alice", "bob", "zelda");
            auto const seq = env.seq("alice");
            env(escrow("alice", "bob", SFC(1000)), finish_time(env.now() + 5s));
            env.require(balance("alice", SFC(4000) - drops(10)));
            ...
            env.require(balance("alice", SFC(4000) - drops(10)));
            env.require(balance("bob", SFC(6000)));
            env.require(balance("zelda", SFC(5000) - drops(40)));
        }
        {
            ...
            env.require(balance("alice", SFC(4000) - (baseFee * 5)));
            env.require(balance("bob", SFC(6000) - (baseFee * 5)));
            env.require(balance("zelda", SFC(5000) - (baseFee * 4)));
        }
        {
            ...
            auto const baseFee = env.current()->fees().base;
            env.require(balance("alice", SFC(4000) - (baseFee * 2)));
            env.require(balance("bob", SFC(6000) - (baseFee * 2)));
            env.require(balance("zelda", SFC(5000) - (baseFee * 1)));
        }
        {
            // Conditional
            Env env(*this);
            env.fund(SFC(5000), "alice", "bob");
            auto const seq = env.seq("alice");
            env(escrow("alice", "alice", SFC(1000)), condition(cb2), finish_time(env.now() + 5s));
            env.require(balance("alice", SFC(4000) - drops(10)));
        ...
        }
        {
            ...
            env.fund(SFC(5000), "alice", "bob");
            auto const seq = env.seq("alice");
            env(escrow("alice", "alice", SFC(1000)), condition(cb3), finish_time(env.now() + 5s));
            env.require(balance("alice", SFC(4000) - drops(10)));
            env.close();
    ...
    testEscrowConditions()
    {
        ...
        { // Test cryptoconditions
            ...
            env(escrow("alice", "carol", SFC(1000)), condition(cb1),
                cancel_time(env.now() + 1s));
            BEAST_EXPECT((*env.le("alice"))[sfOwnerCount] == 1);
            env.require(balance("alice", SFC(4000) - drops(10)));
            env.require(balance("carol", SFC(5000)));
            env(cancel("bob", "alice", seq), ter(tecNO_PERMISSION));
            BEAST_EXPECT((*env.le("alice"))[sfOwnerCount] == 1);
            ...
            BEAST_EXPECT((*env.le("alice"))[sfOwnerCount] == 0);
            env.require(balance("carol", SFC(6000)));
        ...
        { // Test cancel when condition is present
            ...
            auto const seq = env.seq("alice");
            BEAST_EXPECT((*env.le("alice"))[sfOwnerCount] == 0);
            env(escrow("alice", "carol", SFC(1000)), condition(cb2),
                cancel_time(env.now() + 1s));
            env.close();
            env.require(balance("alice", SFC(4000) - drops(10)));
        ...
        }
        {
            ...
            env.close();
            auto const seq = env.seq("alice");
            env(escrow("alice", "carol", SFC(1000)), condition(cb3),
                cancel_time(env.now() + 1s));
            BEAST_EXPECT((*env.le("alice"))[sfOwnerCount] == 1);
        ...
        { // Test long & short conditions during creation
            ...
            env(escrow("alice", "carol", SFC(1000)), condition(Slice{p, s}),
                cancel_time(ts), ter(temMALFORMED));
            env(escrow("alice", "carol", SFC(1000)), condition(Slice{p, s - 1}),
                cancel_time(ts), ter(temMALFORMED));
            env(escrow("alice", "carol", SFC(1000)), condition(Slice{p, s - 2}),
                cancel_time(ts), ter(temMALFORMED));
            env(escrow("alice", "carol", SFC(1000)), condition(Slice{p + 1, s - 1}),
                cancel_time(ts), ter(temMALFORMED));
            env(escrow("alice", "carol", SFC(1000)), condition(Slice{p + 1, s - 3}),
                cancel_time(ts), ter(temMALFORMED));
            env(escrow("alice", "carol", SFC(1000)), condition(Slice{p + 2, s - 2}),
                cancel_time(ts), ter(temMALFORMED));
            env(escrow("alice", "carol", SFC(1000)), condition(Slice{p + 2, s - 3}),
                cancel_time(ts), ter(temMALFORMED));

            auto const seq = env.seq("alice");
            env(escrow("alice", "carol", SFC(1000)), condition(Slice{p + 1, s - 2}),
                cancel_time(ts), fee(100));
            env(finish("bob", "alice", seq),
                condition(cb1), fulfillment(fb1), fee(1500));
            env.require(balance("alice", SFC(4000) - drops(100)));
            env.require(balance("bob", SFC(5000) - drops(1500)));
            env.require(balance("carol", SFC(6000)));
        }
        { // Test long and short conditions & fulfillments during finish
            ...
            // All these are expected to fail, because the
            // condition we pass in is malformed in some way
            env(escrow("alice", "carol", SFC(1000)), condition(Slice{cp, cs}),
                cancel_time(ts), ter(temMALFORMED));
            env(escrow("alice", "carol", SFC(1000)), condition(Slice{cp, cs - 1}),
                cancel_time(ts), ter(temMALFORMED));
            env(escrow("alice", "carol", SFC(1000)), condition(Slice{cp, cs - 2}),
                cancel_time(ts), ter(temMALFORMED));
            env(escrow("alice", "carol", SFC(1000)), condition(Slice{cp + 1, cs - 1}),
                cancel_time(ts), ter(temMALFORMED));
            env(escrow("alice", "carol", SFC(1000)), condition(Slice{cp + 1, cs - 3}),
                cancel_time(ts), ter(temMALFORMED));
            env(escrow("alice", "carol", SFC(1000)), condition(Slice{cp + 2, cs - 2}),
                cancel_time(ts), ter(temMALFORMED));
            env(escrow("alice", "carol", SFC(1000)), condition(Slice{cp + 2, cs - 3}),
                cancel_time(ts), ter(temMALFORMED));

            auto const seq = env.seq("alice");
            env(escrow("alice", "carol", SFC(1000)), condition(Slice{cp + 1, cs - 2}),
                cancel_time(ts), fee(100));
        ...
        { // Test empty condition during creation and
          // empty condition & fulfillment during finish
            ...
            env(escrow("alice", "carol", SFC(1000)), condition(Slice{}),
                cancel_time(env.now() + 1s), ter(temMALFORMED));
        ...
        { // Test a condition other than PreimageSha256, which
          // would require a separate amendment
            ...
            env(escrow("alice", "bob", SFC(1000)), condition(cb),
                cancel_time(env.now() + 1s), ter(temMALFORMED));
        }
    }
    ...
    testMetaAndOwnership()
    {
...

        {
            ...
            env.fund(SFC(5000), alice, bruce, carol);

            auto const seq = env.seq(alice);
            env(escrow(alice, carol, SFC(1000)), finish_time(env.now() + 1s));
            ...
            SupplyFinanceChain::Dir aod (*env.current(), keylet::ownerDir(alice.id()));
            BEAST_EXPECT(std::distance(aod.begin(), aod.end()) == 1);
            BEAST_EXPECT(std::find(aod.begin(), aod.end(), escrow) != aod.end());

            SupplyFinanceChain::Dir cod (*env.current(), keylet::ownerDir(carol.id()));
        ...
        }
        {
            ...
            env(escrow(alice, alice, SFC(1000)),
            ...
            env(escrow(bruce, bruce, SFC(1000)),
            ...
            {
                SupplyFinanceChain::Dir bod(*env.current(), keylet::ownerDir(bruce.id()));
        ...
        }
        {
            ...
            env(escrow(alice, bruce, SFC(1000)), finish_time(env.now() + 1s));
            BEAST_EXPECT((*env.meta())[sfTransactionResult] ==
                static_cast<std::uint8_t>(tesSUCCESS));
            env.close(5s);
            env(escrow(bruce, carol, SFC(1000)),
                finish_time(env.now() + 1s), cancel_time(env.now() + 2s));
            BEAST_EXPECT((*env.meta())[sfTransactionResult] ==
                static_cast<std::uint8_t>(tesSUCCESS));
            env.close(5s);
    ...
    void testConsequences()
    {
        ...
        {
            auto const jtx = env.jt(escrow("alice", "carol", SFC(1000)),
            ...
            BEAST_EXPECT(conseq.potentialSpend == SFC(1000));
        }

        {
            ...
            BEAST_EXPECT(conseq.category == TxConsequences::normal);
            BEAST_EXPECT(conseq.fee == drops(10));
            BEAST_EXPECT(conseq.potentialSpend == SFC(0));
        }
...
BEAST_DEFINE_TESTSUITE(Escrow,app,SupplyFinanceChain);
