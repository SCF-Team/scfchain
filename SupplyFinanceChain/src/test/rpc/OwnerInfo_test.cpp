...
class OwnerInfo_test : public beast::unit_test::suite
{
    ...
    testBasic ()
    {
        ...
        env(offer(alice, USD(1), SFC(1000)));
        env.close();

        env(pay(gw, alice, USD(50)));
        env(pay(gw, alice, CNY(50)));
        env(offer(alice, CNY(2), SFC(1000)));
        ...
        if (! BEAST_EXPECT (result[jss::accepted].isMember(jss::SupplyFinanceChain_lines)))
            return;
        auto lines = result[jss::accepted][jss::SupplyFinanceChain_lines];
        ...
        BEAST_EXPECT (
            ...
            SFC(1000).value().getJson(JsonOptions::none));
        BEAST_EXPECT (
        ...
        if (! BEAST_EXPECT (result[jss::current].isMember(jss::SupplyFinanceChain_lines)))
            return;
        lines = result[jss::current][jss::SupplyFinanceChain_lines];
        if (! BEAST_EXPECT (lines.isArray() && lines.size() == 2))
            return;
...
BEAST_DEFINE_TESTSUITE(OwnerInfo,app,SupplyFinanceChain);
