        ...
        for (bool const afterSwitchTime : {true, false})
        {
...

            {
                ...
                if (wsc->version() == 2)
                {
                    ...
                    BEAST_EXPECT(
                        jv.isMember(jss::SupplyFinanceChainrpc) &&
                        jv[jss::SupplyFinanceChainrpc] == "2.0");
...
BEAST_DEFINE_TESTSUITE(DeliveredAmount, app, SupplyFinanceChain);
