...
#include <SupplyFinanceChain/beast/utility/temp_dir.h>
#include <SupplyFinanceChain/resource/ResourceManager.h>
#include <SupplyFinanceChain/resource/impl/Entry.h>
#include <SupplyFinanceChain/resource/impl/Tuning.h>
#include <SupplyFinanceChain/rpc/impl/Tuning.h>

namespace SupplyFinanceChain {

class NoSupplyFinanceChainCheck_test : public beast::unit_test::suite
{
    void
    testBadInput ()
    {
        testcase ("Bad input to noSupplyFinanceChain_check");
        ...
        { // missing account field
            auto const result = env.rpc ("json", "noSupplyFinanceChain_check", "{}")
                [jss::result];
    ...
    {
        testcase << "Request noSupplyFinanceChain_check for " <<
        ...
        {
            env (fset (alice, asfDefaultSupplyFinanceChain));
            env (trust (alice, gw["USD"](100)));
        }
        else
        {
            env (fclear (alice, asfDefaultSupplyFinanceChain));
            env (trust (alice, gw["USD"](100), gw, tfSetNoSupplyFinanceChain));
        }
        env.close ();
        ...
        params[jss::ledger] = "current";
        auto result = env.rpc ("json", "noSupplyFinanceChain_check",
            boost::lexical_cast<std::string>(params)) [jss::result];
        ...
        result = env.rpc ("json", "noSupplyFinanceChain_check",
            boost::lexical_cast<std::string>(params)) [jss::result];
...
class NoSupplyFinanceChainCheckLimits_test : public beast::unit_test::suite
{
    void
    testLimits(bool admin)
    {
        ...
        auto checkBalance = [&env]()
        {
            ...
            // cases.
            using namespace SupplyFinanceChain::Resource;
        ...
        for (auto i = 0; i < SupplyFinanceChain::RPC::Tuning::noSupplyFinanceChainCheck.rmax + 5; ++i)
        {
            if (! admin)
                checkBalance();
            ...
            env.memoize(gw);
            env (pay (env.master, gw, SFC(1000)),
                seq (autofill),
                fee (txq.getMetrics(*env.current()).openLedgerFeeLevel + 1),
                sig (autofill));
            env (fset (gw, asfDefaultSupplyFinanceChain),
                seq (autofill),
                fee (txq.getMetrics(*env.current()).openLedgerFeeLevel + 1),
                sig (autofill));
...
BEAST_DEFINE_TESTSUITE(NoSupplyFinanceChainCheck, app, SupplyFinanceChain);
...
BEAST_DEFINE_TESTSUITE_MANUAL_PRIO(NoSupplyFinanceChainCheckLimits, app, SupplyFinanceChain, 1);
