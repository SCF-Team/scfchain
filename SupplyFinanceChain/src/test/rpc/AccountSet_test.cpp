...
public:
    ...
    void testNullAccountSet()
    {
        ...
        env.fund(SFC(10000), noSupplyFinanceChain(alice));
    ...
    void testMostFlags()
    {
        ...
        testFlags ({asfRequireDest, asfRequireAuth, asfDisallowSFC,
            asfGlobalFreeze, asfDisableMaster, asfDefaultSupplyFinanceChain});
    ...
    void testBadInputs(bool withFeatures)
    {
        ...
        auto jt = fset(alice, asfDisallowSFC);
        jt[jss::ClearFlag] = asfDisallowSFC;
        ...
        jt = fset(alice, asfDisallowSFC);
        jt[sfFlags.fieldName] = tfAllowSFC;
        env(jt, ter(temINVALID_FLAG));
...
BEAST_DEFINE_TESTSUITE(AccountSet,app,SupplyFinanceChain);
