...
public:
    ...
    void testLedgerType()
    {
        ...
        env(offer(Account{"bob0"}, USD(100), SFC(100)));
...

        {
            ...
            jv[jss::Amount] = SFC(50).value().getJson(JsonOptions::none);
...

        {
            ...
            jv[jss::Amount] = SFC(100).value().getJson (JsonOptions::none);
        ...
        BEAST_EXPECT( checkArraySize(jrr[jss::state], 1) );
        for (auto const& j : jrr[jss::state])
            BEAST_EXPECT( j["LedgerEntryType"] == jss::SupplyFinanceChainState );
        }
...
BEAST_DEFINE_TESTSUITE_PRIO(LedgerData,app,SupplyFinanceChain,1);
