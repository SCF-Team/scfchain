...
public:
    ...
    testOneSideEmptyBook()
    {
...

        {
            ...
            books[jss::books] = Json::arrayValue;
            {
                ...
                j[jss::snapshot] = true;
                j[jss::taker_gets][jss::currency] = "SFC";
            ...
            if (wsc->version() == 2)
            {
                ...
                BEAST_EXPECT(jv.isMember(jss::SupplyFinanceChainrpc) && jv[jss::SupplyFinanceChainrpc] == "2.0");
...

        {
            ...
            env(offer("alice", SFC(700), USD(100)),
                ...
                [&](auto const& jv)
                {
                    ...
                    return t[jss::TransactionType] == jss::OfferCreate &&
                        ...
                        USD(100).value().getJson(JsonOptions::none) &&
                            t[jss::TakerPays] ==
                            SFC(700).value().getJson(JsonOptions::none);
                }));
        }

        {
            // Create a bid: TakerPays 100/USD, TakerGets 75
            env(offer("alice", USD(100), SFC(75)),
    ...
    testOneSideOffersInBook()
    {
        ...
        env(offer("alice", USD(100), SFC(200)),
            require(owners("alice", 2)));
        env.close();

        {
            ...
            BEAST_EXPECT(jv[jss::result][jss::offers][0u][jss::TakerGets] ==
                SFC(200).value().getJson(JsonOptions::none));
...

        {
            ...
            BEAST_EXPECT(wsc->findMsg(5s,
                [&](auto const& jv)
                {
                    ...
                    return t[jss::TransactionType] == jss::OfferCreate &&
                        t[jss::TakerGets] ==
                        SFC(75).value().getJson(JsonOptions::none) &&
                            t[jss::TakerPays] ==
                            USD(100).value().getJson(JsonOptions::none);
                }));
        }
    ...
    testBothSidesOffersInBook()
    {
...

        {
            ...
            BEAST_EXPECT(jv[jss::result][jss::asks][0u][jss::TakerPays] ==
                SFC(500).value().getJson(JsonOptions::none));
    ...
    testTrackOffers()
    {
        ...
        Account bob {"bob"};
        auto wsc = makeWSClient(env.app().config());
        env.fund(SFC(20000), alice, bob, gw);
        env.close();
        auto USD = gw["USD"];
        ...
        env(offer(alice, SFC(4000), USD(10)));
        env.close();

        Json::Value jvParams;
        jvParams[jss::taker] = env.master.human();
        jvParams[jss::taker_pays][jss::currency] = "SFC";
        ...
        BEAST_EXPECT(jrOffer[jss::TakerPays] ==
            SFC(4000).value().getJson(JsonOptions::none));
        ...
        BEAST_EXPECT(wsc->findMsg(5s,
            [&](auto const& jv)
            {
                ...
                return t[jss::TransactionType] == jss::OfferCreate &&
                       ...
                       t[jss::TakerPays] ==
                       SFC(2000).value().getJson(JsonOptions::none);
            }));
    ...
    testCrossingSingleBookOffer()
    {
        ...
        // Scenario is:
        //  - Alice and Bob place identical offers for USD -> SFC
        ...
        env.fund (SFC(1000000), gw, alice, bob, charlie);
        env.close();
        ...
        // Alice and Bob offer $500 for 500 SFC
        env (offer (alice, SFC(500), USD(500)));
        env (offer (bob, SFC(500), USD(500)));
        env.close();
        ...
        env(offer(charlie, USD(1000), SFC(1000)));
        env.close();
        env.require(offers(alice, 0), offers(bob, 0), offers(charlie, 0));
        using namespace std::chrono_literals;
        BEAST_EXPECT(offerOnlyOnceInStream(wsc, 1s, SFC(1000), USD(1000)));
    ...
    testCrossingMultiBookOffer()
    {
        ...
        // Scenario is:
        //  - Alice has 1 USD and wants 100 SFC
        //  - Bob has 100 SFC and wants 1 EUR
        ...
        env(offer(alice, SFC(100), USD(1)));
        env(offer(bob, EUR(1), SFC(100)));
        env.close();
...

        {
...

            {
                ...
                j[jss::taker_pays][jss::currency] = "SFC";
            }
    ...
    testBookOfferErrors()
    {
...

        {
            ...
            jvParams[jss::taker_gets][jss::currency] = "SFC";
            auto const jrr = env.rpc(
                "json", "book_offers", to_string(jvParams)) [jss::result];
...

        {
            ...
            BEAST_EXPECT(jrr[jss::error_message] ==
                ...
                "for SFC currency specification.");
        }

        {
            ...
            jvParams[jss::taker_pays][jss::issuer]   = toBase58(sfcAccount());
            jvParams[jss::taker_gets][jss::currency] = "USD";
            jvParams[jss::taker_gets][jss::issuer]   = gw.human();
            auto const jrr = env.rpc(
                "json", "book_offers", to_string(jvParams)) [jss::result];
    ...
    testBookOfferLimits(bool asAdmin)
    {
        ...
        Account gw {"gw"};
        env.fund(SFC(200000), gw);
        env.close();
        auto USD = gw["USD"];

        for(auto i = 0; i <= RPC::Tuning::bookOffers.rmax; i++)
            env(offer(gw, SFC(50 + 1*i), USD(1.0 + 0.1*i)));
        env.close();
...
BEAST_DEFINE_TESTSUITE_PRIO(Book,app,SupplyFinanceChain,1);
