...
public:
    void testErrors()
    {
       ...
       // test errors on marker
        {
            Account const gw{ "G" };
            env.fund(SFC(1000), gw);
            auto const USD = gw["USD"];
            env.trust(USD(1000), bob);
            env(pay(gw, bob, SFC(1)));
            env(offer(bob, SFC(100), bob["USD"](1)), txflags(tfPassive));
    ...
    void testUnsteppedThenStepped()
    {
        ...
        env(offer(bob, SFC(100), bob["USD"](1)),txflags(tfPassive));
        env(offer(bob, SFC(100), USD1(1)), txflags(tfPassive));
    ...
    void testObjectTypes()
    {
        ...
        }
        {
            ...
            jvEscrow[jss::Amount] =
                SFC(100).value().getJson(JsonOptions::none);
        ...
        }
        {
            ...
            jvPayChan[jss::Amount] =
                SFC (300).value().getJson (JsonOptions::none);
...
BEAST_DEFINE_TESTSUITE(AccountObjects,app,SupplyFinanceChain);
