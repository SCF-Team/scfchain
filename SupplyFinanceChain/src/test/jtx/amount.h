...
#ifndef SUPPLYFINANCECHAIN_TEST_JTX_AMOUNT_H_INCLUDED
#define SUPPLYFINANCECHAIN_TEST_JTX_AMOUNT_H_INCLUDED
...
The decision was made to accept amounts of drops and SFC
using an int type, since the range of SFC is 100 billion
...
/** Represents an SFC or IOU quantity
    This customizes the string conversion and supports
    SFC conversions from integer and floating point.
*/
struct PrettyAmount
{
private:
...
{
    AccountID account;
    SupplyFinanceChain::Currency currency;

    BookSpec(AccountID const& account_,
        SupplyFinanceChain::Currency const& currency_)
        : account(account_)
        , currency(currency_)
    {
    }
...
struct SFC_t
{
    ...
    */
    ...
    /** Returns an amount of SFC as STAmount

        @param v The number of SFC (not drops)
    */
    /** @{ */
    ...
    friend
    BookSpec
    operator~ (SFC_t const&)
    {        
        return BookSpec( sfcAccount(),
            sfcCurrency() );
    }
};

/** Converts to SFC Issue or STAmount.

    Examples:
        SFC         Converts to the SFC Issue
        SFC(10)     Returns STAmount of 10 SFC
*/
extern SFC_t const SFC;

/** Returns an SFC STAmount.
