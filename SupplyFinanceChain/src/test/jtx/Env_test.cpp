...
#include <SupplyFinanceChain/beast/hash/uhash.h>
...
public:
    ...
    testAmount()
    {
        ...
        BEAST_EXPECT(to_string(SFC(5)) == "5 SFC");
        BEAST_EXPECT(to_string(SFC(.80)) == "0.8 SFC");
        BEAST_EXPECT(to_string(SFC(.005)) == "5000 drops");
        BEAST_EXPECT(to_string(SFC(0.1)) == "0.1 SFC");
        BEAST_EXPECT(to_string(SFC(10000)) == "10000 SFC");
        BEAST_EXPECT(to_string(drops(10)) == "10 drops");
        BEAST_EXPECT(to_string(drops(123400000)) == "123.4 SFC");
        BEAST_EXPECT(to_string(SFC(-5)) == "-5 SFC");
        BEAST_EXPECT(to_string(SFC(-.99)) == "-0.99 SFC");
        BEAST_EXPECT(to_string(SFC(-.005)) == "-5000 drops");
        BEAST_EXPECT(to_string(SFC(-0.1)) == "-0.1 SFC");
        BEAST_EXPECT(to_string(drops(-10)) == "-10 drops");
        BEAST_EXPECT(to_string(drops(-123400000)) == "-123.4 SFC");

        BEAST_EXPECT(SFC(1) == drops(1000000));
        BEAST_EXPECT(SFC(1) == STAmount(1000000));
        BEAST_EXPECT(STAmount(1000000) == SFC(1));
    ...
    testEnv()
    {
        ...
        {
            Env env(*this);
            env(pay("alice", "bob", SFC(1000)), seq(1), fee(10), sig("alice"), ter(terNO_ACCOUNT));
        }
        ...
        // fund
        {
            ...
            env.fund(n, "dave", noSupplyFinanceChain("eric"));
            env.fund(n, "fred", noSupplyFinanceChain("gary", "hank"));
            env.fund(n, noSupplyFinanceChain("irene"));
            env.fund(n, noSupplyFinanceChain("jim"), "karen");
            env.fund(n, noSupplyFinanceChain("lisa", "mary"));

            // flags
            env.fund(n, noSupplyFinanceChain("xavier"));
            env.require(nflags("xavier", asfDefaultSupplyFinanceChain));
            env.fund(n, "yana");
            env.require(flags("yana", asfDefaultSupplyFinanceChain));
        }
        ...
        {
            Env env(*this);
            env.fund(n, noSupplyFinanceChain("alice", gw));
    ...
    testRequire()
    {
        ...
        env.require(balance("alice", SFC(none)));
        env.fund(SFC(10000), "alice", gw);
        env.require(balance("alice", USD(none)));
        env.trust(USD(100), "alice");
        env.require(balance("alice", SFC(10000))); // fee refunded
    ...
    testPayments()
    {
        ...
        env.require(balance("alice", SFC(10000)));
        env.require(balance("bob", SFC(10000)));
        env.require(balance("carol", SFC(10000)));
        env.require(balance(gw, SFC(10000)));

        env(pay(env.master, "alice", SFC(1000)), fee(none),     ter(temMALFORMED));
        env(pay(env.master, "alice", SFC(1000)), fee(1),        ter(telINSUF_FEE_P));
        env(pay(env.master, "alice", SFC(1000)), seq(none),     ter(temMALFORMED));
        env(pay(env.master, "alice", SFC(1000)), seq(20),       ter(terPRE_SEQ));
        env(pay(env.master, "alice", SFC(1000)), sig(none),     ter(temMALFORMED));
        env(pay(env.master, "alice", SFC(1000)), sig("bob"),    ter(tefBAD_AUTH));

        env(pay(env.master, "dilbert", SFC(1000)), sig(env.master));
        ...
        env(offer("carol", SFC(50), USD(50)), require(owners("carol", 2)));
        env(pay("alice", "bob", any(USD(10))),                  ter(tecPATH_DRY));
        env(pay("alice", "bob", any(USD(10))),
            paths(SFC), sendmax(SFC(10)),                       ter(tecPATH_PARTIAL));
        env(pay("alice", "bob", any(USD(10))), paths(SFC),
            sendmax(SFC(20)));
...
BEAST_DEFINE_TESTSUITE(Env,app,SupplyFinanceChain);
