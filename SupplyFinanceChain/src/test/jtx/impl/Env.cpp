...
void
Env::fund (bool setDefaultSupplyFinanceChain,
    STAmount const& amount,
        Account const& account)
{
    memoize(account);
    if (setDefaultSupplyFinanceChain)
    {
        ...
        apply(pay(master, account, amount +
            drops(current()->fees().base)),
                jtx::seq(jtx::autofill),
                    fee(jtx::autofill),
                        sig(jtx::autofill));
        apply(fset(account, asfDefaultSupplyFinanceChain),
            jtx::seq(jtx::autofill),
                fee(jtx::autofill),
                    sig(jtx::autofill));
        require(flags(account, asfDefaultSupplyFinanceChain));
    }
    else
    {
        apply(pay(master, account, amount),
            jtx::seq(jtx::autofill),
                fee(jtx::autofill),
                    sig(jtx::autofill));
        require(nflags(account, asfDefaultSupplyFinanceChain));
    }
