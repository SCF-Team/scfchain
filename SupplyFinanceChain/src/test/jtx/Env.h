...
#ifndef SUPPLYFINANCECHAIN_TEST_JTX_ENV_H_INCLUDED
#define SUPPLYFINANCECHAIN_TEST_JTX_ENV_H_INCLUDED
...
#include <string>
#include <tuple>
#include <SupplyFinanceChain/beast/cxx17/type_traits.h> // <type_traits>
#include <utility>
...
/** Designate accounts as no-SupplyFinanceChain in Env::fund */
template <class... Args>
std::array<Account, 1 + sizeof...(Args)>
noSupplyFinanceChain (Account const& account, Args const&... args)
{
    return {{account, args...}};
}
...
supported_amendments()
{
    static const FeatureBitset ids = []{
        auto const& sa = SupplyFinanceChain::detail::supportedAmendments();
...
public:
    ...
    /** Returns the current SupplyFinanceChain Network Time
    ...
    */
    ...
    /** Returns the SFC balance on an account.
...
private:
    void
    fund (bool setDefaultSupplyFinanceChain,
...
public:
    ...
    /** Create a new account with some SFC.
        ...
        Effects:
            The asfDefaultSupplyFinanceChain on the account is set,
            and the sequence number is incremented, unless
            the account is wrapped with a call to noSupplyFinanceChain.

            The account's SFC balance is set to amount.

            Generates a test that the balance is set.

        @param amount The amount of SFC to transfer to
                      each account.

        @param args A heterogeneous list of accounts to fund
                    or calls to noSupplyFinanceChain with lists of accounts
                    to fund.
    */
