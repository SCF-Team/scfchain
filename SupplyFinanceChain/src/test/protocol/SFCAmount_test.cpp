...
class SFCAmount_test : public beast::unit_test::suite
{
public:
    void testSigNum ()
    {
    ...
    void testAddSub ()
    {
        ...
        for (auto i : { -1, 0, 1})
        {
            ...
            for (auto j : { -1, 0, 1})
            {
                ...
                BEAST_EXPECT(SFCAmount(i + j) == (x + y));
                BEAST_EXPECT(SFCAmount(i - j) == (x - y));
...
BEAST_DEFINE_TESTSUITE(SFCAmount,protocol,SupplyFinanceChain);
