...
#include <SupplyFinanceChain/basics/random.h>
...
public:
    ...
    {
        {
            ...
            Issue const sfc (sfcIssue ());

            // fractional SFC (i.e. drops)
            testSetValue ("1", sfc);
            testSetValue ("22", sfc);
            testSetValue ("333", sfc);
            testSetValue ("4444", sfc);
            testSetValue ("55555", sfc);
            testSetValue ("666666", sfc);

            // 1 SFC up to 100 billion, in powers of 10 (in drops)
            testSetValue ("1000000", sfc);
            testSetValue ("10000000", sfc);
            testSetValue ("100000000", sfc);
            testSetValue ("1000000000", sfc);
            testSetValue ("10000000000", sfc);
            testSetValue ("100000000000", sfc);
            testSetValue ("1000000000000", sfc);
            testSetValue ("10000000000000", sfc);
            testSetValue ("100000000000000", sfc);
            testSetValue ("1000000000000000", sfc);
            testSetValue ("10000000000000000", sfc);
            testSetValue ("100000000000000000", sfc);

            // Invalid native values:
            testSetValue ("1.1", sfc, false);
            testSetValue ("100000000000000001", sfc, false);
            testSetValue ("1000000000000000000", sfc, false);
        }
    ...
    void testNativeCurrency ()
    {
        ...
        unexpected (to_string (Currency ()) != "SFC", "cHC(SFC)");
        Currency c;
    ...
    void testCustomCurrency ()
    {
        ...
        unexpected (multiply (STAmount (noIssue(), 20), STAmount (3), sfcIssue ()).getText () != "60",
            "STAmount multiply fail 2");
        unexpected (multiply (STAmount (20), STAmount (3), noIssue()).getText () != "60",
            "STAmount multiply fail 3");
        unexpected (multiply (STAmount (20), STAmount (3), sfcIssue ()).getText () != "60",
        ...
        unexpected (divide (STAmount (noIssue(), 60), STAmount (3), sfcIssue ()).getText () != "20",
            "STAmount divide fail");

        unexpected (divide (STAmount (noIssue(), 60), STAmount (noIssue(), 3), noIssue()).getText () != "20",
            "STAmount divide fail");

        unexpected (divide (STAmount (noIssue(), 60), STAmount (noIssue(), 3), sfcIssue ()).getText () != "20",
            "STAmount divide fail");
    ...
    void testUnderflow ()
    {
        ...
        bigDsmall = divide (smallValue, bigValue, sfcIssue ());

        BEAST_EXPECT(bigDsmall == beast::zero);

        bigDsmall = divide (smallValue, bigNative, sfcIssue ());
    ...
    void testRounding ()
    {
        ...
        STAmount dripTest1 = mulRound (twoThird2, two, sfcIssue (), false);
        STAmount dripTest2 = multiply (twoThird2, two, sfcIssue ());
        STAmount dripTest3 = mulRound (twoThird2, two, sfcIssue (), true);
        log << dripTest1;
        log << dripTest2;
        log << dripTest3;
#endif
    ...
    testConvertSFC ()
    {
        testcase ("STAmount to SFCAmount conversions");
        ...
        {
            auto const t = amountFromString (sfc, std::to_string (drops));
            auto const s = t.sfc ();
            BEAST_EXPECT(s.drops() == drops);
            BEAST_EXPECT(t == STAmount (SFCAmount (drops)));
            BEAST_EXPECT(s == SFCAmount (drops));
        }

        try
        {
            auto const t = amountFromString (usd, "136500");
            fail (to_string (t.sfc ()));
        }
    ...
    testConvertIOU ()
    {
        ...
        try
        {
            auto const t = amountFromString (sfc, "136500");
...
BEAST_DEFINE_TESTSUITE(STAmount,SupplyFinanceChain_data,SupplyFinanceChain);
