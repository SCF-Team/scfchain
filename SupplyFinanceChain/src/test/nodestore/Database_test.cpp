...
public:
        ...
        if (type == "memory")
        {
            // Earliest ledger sequence tests
            {
                ...
                std::unique_ptr<Database> db =
                    Manager::instance().make_Database(
                        "test", scheduler, 2, parent, nodeParams, journal_);
                BEAST_EXPECT(db->earliestSeq() == SFC_LEDGER_EARLIEST_SEQ);
            }
            ...
            try
            {
                ...
                nodeParams.set("earliest_seq",
                    std::to_string(SFC_LEDGER_EARLIEST_SEQ));
    ...
    void run () override
    {
        ...
        #if SUPPLYFINANCECHAIN_ENABLE_SQLITE_BACKEND_TESTS
...
BEAST_DEFINE_TESTSUITE(Database,NodeStore,SupplyFinanceChain);
