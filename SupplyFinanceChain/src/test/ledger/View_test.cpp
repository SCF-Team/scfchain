...
#include <SupplyFinanceChain/ledger/OpenView.h>
    ...
    : public beast::unit_test::suite
{
    ...
    testFlags()
    {
        ...
        env.trust(USD(100), alice, bob, carol);
        {
            ...
            env(offer(alice, SFC(5), USD(5)));

            // Now freeze gw.
            env(fset (gw, asfGlobalFreeze));
            env.close();
            env(offer(alice, SFC(4), USD(5)), ter(tecFROZEN));
            env.close();
        ...
        }
        {
            ...
            // carol's SFC balance should be her holdings minus her reserve.
            auto const carolsSFC = accountHolds (*env.closed(), carol,
                sfcCurrency(), sfcAccount(), fhZERO_IF_FROZEN, env.journal);
            // carol's SFC balance:              10000
            // base reserve:                      -200
            // 1 trust line times its reserve: 1 * -50
            //                                 -------
            // carol's available balance:         9750
            BEAST_EXPECT(carolsSFC == SFC(9750));

            // carol should be able to spend *more* than her SFC balance on
            // a fee by eating into her reserve.
            env(noop(carol), fee(carolsSFC + SFC(10)));
            env.close();

            // carol's SFC balance should now show as zero.
            BEAST_EXPECT(SFC(0) == accountHolds (*env.closed(),
                carol, sfcCurrency(), gw, fhZERO_IF_FROZEN, env.journal));
        }
        {
            // accountFunds().
    ...
    testAreCompatible()
    {
        ...
        eA.fund(SFC(10000), alice);
        eA.close();
        auto const rdViewA3 = eA.closed();

        eA.fund(SFC(10000), bob);
        eA.close();
        ...
        eB.fund(SFC(10000), bob);
        eB.close();
        auto const rdViewB3 = eB.closed();

        eB.fund(SFC(10000), alice);
        eB.close();
    ...
    testRegressions()
    {
        ...
        // Make sure OpenLedger::empty works
        {
            ...
            env.fund(SFC(10000), Account("test"));
            BEAST_EXPECT(! env.app().openLedger().empty());
        }
    }
...
BEAST_DEFINE_TESTSUITE(View,ledger,SupplyFinanceChain);
BEAST_DEFINE_TESTSUITE(GetAmendments,ledger,SupplyFinanceChain);
