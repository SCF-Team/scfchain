...
struct Directory_test : public beast::unit_test::suite
{
    ...
    void testDirectoryOrdering()
    {
...

        {
            ...
            Env env(*this);
            env.fund(SFC(10000000), alice, gw);

            for (std::size_t i = 1; i <= 400; ++i)
                env(offer(alice, USD(i), SFC(i)));
            env.close();
            ...
            for (auto const& offer : book)
            {
                ...
                BEAST_EXPECT(offer->getFieldAmount(sfTakerGets) == SFC(count));
            }
        }
    }
    ...
    testDirIsEmpty()
    {
        ...
        env.fund(SFC(1000000), alice, charlie, gw);
        env.close();
    ...
    testRipd1353()
    {
        ...
        // should have no entries and be empty:
        {
            ...
            uint256 const bookBase = getBookBase({sfcIssue(), USD.issue()});
...
BEAST_DEFINE_TESTSUITE_PRIO(Directory,ledger,SupplyFinanceChain,1);
