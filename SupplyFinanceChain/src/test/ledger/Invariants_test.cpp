...
#include <SupplyFinanceChain/app/tx/impl/ApplyContext.h>
...
class Invariants_test : public beast::unit_test::suite
{
    ...
    void
    testSFCNotCreated (bool enabled)
    {
        using namespace test::jtx;
        testcase << "checks " << (enabled ? "enabled" : "disabled") <<
            " - SFC created";
        doInvariantCheck (enabled,
            {{ "SFC net change was positive: 500" }},
            [](Account const& A1, Account const&, ApplyContext& ac)
            {
                // put a single account in the view and "manufacture" some SFC
    ...
    testTypesMatch(bool enabled)
    {
        ...
        doInvariantCheck (enabled,
            {{ "ledger entry type mismatch" },
             { "SFC net change of -1000000000 doesn't match fee 0" }},
            [](Account const& A1, Account const&, ApplyContext& ac)
            {
    ...
    void
    testNoSFCTrustLine(bool enabled)
    {
        using namespace test::jtx;
        testcase << "checks " << (enabled ? "enabled" : "disabled") <<
            " - trust lines with SFC not allowed";
        doInvariantCheck (enabled,
            {{ "an SFC trust line was created" }},
            [](Account const& A1, Account const& A2, ApplyContext& ac)
            {
                // create simple trust SLE with sfc currency
                auto index = getSupplyFinanceChainStateIndex (A1, A2, sfcIssue().currency);
                auto const sleNew = std::make_shared<SLE>(
                    ltSUPPLYFINANCECHAIN_STATE, index);
                ac.view().insert (sleNew);
                return true;
            });
    }
    ...
    void
    testSFCBalanceCheck(bool enabled)
    {
        using namespace test::jtx;
        testcase << "checks " << (enabled ? "enabled" : "disabled") <<
            " - SFC balance checks";
        ...
        doInvariantCheck (enabled,
            {{ "Cannot return non-native STAmount as SFCAmount" }},
            [](Account const& A1, Account const& A2, ApplyContext& ac)
            {
                //non-native balance
                auto const sle = ac.view().peek (keylet::account(A1.id()));
        ...
        doInvariantCheck (enabled,
            {{ "incorrect account SFC balance" },
             {  "SFC net change was positive: 99999999000000001" }},
            [](Account const& A1, Account const&, ApplyContext& ac)
            {
        ...
        doInvariantCheck (enabled,
            {{ "incorrect account SFC balance" },
             { "SFC net change of -1000000001 doesn't match fee 0" }},
            [](Account const& A1, Account const&, ApplyContext& ac)
            {
                // balance is negative
                auto const sle = ac.view().peek (keylet::account(A1.id()));
    ...
    testTransactionFeeCheck(bool enabled)
    {
        ...
        doInvariantCheck (enabled,
            {{ "fee paid was negative: -1" },
             { "SFC net change of 0 doesn't match fee -1" }},
        ...
        doInvariantCheck (enabled,
            {{ "fee paid exceeds system limit: "s +
             ...
             { "SFC net change of 0 doesn't match fee "s +
                std::to_string(SYSTEM_CURRENCY_START) }},
            [](Account const&, Account const&, ApplyContext&) { return true; },
            SFCAmount{SYSTEM_CURRENCY_START});

         doInvariantCheck (enabled,
            {{ "fee paid is 20 exceeds fee specified in transaction." },
             { "SFC net change of 0 doesn't match fee 20" }},
            [](Account const&, Account const&, ApplyContext&) { return true; },
            SFCAmount{20},
            [](STTx& tx) { tx.setFieldAmount(sfFee, SFCAmount{10}); } );
    }
    ...
    testNoBadOffers(bool enabled)
    {
        ...
        doInvariantCheck (enabled,
            ...
            [](Account const& A1, Account const&, ApplyContext& ac)
            {
                ...
                sleNew->setFieldAmount (sfTakerPays, SFC(-1));
                ac.view().insert (sleNew);
                return true;
            });
        ...
        doInvariantCheck (enabled,
            ...
            [](Account const& A1, Account const&, ApplyContext& ac)
            {
                ...
                sleNew->setFieldAmount (sfTakerGets, SFC(-1));
                ac.view().insert (sleNew);
                return true;
            });
        ...
        doInvariantCheck (enabled,
            ...
            [](Account const& A1, Account const&, ApplyContext& ac)
            {
                ...
                sleNew->setAccountID (sfAccount, A1.id());
                sleNew->setFieldU32 (sfSequence, (*sle)[sfSequence]);
                sleNew->setFieldAmount (sfTakerPays, SFC(10));
                sleNew->setFieldAmount (sfTakerGets, SFC(11));
                ac.view().insert (sleNew);
                return true;
            });
    }
    ...
    testNoZeroEscrow(bool enabled)
    {
        ...
        doInvariantCheck (enabled,
            {{ "SFC net change of -1000000 doesn't match fee 0"},
            ...
            [](Account const& A1, Account const&, ApplyContext& ac)
            {
                ...
                auto sleNew = std::make_shared<SLE> (
                    keylet::escrow(A1, (*sle)[sfSequence] + 2));
                sleNew->setFieldAmount (sfAmount, SFC(-1));
                ac.view().insert (sleNew);
                return true;
            });

        doInvariantCheck (enabled,
            {{ "SFC net change was positive: 100000000000000001" },
             {  "escrow specifies invalid amount" }},
            [](Account const& A1, Account const&, ApplyContext& ac)
            {
...
public:
    void run () override
    {
        ...
        for(auto const& b : {false, true})
        {
            ...
            testTypesMatch (b);
            testNoSFCTrustLine (b);
            testSFCBalanceCheck (b);
...
BEAST_DEFINE_TESTSUITE (Invariants, ledger, SupplyFinanceChain);
