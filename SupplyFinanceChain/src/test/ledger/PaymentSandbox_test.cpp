...
#include <SupplyFinanceChain/protocol/AmountConversions.h>
...
class PaymentSandbox_test : public beast::unit_test::suite
{
    ...
    */
    void testSelfFunding (FeatureBitset features)
    {
        ...
        env.fund (SFC (10000), snd, rcv, gw1, gw2);
    ...
    void testSubtractCredits (FeatureBitset features)
    {
        ...
        env.fund (SFC (10000), alice, gw1, gw2);
        ...
        {
            // SupplyFinanceChainCredit, no deferredCredits
            ApplyViewImpl av (&*env.current(), tapNONE);
            ...
            SupplyFinanceChainCredit (av, gw1, alice, toCredit, true, j);
            BEAST_EXPECT(accountHolds (av, alice, iss.currency, iss.account,
                        fhIGNORE_FREEZE, j) ==
                    startingAmount + toCredit);

            SupplyFinanceChainCredit (av, alice, gw1, toDebit, true, j);
            BEAST_EXPECT(accountHolds (av, alice, iss.currency, iss.account,
                        fhIGNORE_FREEZE, j) ==
                    startingAmount + toCredit - toDebit);
        }
        ...
        {
            // SupplyFinanceChainCredit, w/ deferredCredits
            ApplyViewImpl av (&*env.current(), tapNONE);
            PaymentSandbox pv (&av);
            ...
            SupplyFinanceChainCredit (pv, gw1, alice, toCredit, true, j);
            BEAST_EXPECT(accountHolds (pv, alice, iss.currency, iss.account,
                        fhIGNORE_FREEZE, j) ==
                    startingAmount);
        }
    ...
    void testReserve(FeatureBitset features)
    {
        ...
        auto accountFundsSFC = [](ReadView const& view,
            AccountID const& id, beast::Journal j) -> SFCAmount
        {
            return toAmount<SFCAmount> (accountHolds (
                view, id, sfcCurrency (), sfcAccount (), fhZERO_IF_FROZEN, j));
        };

        auto reserve = [](jtx::Env& env, std::uint32_t count) -> SFCAmount
        {
        ...
        PaymentSandbox sb (&av);
        {
            ...
            accountSend (sb, sfcAccount (), alice, SFC(100), env.journal);
            accountSend (sb, alice, sfcAccount (), SFC(100), env.journal);
            BEAST_EXPECT(
                accountFundsSFC (sb, alice, env.journal) == beast::zero);
        }
    }
...
BEAST_DEFINE_TESTSUITE (PaymentSandbox, ledger, SupplyFinanceChain);
