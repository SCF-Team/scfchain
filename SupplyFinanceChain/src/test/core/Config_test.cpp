...
{
    static boost::format configContentsTemplate (R"SFChainConfig(
[server]
port_rpc
port_peer
port_wss_admin
...
# This is primary persistent datastore for SupplyFinanceChaind.  This includes transaction
# metadata, account states, and ledger headers.  Helpful information can be
# found here: https://SupplyFinanceChain.com/wiki/NodeBackEnd
# delete old ledgers while maintaining at least 2000. Do not require an
# external administrative command to initiate deletion.
[node_db]
type=memory
path=/Users/dummy/SupplyFinanceChain/config/db/rocksdb
open_files=2000
filter_bits=12
cache_mb=256
file_size_mb=8
...
[debug_logfile]
/Users/dummy/SupplyFinanceChain/config/log/debug.log
...
# Where to find some other servers speaking the SupplyFinanceChain protocol.
#
[ips]
r.SupplyFinanceChain.com 51235
...
/**
   Write a SupplyFinanceChaind config file and remove when done.
 */
class SupplyFinanceChaindCfgGuard : public SupplyFinanceChain::test::detail::FileDirGuard
{
private:
    path dataDir_;
...
public:
    SupplyFinanceChaindCfgGuard (beast::unit_test::suite& test,
    ...
    ~SupplyFinanceChaindCfgGuard ()
    {
        ...
        catch (std::exception& e)
        {
            ...
            test_.log << "Error in ~SupplyFinanceChaindCfgGuard: "
                      << e.what () << std::endl;
        };
    }
};
...
{
    std::string configContents (R"SFChainConfig(
[validators]
...
recommendedSupplyFinanceChainvalidators.com
moreSupplyFinanceChainvalidators.net
...
public:
    void testLegacy ()
    {
        ...
        std::string toLoad(R"SFChainConfig(
[server]
port_rpc
port_peer
port_wss_admin
...
0
    ...
    void testDbPath ()
    {
        ...
        }
        {
            ...
            auto const cwd = current_path ();
            SupplyFinanceChain::test::detail::DirGuard const g0(*this, "test_db");
            path const dataDirRel ("test_data_dir");
            path const dataDirAbs(cwd / g0.subdir () / dataDirRel);
            detail::SupplyFinanceChaindCfgGuard const g (*this, g0.subdir(),
                dataDirAbs, "", false);
        ...
        }
        {
            ...
            detail::SupplyFinanceChaindCfgGuard const g (*this, "test_db", dbPath, "");
            auto const& c (g.config ());
        ...
        }
        {
            ...
            detail::SupplyFinanceChaindCfgGuard const g (*this, "test_db", "", "");
            auto const& c (g.config ());
    ...
    void testValidatorKeys ()
    {
        ...
        {
            Config c;
            static boost::format configTemplate (R"SFChainConfig(
[validation_seed]
%1%
...
nHBu9PTL9dn2GuZtdW4U2WzBwffyX9qsQCd9CNU4Z5YG3PQfViM8
        ...
        }
        {
            ...
            std::string toLoad(R"SFChainConfig(
[validator_list_sites]
SupplyFinanceChainvalidators.com
...
trustthesevalidators.gov
        ...
        }
        {
            ...
            detail::SupplyFinanceChaindCfgGuard const rcg (
        ...
        }
        {
            ...
            detail::ValidatorsTxtGuard const vtg (
                *this, "test_cfg", "validators.txt");
            detail::SupplyFinanceChaindCfgGuard const rcg (*this, vtg.subdir (),
                "", "", false);
            BEAST_EXPECT(vtg.validatorsFileExists ());
            BEAST_EXPECT(rcg.configFileExists ());
            auto const& c (rcg.config ());
...

        {
            ...
            boost::format cc (R"SFChainConfig(
[validators_file]
%1%
...
021A99A537FDEBC34E4FCA03B39BEADD04299BB19E85097EC92B15A3518801E566
        ...
        }
        {
            ...
            // [validator_list_keys] are missing from SupplyFinanceChaind cfg and
            // validators file
            Config c;
            boost::format cc ("[validators_file]\n%1%\n");
            std::string error;
    ...
    {
        detail::SupplyFinanceChaindCfgGuard const cfg(*this, "testSetup",
            explicitPath ? "test_db" : "", "");
        /* SupplyFinanceChaindCfgGuard has a Config object that gets loaded on
    ...
    void testPort ()
    {
        detail::SupplyFinanceChaindCfgGuard const cfg(*this, "testPort", "", "");
...
r.SupplyFinanceChain.com 51235
  ...
  [ips_fixed])" "\x20\x20" R"(
    # COMMENT
    s1.SupplyFinanceChain.com 51235
    s2.SupplyFinanceChain.com 51235
...
BEAST_DEFINE_TESTSUITE (Config, core, SupplyFinanceChain);
